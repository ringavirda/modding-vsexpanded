...
public abstract class BlockEntityBoiler : BlockEntityMultiblockStructure
{
  ...
  /// volume multiplier - 0 when the medium leaves it to the caller, so we fall back to the lpex
  ...
  /// expansion, <see cref="LpexValues.SteamExpansionFactor"/> by default, L steam).</summary>
  ...
  private float DisplayWaterTemperature() =>
    _state switch
    {
      ...
      BoilerState.Heating => 20f
        + (LpexValues.BoilingPoint - 20f) * HeatProgress,
      _ => 20f,
    };
