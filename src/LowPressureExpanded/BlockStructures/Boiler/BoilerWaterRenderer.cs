...
/// <see cref="Temperature"/> nears boiling and blended see-through like vanilla barrel water. The
/// boiler drives the level in discrete steps (hidden, low, high) so the flat quad lands inside the
/// vessel rather than slicing through its geometry.
/// </summary>
public class BoilerWaterRenderer : SurfaceRenderer {
  private readonly int _textureId;

  /// <summary>
  /// Absolute surface height in block units above the boiler's base cell. 0 or less hides the
  /// surface entirely.
  /// </summary>
  public float SurfaceLevel;

  /// <summary>Water temperature in °C, driving the faint hot-water glow.</summary>
  public float Temperature;

  // Must render after the animated geometry (RenderOrder 1.0): drawing the translucent surface first
  // would write depth and cull the interior below the water line.
  ...
  protected override void ConfigureShader(
    ...
    // Translucent blue tint, see-through like vanilla barrel water. What shows through is the vessel
    // interior, not the world below.
