...
public class BlockEntitySteamCondenser : BlockEntity
{
  ...
  )
  {
    ...
    if (wa != null && ReferenceEquals(wa, wb))
    {
      if (!steam)
        return false;
      float looped =
        steamNet!.TryConsumeGas(LpexValues.CondenserSteamPerSecond * dt, ba)
        / condenseFactor;
      if (looped <= 0f)
        return false;
    ...
    float move =
      inNet != null && passSpace > 0f
        ? inNet.TryConsumeLiquid(
          Math.Min(LpexValues.CondenserWaterThroughput * dt, passSpace),
          ba
        )
        : 0f;
  ...
  )
  {
    ...
    dsc.AppendLine(
      Lang.Get(
        _condensing
          ? "lpex:condenser-info-condensing"
          : "lpex:condenser-info-idle"
      )
    );
  }
}
