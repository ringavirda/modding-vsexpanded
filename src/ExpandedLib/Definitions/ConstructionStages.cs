...
/// <summary>
/// Typed builder for the <c>ExRightClickConstructable</c> behavior's <c>stages</c> table - the staged
/// material/shape blob that was the deepest hand-nested part of a mega-block's JSON. Each stage adds/removes
/// shape elements and optionally requires materials; the builder emits the exact <c>{ "stages": [...] }</c>
/// properties object the vanilla behavior consumes, so authoring a build sequence is a few fluent calls
/// instead of a coordinate-and-quantity JSON pyramid.
/// </summary>
