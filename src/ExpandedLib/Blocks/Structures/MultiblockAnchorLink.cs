...
/// find - and keep reading - the multiblock anchor it belongs to. The anchor pushes to its components
/// by structure-local offset and nothing points back, so the component scans a bounded box for the
/// anchor (<see cref="BlockEntityMultiblockStructure.FindAnchorOwning{T}"/>).
/// <para>
/// That scan is the only expensive part, and <c>GetBlockInfo</c> runs every frame the player looks at
/// the block, so it is re-run at most once a second; a resolved anchor is then cached and re-validated
/// cheaply each call (one block-entity lookup + one ownership check), so the component's HUD reflects
/// the anchor's synced state live without a per-frame scan. When the anchor moves or the structure
/// breaks, the cache clears and a (throttled) re-scan takes over.
/// </para>
/// </summary>
  ...
  where T : BlockEntityMultiblockStructure
{
  ...
  /// <summary>
  /// The owning anchor, or null when none is in range. A still-valid cached anchor is read live (the
  /// common path once resolved); an empty or invalidated cache triggers a bounded re-scan, at most once
  /// per <see cref="RescanIntervalMs"/> so a component with no anchor does not scan every frame.
  /// </summary>
  public T? Resolve()
  {
