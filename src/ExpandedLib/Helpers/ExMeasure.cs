...
/// <summary>
/// Shared display-unit helper used across the mods (iwex pipes/canals, lpex steam machinery, smex).
/// All gameplay values are stored and calculated in metric (litres, atm, °C); these methods convert a
/// metric value into a player-facing string in the currently active <see cref="System"/>, so
/// block-info / handbook code can stay unit-agnostic.
/// <para>
/// It lives in exlib (below every mod) so any tier can format without a cross-mod dependency. The
/// <see cref="System"/> value is a per-player, client-side preference: a content mod (lpex's
/// <c>MeasurePreference</c>) drives it through the library's generic preferences store, applies it for
/// the local player on join, and changes it via <c>.exmod measure</c>. exlib itself only reads it.
/// </para>
/// </summary>
