...
/// <para>
/// Positions are supplied by the network from its own node set, so this contract carries none - it is
/// purely the cell's state and per-tick operations.
/// </para>
/// </summary>
public interface IMoltenCell
{
  ...
  /// <summary>Whether this cell is clay-sealed (a manual valve that severs flow at its position).</summary>
  bool Sealed { get; }

  /// <summary>Whether this cell's metal has solidified (blocks flow until chiselled or broken).</summary>
  bool Solidified { get; }
  ...
  /// <summary>True for a drain fitting (tap, mold pedestal) that takes even a sub-minimum final transfer so a run can empty completely.</summary>
  bool AcceptsSubMinimumFlow { get; }

  /// <summary>Rebuilds the server temperature carrier after a world load (only type + temperature persist).</summary>
  void EnsureMetalStack(IWorldAccessor world);

  /// <summary>Pushes up to <paramref name="amount"/> units of <paramref name="metalType"/> into this cell (temperature-averaging with any same-type metal already present). Returns the amount accepted.</summary>
  int PushMetalRaw(int amount, string metalType, float temperature, IWorldAccessor world);
