...
/// </para>
/// </summary>
public static class ProcessRouteLoader {
  ...
  /// declaration does not cost every other mod its routes. Asset-free, so it runs headless.
  /// </summary>
  ...
  public static List<string> Load(
    ...
    registry ??= ProcessRouteRegistry.Shared;
    ...
    List<ProcessRoute> routes = Parse(sources, out List<string> errors);

    registry.Clear();
    int i = 0;
    foreach (ProcessRoute route in routes) {
      // Parse preserves order and skips nothing silently, so the nth route is the nth file that parsed;
      // a clash is reported against the file that lost, which is the one its author can fix.
      string source = SourceOf(sources, routes, i++);
      foreach (string conflict in registry.Contribute(route))
        errors.Add($"{source}: {conflict}");
    }
    return errors;
  }

  // The file a parsed route came from. Parse drops malformed files, so the indices only line up when
