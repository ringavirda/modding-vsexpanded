...
public static class Burden {
  ...
  private const string OreCode = "iiex:burden";
  ...
  /// <summary>True when <paramref name="stack"/> is the burden item (<c>iiex:burden</c>), the prepared
  ...
  public static bool Is([NotNullWhen(true)] ItemStack? stack) =>
    stack?.Collectible?.Code is { Domain: "iiex", Path: "burden" };
  ...
  public static string ProfileLangKey(BurdenMix mix) {
    if (!mix.HasContent)
      return "iiex:burden-profile-empty";

    foreach (BurdenProfile p in IiexValues.BurdenProfiles)
      if (InBand(mix.FluxFrac, p.MinFlux, p.MaxFlux))
        return "iiex:burden-profile-" + p.Key;

    // Unreachable with the shipped bands (they tile 0..1), but a player's edited config can leave a gap.
    return "iiex:burden-profile-offspec";
  }
