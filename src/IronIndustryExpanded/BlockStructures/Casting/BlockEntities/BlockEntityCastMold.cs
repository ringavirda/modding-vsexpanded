...
public class BlockEntityCastMold : BlockEntity, ILiquidMetalSink {
  ...
  public void ReceiveLiquidMetal(
    ...
    // the two is hotter.
    float reached = temperature * IiexValues.CastMoldHeatSinkFraction;
  ...
  private void OnServerTick() {
    ...
    if (_moldTemperature > IiexValues.MoltenAmbientTemperature) {
      _moldTemperature = Math.Max(
        IiexValues.MoltenAmbientTemperature,
        _moldTemperature - IiexValues.CastMoldBodyCooldownPerSecond
      );
      MarkDirty();
    }
    UpdateGlow();
  ...
  public bool TryTakeOut(IPlayer byPlayer) {
    ...
    if (!IsHardened) {
      (byPlayer as IServerPlayer)?.SendIngameError("iiex-castmold-toohot");
      return true;
    }
  ...
  /// domain, so cast iron comes out as <c>iiex:metalplate-castiron</c> while vanilla metals keep theirs.
  ...
  public override void GetBlockInfo(IPlayer forPlayer, StringBuilder dsc) {
    ...
    if (MetalContent == null || CurrentUnitAmount <= 0) {
      dsc.AppendLine(Lang.Get("iiex:castmold-empty", MaxUnitAmount));
      return;
    }
    string state = Lang.Get(
      MoltenMetal.StateOf(Api.World, MetalContent) switch {
        ...
        _ => "iiex:metalstate-soft",
      }
    );
    dsc.AppendLine(
      Lang.Get(
        "iiex:castmold-units-state",
        CurrentUnitAmount,
        MaxUnitAmount,
        state,
