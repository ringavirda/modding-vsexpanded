...
public abstract class BlockEntityEngine : BlockEntityProductionMachine {
  ...
  public float OverPressureRemaining =>
    _overPressure.Remaining(IiexValues.EngineOverPressureSeconds);
  ...
  /// <summary>
  /// MP load the generator holds at <see cref="IiexValues.MpRatedSpeed"/> with the engine at full
  ...
  /// </summary>
  public float MpRatedLoad => MaxPower * IiexValues.MpLoadPerEnginePower;
  ...
  public float MpPowerBudget =>
    AvailablePower * IiexValues.MpLoadPerEnginePower * IiexValues.MpRatedSpeed;
  ...
  protected override void OnProductionTick(float dt) {
    ...
    if (
      _overPressure.Update(
        overPressure,
        dt,
        IiexValues.EngineOverPressureSeconds
      )
    ) {
      Break();
      return;
  ...
  public override void GetBlockInfo(
    ...
    if (IsBroken) {
      dsc.AppendLine(Lang.Get(IiexLang.EngineInfoBroken));
      return;
    }

    // Inlet pressure against the operating band, plus the steam drawn while running.
    dsc.AppendLine(Lang.Get("iiex:engine-info-clock-" + ClockState));
    if (IsRunning)
      dsc.AppendLine(
        Lang.Get(
          "iiex:engine-info-steam",
          ExMeasure.FlowRate(RunSteamRate, "F0")
        )
      );
    if (_overPressure.IsCounting)
      dsc.AppendLine(
        Lang.Get("iiex:engine-info-overpressure", OverPressureRemaining)
      );
  }
