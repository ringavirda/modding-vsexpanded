...
public class BlockEntityMoltenBarrel
  ...
  public int MaxUnitAmount = IiexValues.BarrelDefaultMaxUnits;
  ...
  // `/exmod config iiex MoltenCooldownSpeed ...` change affects metal already in the barrel,
  ...
  public override void GetBlockInfo(IPlayer forPlayer, StringBuilder dsc) {
    ...
    if (MetalContent == null || CurrentUnitAmount <= 0) {
      dsc.AppendLine(Lang.Get("iiex:moltenbarrel-info-empty", MaxUnitAmount));
      return;
    }
    ...
    dsc.AppendLine(
      Lang.Get(
        "iiex:moltenbarrel-info-units-state",
        CurrentUnitAmount,
        MaxUnitAmount,
        state,
