...
public class BlockEntityMoltenCanalMoldPedestal : BlockEntityMoltenCanal {
  ...
  private static float MoldCooldownSpeed =>
    IiexValues.MoltenCooldownSpeed * IiexValues.MoldPedestalCooldownCoefficient;
  ...
  public override void GetBlockInfo(IPlayer forPlayer, StringBuilder dsc) {
    ...
    if (!IsMold) {
      dsc.AppendLine(Lang.Get("iiex:moldpedestal-nomold"));
      return;
    }

    if (MoldMetalContent == null || MoldCurrentUnits <= 0) {
      dsc.AppendLine(Lang.Get("iiex:mold-empty"));
      return;
    }
