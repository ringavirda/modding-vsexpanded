    ...
    ExBlockDef def = ExBlockDef
      ...
      .Construction(c =>
        c.Stage(s => s.AddElements("Base"))
          .Stage(s =>
            // iwex's own cast gear: iwex must not require an lpex item, which would leave this
            // unbuildable for an iwex-only player.
            s.Require(
                "iwex:" + Items.SpurGearItemDefinitions.Code,
                2,
                "iwex:rcc-ingredient-transmissiongears"
              )
              .Require(
                "game:ingot-iron",
                4,
                "iwex:rcc-ingredient-transmissionshafts"
              )
          ...
          .Stage(s =>
            s.Require(
                "game:ingot-iron",
                2,
                "iwex:rcc-ingredient-transmissionsupport"
              )
