...
/// <see cref="StageLadder"/> the set's roller family accepts, walked thickest first. The two halves are
/// declared apart on purpose - the set carries the tooling's own limits, the ladder carries the states -
/// so the mill reads a schedule off the pair and names no product itself.
/// <para>
/// A fork is why the branch and not the ladder is the unit: the same stock at the same gauge is a rod on
...
public sealed class MillSchedule {
  ...
  /// <summary>The stock family's whole ladder, both branches of a fork included.</summary>
  public StageLadder Ladder { get; }
  ...
  /// none - no set fitted, tooling that will not bite that stock, a stock family with no ladder, or a
  /// ladder with no stage this set's family accepts.
  /// </summary>
  public static MillSchedule? For(
    ...
    StageLadder? ladder = (registry ?? StageLadderRegistry.Shared).Ladder(form);
    if (ladder == null)
      return null;

    ProcessStage[] stages = [.. ladder.AcceptedBy(set.Family)];
    return stages.Length == 0 ? null : new MillSchedule(set, ladder, stages);
  }
  ...
  /// code and the piece stays stock. Matched with the ladder's own tolerance, because the gauge a piece
  /// carries is arrived at by arithmetic rather than by re-reading the literal.</summary>
  public string? OutputAt(float thickness) =>
    Ladder.StageAt(thickness, Set.Family)?.Code;
}
