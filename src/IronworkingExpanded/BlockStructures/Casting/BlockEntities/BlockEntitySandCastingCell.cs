...
public class BlockEntitySandCastingCell : BlockEntity {
  ...
  /// <c>lpex:pattern-cylinder-oak</c>). The code carries its own domain, so a pattern shipped by another
  ...
  public override void GetBlockInfo(IPlayer forPlayer, StringBuilder dsc) {
    ...
    if (!HasImpression) {
      dsc.AppendLine(
        Lang.Get(
          _sand == SandLevel.Full
            ? "iwex:castingcell-needspattern"
            : "iwex:castingcell-needssand"
        )
      );
      return;
    }
    ...
    string state = cell.IsHardened
      ? Lang.Get("iwex:metalstate-hardened")
