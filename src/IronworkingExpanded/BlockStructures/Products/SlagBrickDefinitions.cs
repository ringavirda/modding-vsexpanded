...
/// <summary>
/// Slag-brick masonry: the block eight cast <c>slagbrick</c>s and a dab of mortar are laid up into.
/// <para>
/// Modelled directly on vanilla's <c>stonebricks</c> - a plain cube, stone material, pickaxe-mined - because
/// this is meant to be an ordinary building block and the surest way to make it feel ordinary is to give it
/// exactly the surface players already know. It uses a vanilla class; there is no behaviour here worth
/// writing.
/// </para>
/// <para>
/// It is the terminal product of the slag line, and the line closes on itself rather neatly: the bricks are
/// cast from tapped slag in the bed's brick molds, and the mortar that lays them up is itself made from
/// ground slag and slaked lime (smex's <c>mortarfromslag</c>). A furnace's waste builds the walls around it,
...
public class SlagBrickDefinitions : IExBlockDefProvider
{
  ...
  // Everything the three share: cast slag is a hard, brittle stone, so the whole family chisels, takes a
  // pickaxe and sounds like stone - exactly vanilla's stonebrick trio, which is the surface being copied.
  private static ExBlockDef Common(ExBlockDef def) =>
    def.Material(EnumBlockMaterial.Stone)
      ...
      .SoundByTool(EnumTool.Pickaxe, "game:block/rock-hit-pickaxe", "game:block/rock-break-pickaxe")
  ...
  private static ExBlockDef SlagBricks(string domain) =>
    Common(ExBlockDef.Create(domain, "slag-bricks", "slag/bricks"))
      ...
      // Base + overlay, the way vanilla's coloured bricks are built - not the overlay alone.
      // `slagbrick` is 82 % transparent: it is the brick coursing and mortar lines, drawn to sit on a
      // surface. Used as the whole texture it left the block four-fifths see-through with nothing
      // behind it. `slag` is the opaque cast-slag surface (the same one the raw item wears), so the
      // pair reads as slag laid up into courses, which is what the block is.
      .Texture(
        "all",
  ...
  // wrench. `*-up-snow` is skipped because snow cannot sit under an upward-facing slab.
  private static ExBlockDef Slab(string domain) =>
    Common(ExBlockDef.Create(domain, "slag-brickslab", "slag/brickslab"))
      ...
      .Behavior("WrenchOrientable", new { baseCode = "slag-brickslab-*-{cover}" })
      ...
      .AttributeByType("liquidBarrierOnSidesByType", "*-down-*", new[] { 0.5, 0.5, 0.5, 0.5 })
      .Replaceable(200)
      ...
      .RawByType("sideSolidOpaqueAoByType", "*-north-*", new { all = false, north = true })
      .RawByType("sideSolidOpaqueAoByType", "*-east-*", new { all = false, east = true })
      .RawByType("sideSolidOpaqueAoByType", "*-south-*", new { all = false, south = true })
      .RawByType("sideSolidOpaqueAoByType", "*-west-*", new { all = false, west = true })
      .RawByType("sideSolidOpaqueAoByType", "*-up-*", new { all = false, up = true })
      .RawByType("sideSolidOpaqueAoByType", "*-down-*", new { all = false, down = true })
      .SideAo(true)
      .RawByType("emitSideAoByType", "*-up-*", new { all = false, up = true })
      .RawByType("emitSideAoByType", "*-down-*", new { all = false, down = true })
  ...
  // Vanilla's stonebrickstairs, trimmed to the up-facing half the slag paths already ship (a down-facing
  // stair is a separate art set this family does not have a use for yet).
  private static ExBlockDef Stairs(string domain)
  {
    ...
    return Common(ExBlockDef.Create(domain, "slag-brickstairs", "slag/brickstairs"))
      .Class("BlockStairs")
      .Behavior("WrenchOrientable", new { baseCode = "slag-brickstairs-up-*-{cover}" })
      ...
      .CreativeTab("iwex", "*-up-north-free")
...
{
  /// <summary>Stairs cull as stairs when bare and never when snow-capped - vanilla's split.</summary>
