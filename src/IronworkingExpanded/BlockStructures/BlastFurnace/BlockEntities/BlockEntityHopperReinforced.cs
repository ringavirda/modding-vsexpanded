...
public class BlockEntityHopperReinforced : BlockEntityContainer
{
  ...
  private void ToggleDialog(ICoreClientAPI capi, IPlayer byPlayer)
  {
    ...
    _invDialog = new GuiDialogHopper(
      Lang.Get("iwex:hopper-dialog-title"),
      Inventory,
      Pos,
      capi
    );
  ...
  public override void GetBlockInfo(IPlayer forPlayer, StringBuilder dsc)
  {
    ...
    )
    {
      ...
      {
        dsc.AppendLine(Lang.Get("iwex:hopper-info-furnacefull"));
      }
    }
    else
    {
      dsc.AppendLine(Lang.Get("iwex:hopper-info-nobell"));
    }
  }
  ...
  private MeshData? BuildContentsMesh(ITesselatorAPI tesselator)
  {
    Shape? shape = Api
      .Assets.TryGet(
        new AssetLocation("iwex:shapes/blastfurnace/contents-blastmix.json")
      )
      ?.ToObject<Shape>();
    if (shape == null)
      return null;
...
public class GuiDialogHopper : GuiDialogBlockEntity
{
    ...
    : base(dialogTitle, inventory, blockEntityPos, capi)
  {
    ...
    SingleComposer = capi
      ...
      .AddDynamicText(
        Lang.Get("iwex:hopper-slot-iron"),
        ironFont,
        ironTextBounds
      )
      .AddDynamicText(
        Lang.Get("iwex:hopper-slot-coke"),
        cokeFont,
        cokeTextBounds
      )
      .AddDynamicText(
        Lang.Get("iwex:hopper-slot-flux"),
        limeFont,
        limeTextBounds
      )
