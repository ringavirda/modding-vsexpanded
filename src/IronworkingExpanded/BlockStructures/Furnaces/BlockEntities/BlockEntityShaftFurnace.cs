...
public abstract class BlockEntityShaftFurnace : BlockEntityFurnaceCore {
  ...
  /// furnace makes molten pig iron (crude high-carbon iron, <c>iwex:ingot-pigiron</c>); plain iron comes
  ...
  /// <see cref="IwexValues.BfIronPerOreUnit"/>.
  /// </summary>
  ...
  /// </summary>
  /// <remarks>
  /// Two constants need it. <see cref="IwexValues.BfRacewayCarbonPerTuyerePerSecond"/> is a rate in charge
  /// units, so it scales directly; <see cref="IwexValues.BfShaftGasTransferFrac"/> is a fraction absorbed
  ...
  /// <summary>
  /// Re-reads the product and charge tunables alongside the core's, so a live <c>/exmod config iwex ...</c>
  ...
  /// into the burden's stamp, so a column holds <c>iwex:burden</c> and <c>game:coke</c> side by side. The
  ...
  /// <see cref="IwexValues.BfBurdenPerCarbonUnit"/>, scaled by <c>MeltSpeedFactor</c> so a hotter furnace
  ...
  protected override void SmeltCycle(object chargeHandle, float dt) {
    float budget =
      (
        ...
        * IwexValues.BfBurdenPerCarbonUnit
        * MeltSpeedFactor()
      ) + _meltCarry;
