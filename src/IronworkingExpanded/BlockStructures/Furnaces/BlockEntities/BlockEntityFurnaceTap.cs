...
/// <summary>
/// Block entity for a shaft-furnace tap-hole - the iron notch and the cinder notch alike. Tracks the
/// pouring state (shown via the shape's open animation pose) and accepts molten metal pushed from the
/// furnace to hand down into the canal start beneath it.
/// </summary>
[BlockEntityRegister]
public class BlockEntityFurnaceTap : BlockEntity, IMultiblockComponent
{
  ...
  // Both taps of a furnace run this same block entity; each scans up to the furnace core it belongs to
  // and shows the pool it drains - the lower (metal) tap the metal, the upper the slag. Which one a tap
  // is is decided by comparing its cell against the core's Metal/SlagTapPos.
  //
  // The block is now typed (irontap / slagtap) and the layouts pin the type per cell, so type and
  // position agree by construction. The positional test is kept as the gate anyway, because it answers a
  // question the type cannot: whether this furnace's drawing marks this cell as a tap at all - a
  // reverberatory hearth marks neither, and a tap stuck on one must show no pool.
  //
  ...
  private void BuildAnimator(BEBehaviorAnimatable animatable)
  {
    ...
    // string: the two tap types share a shape today but will not always, and a stale key would serve one
    // type's mesh for the other. A code cannot drift from the block it came off.
  ...
  public override void GetBlockInfo(IPlayer forPlayer, StringBuilder dsc)
  {
    ...
    // and so does a tap standing on a furnace whose drawing marks no tap at all: both Metal/SlagTapPos are
    // null on a reverberatory hearth, which pours nothing.
  ...
  public int TryPourMetal(ItemStack moltenMetal, float temperature)
  {
    ...
    // ExOrientation.FacingFromSide, not BlockFacing.FromCode. The null guard below is real and
    // already correct - which is exactly what made the old call dangerous rather than merely wrong:
    // FromCode returns null for a single-letter token, so a letter-spelled side would make this return
    // 0 forever and the tap would silently never pour. No error, no log, just a furnace that fills up
    // and will not drain.
