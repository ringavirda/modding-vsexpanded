...
/// string (<c>Code.ToShortString()</c>, e.g. <c>iwex:burden</c>, <c>game:coke</c>) rather than an
...
public sealed class ChargeColumn {
  ...
  // arrives as a parameter rather than being read off IwexValues, so both the renderer and the block
  ...
  /// block to draw it. <c>SyncChargeBlocks</c> places and removes <c>iwex:furnace-chargepile</c> as this
  /// crosses a boundary.
  /// </summary>
