...
public abstract class BlockEntityFurnaceCore : BlockEntityMultiblockStructure {
  ...
  /// whatever its coke shortfall against <see cref="IwexValues.BfReferenceFuelFrac"/> adds, clamped. A
  ...
  public float RequiredBlastPressureFor(BurdenMix mix) {
    ...
    return GameMath.Clamp(
      BlastPressureThreshold
        + shortfall * IwexValues.BfBlastPressureCokeSensitivity,
    ...
    foreach (var (key, column) in _shaftColumns) {
      ...
      for (int i = 0; i < local.Count; i++) {
        ...
        if (i < want && !has) {
          ...
          // the crucible, which the shipped drawings mark Chargeable and Pool at once: `iwex:hearthmetal`
  ...
  /// <see cref="IwexValues.BfFuelCarbonReference"/>.
  /// </summary>
  ...
  protected override void OnProductionTick(float dt) {
    ...
    // Re-read the tunables each tick so a live `/exmod config iwex ...` change takes effect immediately
  ...
  private void SolidifyBottomLayer() {
    ...
    foreach (BlockPos pos in PoolCells) {
      ...
      // `iwex:furnace-chargepile`, not `game:coalpile`; testing for the wrong code reads as "no free cell"
