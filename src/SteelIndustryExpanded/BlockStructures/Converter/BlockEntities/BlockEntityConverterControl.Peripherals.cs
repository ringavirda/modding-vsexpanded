...
public partial class BlockEntityConverterControl {
  ...
  private float TryConsumeBlast(float amount) {
    ...
    if (
      ...
      || pipeNet.State.Pressure < SiexValues.BlastPressureThreshold
    )
      return 0f;
  ...
  public bool TrySpawnConverter(IPlayer byPlayer, out string error) {
    ...
    if (IsConverterPresent()) {
      error = Lang.Get("siex:bessemer-err-converter-present");
      return false;
    }
    ...
    if (existing.Id != 0 && !existing.IsReplacableBy(converter)) {
      error = Lang.Get("siex:bessemer-err-converter-blocked");
      return false;
    }
    ...
    if (!isCreative && !HasSpawnMaterials(byPlayer)) {
      error = Lang.Get(
        "siex:bessemer-err-materials",
        SiexValues.BessemerRequiredGears,
        SiexValues.BessemerRequiredRods
      );
      return false;
    }
  ...
  private Block? GetConverterBlock() {
    ...
    return Api.World.GetBlock(
      new AssetLocation("siex:" + BlockConverterBessemer.BaseCode + "-" + side)
    );
  }
