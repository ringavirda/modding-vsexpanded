# Changelog - Steel Industry Expanded (`siex`)
...
- **Renamed to Steel Industry Expanded (`siex`), absorbing High Pressure Expanded.**
  The steel line and the high-pressure steam leaves ship as one mod and one domain.
  Every placed block, block entity and config value carries over automatically:
  `smex:` codes are remapped on load, the block-entity class strings saved in your
  world resolve through aliases, and your tuning in the `smex` (and `hpex`) sections
  of `ex_values.json` / `ex_recipes.json` is carried into the `siex` section.
- The **Lancashire boiler**, **Cornish engine** and the **rolled pipe tier** now come
  from this mod rather than a separate one; High Pressure Expanded no longer exists
  and should be removed after updating.
- Dependencies are now just `exlib` and `iiex`.
- `/exmod config smex` and `/exmod recipes smex` become `/exmod config siex` and
  `/exmod recipes siex`. The retired `/exmod steel` alias and its strings are gone.
