...
public sealed class AssetStore {
  ...
  private readonly Dictionary<string, Dictionary<string, JToken>> _byDomain = new(
  ...
  public void AddRoot(
    ...
    foreach (string domainDir in Directory.EnumerateDirectories(assetsDir)) {
      string domain = (remapDomain?.Invoke(Path.GetFileName(domainDir)) ?? Path.GetFileName(domainDir))
        .ToLowerInvariant();
      Dictionary<string, JToken> files = _byDomain.TryGetValue(domain, out var existing)
        ? existing
        : _byDomain[domain] = new Dictionary<string, JToken>(StringComparer.Ordinal);

      foreach (string file in Directory.EnumerateFiles(domainDir, "*.json", SearchOption.AllDirectories)) {
        string relPath = Path
        ...
        try {
          ...
          onParseError?.Invoke(file, reader.Message, reader.LineNumber, reader.LinePosition);
  ...
  public IEnumerable<(string Path, JToken Json)> Under(string domain, string prefix) {
  ...
  public void Set(string domain, string path, JToken patched) {
    Dictionary<string, JToken> files = _byDomain.TryGetValue(domain.ToLowerInvariant(), out var existing)
      ? existing
      : _byDomain[domain.ToLowerInvariant()] = new Dictionary<string, JToken>(StringComparer.Ordinal);
