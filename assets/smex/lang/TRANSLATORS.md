...
- **Translate values, never keys.** Every key must stay byte-for-byte identical to `en.json`.
  Keys ending in `*` (e.g. `block-cowperstove-*`) and keys starting with `game:` are normal -
...
The unit symbols live in the `unit-*` keys (in the **iiex** domain), and you may localize them:
...
- **Handbook prose converts itself.** When a player is in imperial mode, the mod rescans the article
  text and rewrites every metric value to imperial *on the fly* (and re-renders when you switch units
  ...
  - **Steelmaking Expanded handbook prose uses iiex's `unit-*` symbols too** (the conversion is
    shared). Keep the metric symbols in `smex` articles identical to your iiex `unit-*` values
    (`°C`, `atm`, `L`, `L/s` => your translations of them).

## handbooksearch links

`<a href="handbooksearch://blast furnace">…</a>` runs an in-game handbook *search* for that text.
