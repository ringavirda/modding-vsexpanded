...
> sources.
>
> ★ **Triaged 2026-08-14: still live.** This file's findings were always scoped to the unstarted units
> (U4.4 onward, U6–U11), and those units are *still* unstarted — see the state table in
> [the expansion plan](2026-08-04-iwex-u2-u10-expansion.md). Mentions of U2, U3 and U5 are context, not
> findings. Nothing here was superseded by the two merges.
...
- **Why it matters:** The firebox is a required cell in both reverberatory layouts now, not an optional coal pile: mods/iiex/tests/goldens/iiex/blocktypes/furnace/puddlingcore.json declares `iwex:furnace-firebox-*-*` at (-5,1,0) and heatingcore.json declares two. Verified there is no recipe: nothing under mods/iiex/src/Recipes/ outputs it and there is no goldens/iiex/recipes/grid/firebox.json. So U6's own Gate — "A player builds a puddling furnace from craftable blocks … loads the firebox with coke" — cannot pass, and U6.1's whole point (make both layouts buildable by a player) is undone by a missing fifth block. docs/design/machines/reheat-furnace.md:431 lists the same gap explicitly ("No recipe for core, hearth, charge door or **firebox**"). Note U9.4 Step 3 does exactly this diligence for the coke oven's chargelid ("the one shipped block in the coke-oven layout with no recipe at all … without it the layout cannot be built") — U6 needs the same step.
...
- **Why it matters:** The plan is right and the doc is stale. The same-dated firebox cutover moved both reverberatory furnaces off the coal pile (BlockPuddlingFurnaceCore.cs:110 and BlockHeatingFurnaceCore.cs:72 now legend `F` -> IiexBlocks.FurnaceFirebox.Any; BlockPuddlingFurnaceCore.cs:104 records 'This was `c` -> `@(air|coalpile)` until 2026-08-03'), and the shaft furnaces moved to IiexCodes.ChargeShaft. The danger is the Settled banner at :55: an implementer who reads coke-oven.md alone will rebuild a coal-pile dependency, and VanillaCodes.cs:310's own warning is that `@(air|coalpile)` completes a structure with no fuel cell built at all. U9.1 Step 2 already instructs recording the reversal in the page — that edit is the fix and must not be skipped.
...
- **Why it matters:** Five of the eight rows in the table under that sentence are already wrong: the puddling and reheat furnaces now legend `F` -> IiexBlocks.FurnaceFirebox.Any (BlockPuddlingFurnaceCore.cs:110, BlockHeatingFurnaceCore.cs:72) and the cold blast furnace, cupola and hot blast furnace now legend `c` -> IiexCodes.ChargeShaft (`*:@(air|coalpile|furnace-chargepile)`, BlockBlastFurnaceCoreCold.cs:101, BlockCupolaFurnaceCore.cs:79, BlockBlastFurnaceCoreHot.cs:87). Gotcha 1 at :265 ('Nothing burns coke as coke. Roles.Fuel has two call sites, both in the mixer') is dead the same way. This is the page the coke oven's input filter belongs to, so U9.1's substrate decision is being taken against a description of the world before the firebox cutover — the exact failure shape this file exists to catch, one page upstream.
...
- **Plan says:** "Produces: iwex:steelcrucible-{burned|smelted}" with its blocktype golden created at `goldens/iiex/blocktypes/furnace/steelcrucible.json`; Step 5 then treats the code as an open choice ("Either name it so the prefix matches, or extend both caches")
...
- **Plan says:** U7.3 Step 8: 'Since the stages are now drawn (ruling 1: no redraws), write scripts/export-stock-stages.py to lift each named element out of the editable family file … and emit mods/iiex/assets/iiex/shapes/forming/stock-{form}-{key}.json for keys 300/275/250/225/200/175/150/125/100. Delete the ten existing generated files'
- **Why it matters:** Verified by parsing workbench/shapes/item-shingled-slab.json: it has exactly one top-level element, ShingledSlab1 — confirming the doc. So eight of the nine slab keys have no source element, while the plan deletes the five existing stock-slab-* files outright. StockItemDefinitions still emits stock-slab (StockForm.Slab stays in All), so IiexDefinitionGoldenTests.Every_shape_reference_resolves_to_a_shipped_file (the test U7.4 Step 9 leans on) and the rewritten RolledStockStagesTests over the nine-gap list have nothing to resolve or measure for the slab. The plan's own U7.3 Step 1 'measure the art first' inventory lists only item-shingled-bar / item-rolled-beam / item-rolled-rod and never mentions the slab, so the gap is invisible until the script runs.
...
- **Plan says:** Create `mods/iiex/src/BlockStructures/Forming/BlockEntities/BlockEntityMpBench.cs` — `public abstract class BlockEntityMpBench : BlockEntityNetworkNode, IMpEnergyConsumer`, the shared base of all three benches
...
- **Plan says:** "Produces: `iwex:puddled-ironball` at 200 u", with its shape at `mods/iiex/assets/iiex/shapes/item/puddled-ironball.json` (no `puddled/` folder)
...
- **Doc says:** | `workbench/shapes/item-rollers-flat.json` | four 4-voxel barrel segments named `20Gap`/`15Gap`/`10Gap`/`05Gap`, up-and-down roll pairs | iwex `flat` | drawn to the **shipped** schedule, not the settled one |
