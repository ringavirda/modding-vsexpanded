...
> | **U9** coke oven, crucible steel | ★★ **DONE 2026-08-22** — all eleven tasks. The coke oven bakes, cold blister steel crushes to charge, and the crucible furnace runs coal to a crucible-steel ingot end to end, craftable and documented |
> | **U10** connector check | **DONE** 2026-08-22 |
> | **U11** ladle | **DESIGN ONLY** — art drawn, no code |
>
> ⛔ Nothing in U2-U10 is live any more: **U8, U9 and U10 all closed 2026-08-22** (U6 closed 2026-08-21, U7 on the same day). All of it is a
...
- **`castwheelsection` is iwex's; `castshell` is lpex's** *(the ownership split, settled 2026-08-04)*. Each part lives with the
  ...
  **U1.5 shipped 2026-08-04** — both items, both patterns (and so both diagrams and both carve recipes,
  ...
  - **48 of the 68 drawn diagram sheets are for units not yet built** — every furnace, the burdenmaker,
    ...
    `workbench/textures/diagrams/` before planning any diagram work.** Every *declared* type has its
...
Position (2026-08-22): Phase 0 and **U2-U10 are all done - the plan is CLOSED**; U9 is closed - the coke oven is finished and craftable, the crucible art is exported, crucible steel is a metal, the pot is clayformable, cold blister steel crushes into charge, and the crucible furnace runs coal to a crucible-steel ingot end to end, craftable and documented; and **U10 closed the connector hole** - a layout demands an outward connector face instead of pinning a node's variant, and pinning one is refused at build. ⛔ U7 carries one blocked residue — U7.10, the roll sets, which the machining-line ruling re-homed on the unbuilt lathe.
...
**Shared files** (collision risk): `mods/iiex/src/BlockStructures/Furnaces/BlockEntityFurnaceCore.cs — U2 (CollectCharge/EnumerateChargePiles seam), U3 (FSM dissolution), U6 (puddling); U4 removes the SolidProductBlock/DrainedMetalUnits/StampSolidProduct/ClearMoltenPools quartet at :1394-1410 and rewires ExtinguishResidue at :1453-1458`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityShaftFurnace.cs — U2 (ReadChargeMix/SmeltCycle onto columns), U3 (raceway + melt condition), U5 (AcceptedFamilies when the ore/remelt family model dies); U4 rewrites the whole 'Products: capacity, drain, residue' region (:329-443) and the serialization region (:445-464)`, `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockBlastFurnaceCoreCold.cs — U3 moves the tuyere glyphs to y=2; U4.5 deletes `.Role('h', CellRole.Chargeable)` at :120. Both re-bless goldens/iiex/blocktypes/furnace/blastcore.json`, `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockCupolaFurnaceCore.cs — U5.3 (cupola charges pig and scrap directly); U4.5 deletes `.Role('h', CellRole.Chargeable)` at :90`, `mods/siex/src/BlockStructures/HotBlastFurnace/Blocks/BlockBlastFurnaceCoreHot.cs — U2/U3 bring it along as a ShaftFurnace leaf; U4.6 either moves its `S` glyph from Layer 2 to Layer 1 or explicitly defers it`, `mods/iiex/src/IiexConfig.cs — U2 (BlastMixRequiredToFire, HopperTallPileCap), U3 (all the FSM timers), U5 (burden grade bands); U4 deletes :274, :277, :333, :336 and adds the hearth-band + plug-cost constants`, `mods/iiex/src/IiexCodes.cs — U2 touches `ChargeShaft`; U4.1 rewrites `HearthCell` at :98-99. Only two constants in the file — a merge here is near-certain`, `mods/iiex/src/Generated/IiexBlocks.g.cs — regenerated by every unit that adds or renames a block; U4.1 collapses two entries into one variant-grouped class. Never hand-edit; always `EXLIB_WRITE_BLOCKCODES=1``, `mods/exlib/src/Blocks/Structures/BEBehaviorMoltenCell.cs — U1's casting route is its other live consumer; U4.3 changes its serialization keys. The default must stay `mc_` or every sand-casting-bed save breaks`, `mods/iiex/tests/Fixtures/FurnaceLayoutRig.cs — U2, U3 and U5 all rewrite parts of it; U4 touches `HearthGlyph` (:106-107) and the Chargeable/Pool role-glyph assertions (:630-657)`, `mods/iiex/tests/Fixtures/ColdBlastFurnaceScenes.cs — U2 rewrites ChargePile/Recharge/PileAtLocal onto columns; U4 rewrites the solid-block registration (:218-231) and the MoltenIron/MoltenSlag accessors (:611-613)`, `mods/iiex/tests/Fixtures/CupolaScenes.cs — U2 and U5.3 both rewrite it; U4 touches :77 and :334-350`, `mods/iiex/tests/Scenarios/ColdBlastFurnaceScenarioTests.cs — U2's gate is 'green with no edits to its assertions'; U4 must edit the extinguish case's block-code literal at :429 and the pool-reading assertions at :400-450`, `mods/siex/tests/Blocks/HotBlastFurnace/BlastFurnaceLifecycleTests.cs and BlastFurnaceTests.cs — U2/U3 own most of it; U4 touches the `_moltenIron`/`_moltenSlag` reflection at :114-165, :341, :628 and :88-90, :148-149`, `mods/iiex/assets/iiex/lang/{en,ru,uk}.json — every unit adds keys; U4 renames four block keys and adds the plug + blow-in lines. `LangParityTests` fails if ru/uk lag`
...
**Files**
- Create: `mods/iiex/tests/Blocks/Furnaces/HearthMetalCellTests.cs`
- Modify: `mods/iiex/src/BlockStructures/Products/Blocks/BlockHearthMetal.cs`, `mods/iiex/src/BlockStructures/Products/BlockEntities/BlockEntityHearthMetal.cs`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityShaftFurnace.cs:41-49,92-95,106-114,204-239,331-414,420-441,447-462,473-495`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityCupolaFurnace.cs:71-73`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntityFurnaceCore.cs:1394-1410,1453-1508`, `mods/iiex/src/IiexConfig.cs:274,277,333,336`, `mods/iiex/tests/goldens/iiex/blocktypes/hearthmetal.json`
- Test: `mods/iiex/tests/Blocks/Furnaces/HearthMetalCellTests.cs`, `mods/iiex/tests/Blocks/Furnaces/BlastFurnaceTapTests.cs:212-346`, `mods/iiex/tests/Blocks/Furnaces/FurnaceHudDistributionTests.cs:150-280`, `mods/iiex/tests/Fixtures/ColdBlastFurnaceScenes.cs:611-613`, `mods/iiex/tests/Scenarios/ColdBlastFurnaceScenarioTests.cs:400-450`, `mods/iiex/tests/Fixtures/CupolaScenes.cs:334-350`, `mods/siex/tests/Blocks/HotBlastFurnace/BlastFurnaceLifecycleTests.cs:114-165,341,628`, `mods/siex/tests/Blocks/HotBlastFurnace/BlastFurnaceTests.cs:88-150`
...
- [x] **Step 12.** Re-bless `goldens/iiex/blocktypes/hearthmetal.json` (the def gained two entity behaviours) with `EXLIB_WRITE_GOLDENS=mods/iiex/tests/goldens/iiex/blocktypes/hearthmetal.json`.
...
> overlap. New `mods/siex/tests/Blocks/HotBlastFurnace/CrucibleOverlapTests.cs` pins what
...
**Files**
- Modify: `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockBlastFurnaceCoreCold.cs:115-121`, `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockCupolaFurnaceCore.cs:88-91`, `mods/iiex/tests/goldens/iiex/blocktypes/furnace/blastcore.json`, `mods/iiex/tests/goldens/iiex/blocktypes/furnace/cupolacore.json`
- Test: `mods/iiex/tests/Blocks/Furnaces/ChargeableCellsTests.cs`, `mods/iiex/tests/Blocks/Furnaces/ShaftColumnsTests.cs`, `mods/iiex/tests/Blocks/Furnaces/FurnaceRoleCellsTests.cs`, `mods/iiex/tests/Fixtures/FurnaceLayoutRig.cs:630-658`
...
>
> ⛔ **Step 1's command needs the editable's subfolder**: the converter joins `workbench/shapes` with
...
**Files**
- Create: `mods/iiex/assets/iiex/shapes/furnace/irontap.json`, `mods/iiex/assets/iiex/shapes/furnace/slagtap.json`
- Modify: `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockFurnaceTap.cs:33-39,57-88`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityFurnaceTap.cs:27,57-89,98-116`, `mods/iiex/tests/goldens/iiex/blocktypes/furnace/irontap.json`, `mods/iiex/tests/goldens/iiex/blocktypes/furnace/slagtap.json`, `mods/iiex/assets/iiex/shapes/furnace/tap.json (delete once nothing references it)`
- Test: `mods/iiex/tests/Definitions/IiexDefinitionGoldenTests.cs`, `mods/iiex/tests/Blocks/Furnaces/BlastFurnaceTapTests.cs`

**Consumes:** ExBlockDef.ShapeByTypePerOrientation(string baseShape, int offset = 0) (ExBlockDef.cs:308-317); ExBlockDef.ShapeByType(wildcard, base, rotateX?, rotateY?, rotateZ?); infra/tools/convert-shape.py (texture keys `front1` and `burned` are both already mapped, convert-shape.py:66,72)
...
- [x] **Step 1.** Export both shapes: `python infra/tools/convert-shape.py furnace-block-irontap mods/iiex/assets/iiex/shapes/furnace/irontap.json furnace-block-slagtap mods/iiex/assets/iiex/shapes/furnace/slagtap.json`. Verify the output keeps the top-level `ClayPlug` element in both.
- [x] **Step 2.** Caution: Establish the fact that couples this task to U4.7 before writing any code: the two new shapes have **no `animations` array at all** (verified by walking both files), while the shipped `mods/iiex/assets/iiex/shapes/furnace/tap.json` has one `open` clip and a `Lid` element. So `BlockEntityFurnaceTap.ApplyPourPose` (:98-116) calls `util.StartAnimation(Animation="open")` against a clip that will not exist. Adopting the shapes therefore deletes the pour pose whether or not you intend it. Plan U4.6 and U4.7 as one landing.
...
- [x] **Step 6.** Delete `mods/iiex/assets/iiex/shapes/furnace/tap.json` and confirm `IiexDefinitionGoldenTests.Every_shape_reference_resolves_to_a_shipped_file` still passes (it is what catches a stale path).
- [x] **Step 7.** Re-bless both tap goldens with `EXLIB_WRITE_GOLDENS=mods/iiex/tests/goldens/iiex/blocktypes/furnace/irontap.json,mods/iiex/tests/goldens/iiex/blocktypes/furnace/slagtap.json`.
...
**Files**
- Create: `mods/iiex/tests/Blocks/Furnaces/FurnaceTapPlugTests.cs`
- Modify: `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityFurnaceTap.cs:24-25,55-62,91-118,120-140,142-172`, `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockFurnaceTap.cs:95-187`, `mods/iiex/src/IiexConfig.cs`, `mods/iiex/assets/iiex/lang/en.json`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`
- Test: `mods/iiex/tests/Blocks/Furnaces/FurnaceTapPlugTests.cs`, `mods/iiex/tests/Blocks/Furnaces/BlastFurnaceTapTests.cs:86-114`

**Consumes:** ExShapeElements.Pruned(Shape shape, IReadOnlyCollection<string> keep) (mods/exlib/src/Helpers/ExShapeElements.cs:59-64); ExShapeElements.Matches(string path, IReadOnlyCollection<string> patterns) (:33-45); ExMesh.RotateByShape(MeshData, Block); the fireclay precedent BlockMoltenCanal.cs:382 (`game:clay-fire`) with IiexValues.CanalSealClayCost / .CanalUnsealClayRefund
...
- [x] **Step 3.** Render the plug with `OnTesselation`, copying `BlockEntityPuddlingHearth.OnTesselation` (mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityPuddlingHearth.cs:107-130) verbatim in structure: load the block's own shape via `Api.Assets.TryGet(Block.Shape.Base.Clone().WithPathPrefixOnce("shapes/").WithPathAppendixOnce(".json"))`, call `ExShapeElements.Pruned(shape, keep)` with `["Base", "TapCanal", "ClayPlug"]` when plugged and `["Base", "TapCanal"]` when open, `ExMesh.RotateByShape(mesh, Block)`, and return `true`. This is why `ClayPlug` had to be a top-level element in both shapes — verified: both files have exactly three top-level elements, `Base` / `TapCanal` / `ClayPlug`, so the same two keep-lists drive both types with one code path.
...
**Files**
- Modify: `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockFurnaceTap.cs:124-147`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityFurnaceTap.cs:142-172,180-218`, `mods/iiex/assets/iiex/lang/en.json`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`
- Test: `mods/iiex/tests/Blocks/Furnaces/BlastFurnaceTapTests.cs:117-155`, `mods/iiex/tests/Blocks/Furnaces/FurnaceTapPlugTests.cs`

**Consumes:** BlockMoltenCanalStart; ExOrientation.FacingFromSide(string? side) (mods/exlib/src/Helpers/ExOrientation.cs:202-211); lang key `iwex:tap-err-nocanal`
...
**Files**
- Create: `mods/iiex/tests/Blocks/Furnaces/FurnaceBlowInTests.cs`
- Modify: `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockFurnaceTap.cs:95-187`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityFurnaceTap.cs`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntityFurnaceCore.cs:1020-1036`, `mods/iiex/assets/iiex/lang/en.json`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`
- Test: `mods/iiex/tests/Blocks/Furnaces/FurnaceBlowInTests.cs`, `mods/iiex/tests/Scenarios/ColdBlastFurnaceScenarioTests.cs`
...
- Caution: **`smex:solidifiediron` is a released code and the only test that can see the whole migration chain lives in the hpex suite.** `mods/siex/tests/Migrations/ReleasedCodeCoverageTests.cs` is the sole assertion that the smex row still reaches a live block — running only the iwex suite after the rename proves nothing about it (`BlockMigrationModSystem` drops an unresolvable pair with a `Logger.Warning`, so the failure in game is a block that just stops loading).
...
#### U5.1 — Export the burdenmaker shape to mods/iiex/assets/iiex/shapes/ore/burdenmaker.json
...
**Entry condition.** U1 landed through at least U1.3 (verified: `mods/iiex/src/Items/CastStockItemDefinitions.cs` exists with BilletUnits 600 / BloomUnits 1000 / SlabUnits 3000; `iwex:castplate-heavy` exists at `mods/iiex/src/Items/CastPartItemDefinitions.cs:33`). The pig re-mass landed as U2.0 (2026-08-05): `ItemPig.PigUnits = 375`, matching every yield number below. U2 and U3 are done, so the old constraint against running U6 concurrently with them is satisfied; U3 retired the shaft branch's timer machinery while the firebox branch keeps its explicit FSM, so U6.3's disruption-floor fix lands as a firebox-branch override.

**Shared files** (collision risk): `mods/iiex/src/BlockStructures/Furnaces/BlockEntityFurnaceCore.cs — U2 (the column cutover), U3 (raceway + FSM deletion), U4 (hearth/taps/hearthmetal) and U6.3/U6.4/U6.5/U6.7 all edit it. U3.3 explicitly deletes the disruption floor and both extinguish thresholds U6.3 is overriding. Do not run U6 concurrently with U2 or U3.`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityFireboxFurnace.cs — shared with the reheat furnace (U7.1). U6.3 and U6.4 add sealed/virtual members here that U7's heat-into-stock work will read.`, `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockHeatingHearth.cs and BlockHeatingFurnaceCore.cs — U6.1 fixes the heating furnace's six orphan fillers; U7.1 then builds the reheat process on that same hearth.`, `mods/iiex/src/IiexConfig.cs — every unit adds keys. U6 adds the two loss terms (U6.4), three draught coefficients (U6.5) and the puddling process temperature (U6.6); U3.3 deletes a block of Bf* keys.`, `mods/iiex/tests/Blocks/Furnaces/HeatBalanceTests.cs — U6.4 and U6.6 add calibration rows; U3 rewrites the model these rows describe.`, `mods/iiex/src/BlockStructures/Forming/StockItemDefinitions.cs and StockForm.cs — U6.11 changes only the two mass numbers; U7.2/U7.3/U7.4 rewrite WorkPiece, the roll sets and the form table (including the bloom→bar rename U6 deliberately does not do) and the stage shape key to hundredths.`, `mods/iiex/assets/iiex/lang/{en,ru,uk}.json — U6.2, U6.5, U6.9, U6.10 and U6.11 all add keys; every other unit does too. Merge conflicts here are near-certain; add keys, never reorder.`, `mods/iiex/tests/goldens/iiex/blocktypes/furnace/*.json and .../recipes/grid/*.json — U6.1, U6.5, U6.9 and U6.11 re-bless a handful; U2/U4/U5 re-bless overlapping ones. Always use path-fragment filters so two units' blessings do not overwrite each other.`, `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockPuddlingFurnaceCore.cs — U6.1 (fillers) and U6.5 (the cap/flue position) both edit the layout; keep them in one edit or the golden is re-blessed twice.`, `mods/exlib/testing/StructureRig.cs — read-only for U6, but its Raise() behaviour is the reason U6.1's test must be def-level; U2.2's fixture rewrite touches this file's consumers.`
...
**Files**
- Create: `mods/iiex/tests/Blocks/Furnaces/FurnaceFillerAccountingTests.cs`
- Modify: `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockPuddlingHearth.cs:49-55`, `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockHeatingHearth.cs:46-57`, `mods/iiex/tests/goldens/iiex/blocktypes/furnace/puddlinghearth.json`, `mods/iiex/tests/goldens/iiex/blocktypes/furnace/heatinghearth.json`
- Test: `mods/iiex/tests/Blocks/Furnaces/FurnaceFillerAccountingTests.cs`
...
**Files**
- Modify: `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityPuddlingHearth.cs:170-183`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityChargeDoor.cs:128-142`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityPuddlingChimneyCap.cs:60-67`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityHeatingFurnace.cs:69`, `mods/iiex/assets/iiex/lang/en.json`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`
- Test: `mods/iiex/tests/Definitions/IiexLangCoverageTests.cs`

**Consumes:** ExLangKeyGenerator (mods/exlib/generators/ExLangKeyGenerator.cs) — emits `IiexLang.<PascalKey>` consts from mods/iiex/assets/iiex/lang/en.json; a missing key becomes a compile error

**Produces:** Every puddling-side `Lang.Get("iwex:...")` replaced by an `IiexLang.*` constant, so the key set is compiler-enforced

- [x] **Step 1.** Verify the bug first, because it ships today: `BlockEntityPuddlingHearth.cs:174` calls `Lang.Get("iwex:furnace-puddlinghearth-charge", …)` and `:181` calls `"iwex:furnace-puddlinghearth-needsfettle"`, but `mods/iiex/assets/iiex/lang/en.json` defines `puddlinghearth-charge` / `puddlinghearth-needsfettle` with no `furnace-` prefix. Both render the raw key in game. `BlockEntityHeatingFurnace.cs:69` has the same shape: `iwex:heatingfurnace-ready` is not in en.json at all.
- [x] **Step 2.** Understand why nothing caught it: `mods/exlib/testing/LangCoverage.cs` only checks that every registered block code resolves to a `block-*` name key. It never looks at `Lang.Get` call sites. This is a permanent hole and the migration below is the only structural fix.
...
- [x] **Step 4.** Where a constant does not exist because the key is genuinely missing (`heatingfurnace-ready`), add the key to `mods/iiex/assets/iiex/lang/en.json` and its ru/uk translations, following the RU/UK conventions (single `-`, no em-dash), then use the constant.
...
**Files**
- Modify: `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityFireboxFurnace.cs:193-243`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntityFurnaceCore.cs:224`
- Test: `mods/iiex/tests/Blocks/Furnaces/FireboxChargeTests.cs`, `mods/iiex/tests/Invariants/FurnaceBranchGuards.cs`
...
- [x] **Step 6.** Add the invariant to `mods/iiex/tests/Invariants/FurnaceBranchGuards.cs` as a Law beside `NoFireboxAsksForMoreThanItsCellsCanHold` (:157) — that file already scans every `BlockEntityFireboxFurnace` leaf, so a third hearth added later inherits the guard.
...
**Files**
- Modify: `mods/iiex/src/BlockStructures/Furnaces/BlockEntityFurnaceCore.cs:1211-1270`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityFireboxFurnace.cs:193-243`, `mods/iiex/src/IiexConfig.cs:220-230`
- Test: `mods/iiex/tests/Blocks/Furnaces/HeatBalanceTests.cs`

**Consumes:** BlockEntityFurnaceCore.ComputeHeatBalance(BurdenMix charge, float blastSupplyFrac, float blastTemp, int mixCount) (:1211); IiexValues.BfChargeLossFull = 310 (IiexConfig.cs:225); IiexValues.BfRadiationLossBase = 120 (:221); HeatBalance.Compute(...) in mods/exlib/src/Heat/HeatBalance.cs
...
**Files**
- Create: `mods/iiex/src/BlockStructures/Furnaces/StackDraught.cs`, `mods/iiex/tests/Blocks/Furnaces/StackDraughtTests.cs`
- Modify: `mods/iiex/src/BlockStructures/Furnaces/BlockEntityFurnaceCore.cs:1226-1231`, `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockPuddlingFurnaceCore.cs:110-181`, `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockPuddlingChimneyCap.cs:49-60`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityPuddlingChimneyCap.cs:19-34`, `mods/iiex/src/IiexConfig.cs:206-208`, `mods/iiex/tests/goldens/iiex/blocktypes/furnace/puddlingcore.json`, `mods/iiex/tests/goldens/iiex/blocktypes/furnace/puddlingchimneycap.json`
- Test: `mods/iiex/tests/Blocks/Furnaces/StackDraughtTests.cs`, `mods/iiex/tests/Blocks/Furnaces/FurnaceRoleCellsTests.cs:733-759`

**Consumes:** CellRole.Flue (mods/exlib/src/Blocks/Structures/CellRole.cs:124); BlockEntityMultiblockStructure.CellsWithRole(CellRole) (used at BlockEntityFurnaceCore.cs:373,390,429); BlockEntityPuddlingChimneyCap.IsOpen (:22); BlockEntityChargeDoor.IsVenting (:44); IiexValues.BfNaturalDraughtFactor = 0.5f (IiexConfig.cs:207), read at BlockEntityFurnaceCore.cs:1229
...
- [x] **Step 3.** Create `mods/iiex/src/BlockStructures/Furnaces/StackDraught.cs` as a pure static class beside `HearthRows.cs` / `PuddlingHearthLayout.cs` — those two are the precedent for 'pure, testable without a world'. Back `base`/`gain`/`friction` with three new `IiexConfig` keys next to `BfNaturalDraughtFactor` (`IiexConfig.cs:206-207`).
...
**Files**
- Modify: `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityPuddlingFurnace.cs:55-65`, `mods/iiex/src/IiexConfig.cs:268-290`
- Test: `mods/iiex/tests/Blocks/Furnaces/HeatBalanceTests.cs`, `mods/iiex/tests/Invariants/FurnaceBranchGuards.cs`
...
**Files**
- Modify: `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityPuddlingFurnace.cs:33-53`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntityFurnaceCore.cs:1335-1342`
- Test: `mods/iiex/tests/Blocks/Furnaces/FurnaceRoleCellsTests.cs`
...
- [x] **Step 1.** Write the failing test: stand a puddling furnace up through `StructureRig` (copy the pattern from `mods/iiex/tests/Fixtures/ColdBlastFurnaceScenes.cs:251` — `StructureRig.Around(world, anchor, def, angle)` then `.Complete()`), place a real `BlockPuddlingHearth` + `BlockEntityPuddlingHearth` at the hearth cell via `rig.Occupy(rig.Cell(-2,0,0), …)` before `Raise()`, and assert the core's `Hearth` is that block entity. Repeat at all four facings — `rig.Cell` does the rotation for you.
...
**Files**
- Create: `mods/iiex/tests/Fixtures/PuddlingFurnaceScenes.cs`, `mods/iiex/tests/Blocks/Furnaces/PuddlingHearthTests.cs`
- Modify: `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityPuddlingHearth.cs:42-100`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityPuddlingFurnace.cs:67-76`, `mods/iiex/src/BlockStructures/Furnaces/PuddlingHearthLayout.cs:50-65`
- Test: `mods/iiex/tests/Blocks/Furnaces/PuddlingHearthTests.cs`, `mods/iiex/tests/Blocks/Furnaces/FurnacePartsTests.cs:55-120`
...
**Files**
- Create: `mods/iiex/assets/iiex/shapes/item/tool-rabble.json`, `mods/iiex/assets/iiex/shapes/item/tool-paddle.json`, `mods/iiex/src/Items/PuddlingToolItemDefinitions.cs`
- Modify: `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockChargeDoor.cs:46-57`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityChargeDoor.cs:31-44`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityPuddlingHearth.cs`, `mods/iiex/assets/iiex/lang/en.json`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`
- Test: `mods/iiex/tests/Blocks/Furnaces/PuddlingHearthTests.cs`, `mods/iiex/tests/Definitions/IiexDefinitionGoldenTests.cs`
...
**Files**
- Create: `mods/iiex/src/Items/WroughtBallItemDefinitions.cs`, `mods/iiex/assets/iiex/shapes/item/puddled-ironball.json`
- Modify: `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityPuddlingHearth.cs:77-100`, `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockPuddlingHearth.cs:83-127`, `mods/iiex/src/Items/FettleItemDefinitions.cs:71-79`, `mods/iiex/assets/iiex/lang/en.json`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`
- Test: `mods/iiex/tests/Blocks/Furnaces/PuddlingHearthTests.cs`, `mods/iiex/tests/Items/PuddlingYieldTests.cs`
...
- [x] **Step 3.** The ball art is drawn and untracked: `workbench/shapes/item-puddled-ironball.json` (an `IronBall1` group with seven children). Export it to `mods/iiex/assets/iiex/shapes/item/`. Define the item in a new `WroughtBallItemDefinitions.cs` with `materialUnits = 200`, hot-workable, carrying vanilla's `temperature` attribute so it cools in the hand — copy the `combustibleProps`/`temperatureDamage` shape from `BlockStructures/Forming/StockItemDefinitions.cs:46-55`, since the ball goes straight to the hammer and the cooling is the carry cost.
...
**Files**
- Create: `mods/iiex/src/Recipes/Smithing/ShinglingRecipeDefinitions.cs`
- Modify: `mods/iiex/src/BlockStructures/Forming/StockItemDefinitions.cs:21-27`, `mods/iiex/src/Recipes/Grid/FurnaceRecipeDefinitions.cs:16-20`, `mods/iiex/assets/iiex/lang/en.json`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`
- Test: `mods/iiex/tests/Definitions/IiexRecipeOutputTests.cs`, `mods/iiex/tests/Items/PuddlingYieldTests.cs`

**Consumes:** PigRecipeDefinitions (mods/iiex/src/Recipes/Smithing/PigRecipeDefinitions.cs) — the only existing iwex smithing recipe and the exact template for a helve recipe (ExRecipeDef.Create(domain, "smithing", name).Body(new { ingredient, name, pattern, code, output })); StockItemDefinitions.Units (private static Dictionary<string,int>, :24-27 — bloom 180, slab 400); the ball item from U6.10; iwex:castplate-heavy (Items/CastPartItemDefinitions.cs:33)
...
- [x] **Step 4b.** Caution: **`iwex:furnace-firebox` has no recipe, and it is a required cell in both reverberatory layouts** *(blocking finding, 2026-08-05)*. Step 5's "four puddling blocks" is a count inherited from a doc written **before the firebox became a block** — the `Firebox` role moved off `@(air|coalpile)` onto `iwex:furnace-firebox`, and the shipped goldens confirm it: `puddlingcore.json` requires one at (-5,1,0) and `heatingcore.json` requires two. Verified: nothing under `mods/iiex/src/Recipes/` outputs it. **Without this step U6's own Gate — "a player builds a puddling furnace from craftable blocks" — cannot pass**, and the reheat furnace is unbuildable too. Composition is settled in `machines/firebox.md`: cast or wrought iron rods + refractory brick + a diagram; quantities are not. U9.4 Step 3 does exactly this diligence for the coke-oven lid — copy its shape.
...
**Shared files** (collision risk): `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityHeatingFurnace.cs — U7.1 fills SmeltCycle; U2/U3 rewrite the cadence that calls it`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntityFurnaceCore.cs — U7.1 needs a hearth handle at OnStructureCompleted and possibly a public InternalTempC; U2, U3, U5 and U6 all edit this file (the plan already warns U6 must not overlap U2/U3 for the same reason)`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityHeatingHearth.cs — U7.1 adds the soak; U6 shares HeatingHearthLayout.Rows through BlockEntityPuddlingHearth`, `mods/iiex/src/IiexConfig.cs — U7.1 adds ReheatRateK and edits the Rolling region (:576-636); U2/U3 delete or re-derive the Bf* keys (:237,:280,:283,:286); U5/U6 touch the burden and draught regions`, `mods/iiex/src/BlockStructures/Casting/PatternItemDefinitions.cs — U7.10 adds the `rollers` pattern; U1.3 just added the three longcell patterns and U9's crucible work touches it again`, `mods/iiex/src/Recipes/Grid/FormingRecipeDefinitions.cs — U7.10 adds roll-set recipes; U8 adds shear / nail machine / rivet machine recipes to the same forming provider`, `mods/iiex/assets/iiex/lang/{en,ru,uk}.json — U7.3 deletes two set rows, U7.4 adds four item rows, U7.9 adds one ingameerror row; every other unit adds rows to the same three files, and no test catches a lost row for item codes`, `mods/iiex/tests/goldens/iiex/itemtypes/ — U7.3/U7.5 re-bless rollset.json, U7.6 re-blesses caststock.json, U7.4 adds four files; a domain-wide EXLIB_WRITE_GOLDENS=1 from any unit would absorb the others' pending changes`, `infra/tools/convert-shape.py — U7.4 extends it with element extraction; every unit that exports drawn art (U6's paddle/rabble, U8's three machines) calls it`, `mods/iiex/src/BlockStructures/Forming/StockForm.cs — U7.2 and U7.3 both rewrite it, and smex's cast forms and lpex's wide forms are settled to extend the same table`
...
**Files**
- Modify: `mods/iiex/src/BlockStructures/Forming/WorkPiece.cs:162-180`, `mods/iiex/src/Items/CastStockItemDefinitions.cs:79-88`, `mods/iiex/tests/goldens/iiex/itemtypes/caststock.json`
- Test: `mods/iiex/tests/Blocks/Forming/WorkPieceTests.cs`
...
**Files**
- Modify: `mods/iiex/src/BlockStructures/Forming/BlockEntities/BlockEntityRollingMill.cs:244-261`, `mods/iiex/src/BlockStructures/Forming/Blocks/BlockRollingMill.cs:368-375`
- Test: `mods/iiex/tests/Blocks/Forming/RollingMillFeedTests.cs`, `mods/iiex/tests/Blocks/Forming/MillFeedTests.cs`
...
**Files**
- Modify: `mods/iiex/src/BlockStructures/Forming/WorkPiece.cs`, `mods/iiex/src/BlockStructures/Forming/StockMesh.cs`, `mods/iiex/src/BlockStructures/Forming/Items/ItemStockPiece.cs:26-68`, `mods/iiex/src/BlockStructures/Forming/MillFeed.cs:95-128`, `mods/iiex/src/BlockStructures/Forming/BlockEntities/BlockEntityRollingMill.cs:167-211`, `mods/iiex/src/BlockStructures/Forming/BlockEntities/BlockEntityRollingMill.cs:282-288`, `mods/iiex/src/BlockStructures/Forming/StockItemDefinitions.cs:41`, `mods/iiex/src/BlockStructures/Forming/Blocks/BlockRollingMill.cs:336-339`
- Test: `mods/iiex/tests/Blocks/Forming/WorkPieceTests.cs`, `mods/iiex/tests/Blocks/Forming/StockMeshTests.cs`, `mods/iiex/tests/Blocks/Forming/RollingMillFeedTests.cs`, `mods/iiex/tests/Blocks/Forming/MillFeedTests.cs`
...
**Files**
- Create: `mods/iiex/src/BlockStructures/Forming/SectionClass.cs`, `mods/iiex/tests/Blocks/Forming/SectionLawTests.cs`, `scripts/export-stock-stages.py`
- Modify: `mods/iiex/src/BlockStructures/Forming/RollSetItemDefinitions.cs:56-108`, `mods/iiex/src/BlockStructures/Forming/RollSetSpec.cs:31-38`, `mods/iiex/src/BlockStructures/Forming/RollSetSpec.cs:108-201`, `mods/iiex/src/BlockStructures/Forming/StockForm.cs:28-64`, `mods/iiex/src/BlockStructures/Forming/RollingPass.cs:146-176`, `mods/iiex/assets/iiex/lang/en.json:118-121`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`, `mods/iiex/tests/goldens/iiex/itemtypes/rollset.json`, `infra/tools/generate-rolled-stock.py`
- Test: `mods/iiex/tests/Blocks/Forming/SectionLawTests.cs`, `mods/iiex/tests/Blocks/Forming/RollSetSpecTests.cs`, `mods/iiex/tests/Blocks/Forming/RolledStockStagesTests.cs`, `mods/iiex/tests/Blocks/Forming/MillFeedTests.cs`
...
- [ ] **Step 8.** Replace stage generation with stage export. infra/tools/generate-rolled-stock.py is stale twice over: its forms table names item-shingledbloom / item-shingledslab, both of which git reports as ' D' (renamed to item-shingled-bar.json / item-shingled-slab.json), and it derives stages from the exponent model. Since the stages are now drawn (ruling 1: no redraws), write scripts/export-stock-stages.py to lift each named element out of the editable family file, remap the texture the way infra/tools/convert-shape.py's TEXTURES table does, and emit mods/iiex/assets/iiex/shapes/forming/stock-{form}-{key}.json for keys 300/275/250/225/200/175/150/125/100. Delete the ten existing generated files — stock-bloom-30 measures 3×3×16 and stock-bloom-10 is 7.6 wide against the settled 9.0, so every one is wrong.
...
**Files**
- Create: `mods/iiex/tests/Invariants/RecoverabilityTests.cs`
- Modify: `mods/iiex/src/BlockStructures/Forming/MillFeed.cs:5-37`, `mods/iiex/src/BlockStructures/Forming/MillFeed.cs:95-128`, `mods/iiex/src/BlockStructures/Forming/Blocks/BlockRollingMill.cs:350-359`, `mods/iiex/assets/iiex/lang/en.json`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`
- Test: `mods/iiex/tests/Invariants/RecoverabilityTests.cs`, `mods/iiex/tests/Blocks/Forming/MillFeedTests.cs`
...
**Files**
- Create: `mods/iiex/src/BlockStructures/Forming/StockProducts.cs`, `mods/iiex/tests/Blocks/Forming/StockProductsTests.cs`
- Modify: `mods/iiex/src/BlockStructures/Forming/RollSetSpec.cs:28`, `mods/iiex/src/BlockStructures/Forming/RollSetSpec.cs:35`, `mods/iiex/src/BlockStructures/Forming/RollSetSpec.cs:94-101`, `mods/iiex/src/BlockStructures/Forming/RollSetSpec.cs:159-183`, `mods/iiex/src/BlockStructures/Forming/RollSetItemDefinitions.cs:25-46`, `mods/iiex/src/BlockStructures/Forming/RollSetItemDefinitions.cs:56-104`, `mods/iiex/tests/goldens/iiex/itemtypes/rollset.json`
- Test: `mods/iiex/tests/Blocks/Forming/StockProductsTests.cs`, `mods/iiex/tests/Blocks/Forming/RollSetSpecTests.cs`
...
**Files**
- Create: `mods/iiex/src/BlockStructures/Forming/RolledProductItemDefinitions.cs`, `mods/iiex/assets/iiex/shapes/forming/rod.json`, `mods/iiex/assets/iiex/shapes/forming/nailplate.json`, `mods/iiex/assets/iiex/shapes/forming/beam.json`, `mods/iiex/tests/goldens/iiex/itemtypes/rod.json`, `mods/iiex/tests/goldens/iiex/itemtypes/nailplate.json`, `mods/iiex/tests/goldens/iiex/itemtypes/beam.json`, `mods/iiex/tests/Items/RolledProductMassTests.cs`
- Modify: `mods/iiex/assets/iiex/lang/en.json`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`, `infra/tools/convert-shape.py`
- Test: `mods/iiex/tests/Items/RolledProductMassTests.cs`, `mods/iiex/tests/Definitions/IiexDefinitionGoldenTests.cs`

**Consumes:** IExItemDefProvider plus ExItemDef.Create/Shape/MaxStackSize/MaterialDensity/Attribute/CreativeCommon — ExpandedLib/Definitions/ExItemDef.cs:107,:207; CastStockItemDefinitions as the worked shapeByType/attributesByType example — mods/iiex/src/Items/CastStockItemDefinitions.cs:74-104; StockProducts (U7.5)
...
- [x] **Step 10.** All three versions. ⛔ Two mutations survived the first pass: the chimney-gap case knocked out a **brick** and never the flue centre, and the walk case called `WalkChimney()` directly and never `StackCourses`. Both closed. ⛔ `CollectibleMappingGuardTests` fired on the hearth - a block entity that writes an `ItemStack` owes a mapping pair - which is why pot construction moved out into `CruciblePot`. Original text: ./scripts/exmod.sh test 1.21.
...
**Files**
- Create: `mods/iiex/tests/Blocks/Furnaces/ReheatSoakTests.cs`
- Modify: `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityHeatingFurnace.cs:42-56`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityHeatingHearth.cs`, `mods/iiex/src/IiexConfig.cs:576-636`, `mods/iiex/src/BlockStructures/Forming/RollingPass.cs:178-196`, `mods/iiex/src/BlockStructures/Forming/BlockEntities/BlockEntityRollingMill.cs:333-346`
- Test: `mods/iiex/tests/Blocks/Furnaces/ReheatSoakTests.cs`, `mods/iiex/tests/Blocks/Forming/RollingPassTests.cs`
...
**Files**
- Modify: `mods/iiex/src/BlockStructures/Casting/PatternItemDefinitions.cs:79-175`, `mods/iiex/src/Recipes/Grid/FormingRecipeDefinitions.cs:26-52`, `mods/iiex/src/IiexRecipeConfig.cs`, `mods/iiex/tests/goldens/iiex/itemtypes/pattern.json`, `mods/iiex/assets/iiex/lang/en.json`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`
- Test: `mods/iiex/tests/Definitions/IiexRecipeOutputTests.cs`, `mods/iiex/tests/Definitions/IiexDefinitionGoldenTests.cs`

**Consumes:** private static readonly Dictionary<string, object> Molds — PatternItemDefinitions.cs:86; public static readonly string[] PatternTypes — :204; ExRecipeDef.Create(domain,"grid",name).Grid(...) — the worked example at mods/iiex/src/Recipes/Grid/FormingRecipeDefinitions.cs:39-52; the drawn blank at workbench/shapes/item-sandcast-rollers-blank.json
...
**Entry condition.** U7 must be complete and is not today — verified against src/. Hard prerequisites U8 cannot be started without: (1) U7.2 — `WorkPiece` (mods/iiex/src/BlockStructures/Forming/WorkPiece.cs:34) is still the per-strip `Strips[]`/`Turned[]` record with no `Thickness`, no `Mass` and no `Length`, so a crop cannot be expressed at all; (2) U7.4 — no rolled product item exists (mods/iiex/src/Items/ has no rolledrod/nailplate/beam; the codes live only as string literals at RollSetItemDefinitions.cs:66,77,90,100, four of five naming items that do not resolve); (3) U7.5 — `RollSetSpec.OutputAt` (RollSetSpec.cs:95) still has zero callers in src/, so rolling produces no product to crop; (4) ruling 4 — `StockItemDefinitions.cs:44` still emits the tenths shape key `(int)(form.BaseThickness*10)` and `infra/tools/generate-rolled-stock.py` still writes `stock-bloom-30`; the hundredths key must land in U7 before U8.3 writes a stage-keyed crop table against it. Soft prerequisite: `RollSetItemDefinitions.Sets["grooved"]` opens at a 1.0 gap against 3.0 stock (RollSetItemDefinitions.cs:89), blocker B4, so the 25 u rod the rivet machine eats has no legal entry until U7.3 rewrites it to 1.5 → 1.0.

**Shared files** (collision risk): `mods/iiex/src/IiexConfig.cs — U2, U3 and U7 all add config regions; U8 adds a Forming benches region after the Rolling mill region at :576-633`, `mods/iiex/src/IiexRecipeConfig.cs:45-78 — every unit shipping a craftable block adds catalogue rows to the same dictionary`, `mods/iiex/src/Generated/IiexBlocks.g.cs — regenerated wholesale by EXLIB_WRITE_BLOCKCODES=1; two units adding blocks in parallel will both rewrite this file`, `mods/iiex/tests/goldens/iiex/** — one file per def; the completeness test (Goldens_exactly_cover_the_defs) fails if any unit adds a def without its golden`, `mods/iiex/assets/iiex/lang/en.json, ru.json, uk.json — every unit adding a block or an error code appends here; IiexLangCoverageTests fails on any missing locale`, `mods/iiex/src/BlockStructures/Forming/RollSetSpec.cs — U7.3 rewrites the gap tables while U8.3 deletes Outputs/OutputAt and its parse branch at :159-176`, `mods/iiex/src/BlockStructures/Forming/RollSetItemDefinitions.cs — U7.3 redesigns the sets; U8.3 strips every `outputs` array and the Out() helper at :46`, `mods/iiex/src/BlockStructures/Forming/WorkPiece.cs — U7.2 rewrites it to the two-round model; U8.3 and U8.4 read Thickness/Mass/Length off the result`, `mods/iiex/src/BlockStructures/Forming/StockItemDefinitions.cs:44 — U7.4 changes the shape key from tenths to hundredths; U8.3's crop table keys on the same integer`, `mods/iiex/src/BlockStructures/Forming/Blocks/BlockRollingMillAxle.cs:75-121 — U8.4 generalises its three BlockRollingMill casts so the bus serves four machines; anything else touching the mill's placement touches these lines`, `mods/exlib/src/Definitions/ExIngredients.cs and ConstructionStages.cs — cross-mod files; U8.7's Fastener/RequireFastener helpers change how 36 nail sites and 27 rod sites across all four mods resolve`, `mods/iiex/src/Recipes/Grid/FormingRecipeDefinitions.cs — U8.8 extends it to four recipes and U8.10 adds a fifth`, `mods/iiex/src/Recipes/Grid/PipeRecipeDefinitions.cs:25,34,43,52 — U8.7 retargets all four pipe segments off Nails(1)`, `docs/internal/plans/STATE.md — U8.11 fixes the placement table at :596-599; other units edit the same file's decision log`, `mods/iiex/docs/handbook/*.html plus the NN- prefixed keys in mods/iiex/assets/iiex/lang/en.json — the sync pipeline joins them and drift fails a test, so any unit adding a handbook page collides here`
...
**Files**
- Create: `mods/iiex/assets/iiex/shapes/forming/cutter.json`, `mods/iiex/assets/iiex/shapes/forming/nailcutter.json`, `mods/iiex/assets/iiex/shapes/forming/riveter.json`, `mods/iiex/tests/Blocks/Forming/FormingShapeExportTests.cs`
- Modify: `infra/tools/convert-shape.py:36-88 (only if --check reports an unmapped texture key)`
- Test: `mods/iiex/tests/Blocks/Forming/FormingShapeExportTests.cs`

**Consumes:** infra/tools/convert-shape.py — `python infra/tools/convert-shape.py <editable-name> <runtime-path> [...]` (:100-125), `--check` mode (:139-158); TEXTURES map (:36-88) already maps cast-iron1 -> iwex:block/metal/castiron, iron3 -> game:block/metal/riveted/iron3, iron5 -> game:block/metal/sheet-plain/iron5; clip rewrite rules HOLD_CLIPS (:88) and ONESHOT_CLIPS (:96); DefinitionAssets.MissingShapes(domain, assembly) enforced by IiexDefinitionGoldenTests.Every_shape_reference_resolves_to_a_shipped_file (:52-60)
...
- [ ] **Step 1.** Run `python infra/tools/convert-shape.py --check` from the repo root and confirm it prints 'Every texture key in workbench/shapes is mapped' — the three machine files use only cast-iron1, iron3 and iron5, all already mapped
- [ ] **Step 2.** Run `python infra/tools/convert-shape.py machine-mp-megablock-cutter mods/iiex/assets/iiex/shapes/forming/cutter.json machine-mp-megablock-nailcutter mods/iiex/assets/iiex/shapes/forming/nailcutter.json machine-mp-megablock-riveter mods/iiex/assets/iiex/shapes/forming/riveter.json`
...
**Files**
- Create: `mods/iiex/src/BlockStructures/Forming/BlockEntities/BlockEntityMpBench.cs`, `mods/iiex/tests/Blocks/Forming/MpBenchTests.cs`
- Test: `mods/iiex/tests/Blocks/Forming/MpBenchTests.cs`

**Consumes:** IMpEnergyConsumer.LoadTorque(float speed) — mods/exlib/src/Networks/MpEnergyNodes.cs:35-41; BlockEntityNetworkNode (mods/exlib/src/Blocks/Networks/BlockEntityNetworkNode.cs) with `public override string NetworkType { get; set; }`; live speed read `(NetworkSystem?.GetNetworkAt(Pos) as MpEnergyNetwork)?.State?.Speed ?? 0f` — BlockEntityRollingMill.cs:79-81; RollingPass.CanCarry(float loadTorque, float availableTorque, float speed) — RollingPass.cs:126-127 (currently zero callers in src/); persistence idiom `tree.SetItemstack` + `stack.ResolveBlockOrItem(worldForResolving)` — BlockEntityRollingMill.cs:409-443
...
**Files**
- Create: `mods/iiex/src/BlockStructures/Forming/ShearFeed.cs`, `mods/iiex/tests/Blocks/Forming/ShearFeedTests.cs`
- Modify: — *(none; U7.5 owns every `RollSetSpec` / `RollSetItemDefinitions` deletion)*
- Test: `mods/iiex/tests/Blocks/Forming/ShearFeedTests.cs`, `mods/iiex/tests/Blocks/Forming/StockProductsTests.cs` *(add the two tests carried over)*

**Consumes:** FeedDecision / FeedVerdict and the pure-decision house style — mods/iiex/src/BlockStructures/Forming/MillFeed.cs:6-29 and MillFeed.Decide(...) at :95-128; StockForm.All (StockForm.cs:57-64) and StockForm.WidthAt(float) (:39-40); the U7 WorkPiece with a scalar `Thickness`; **`StockProducts.At(form, section, stage)` and its `int StageKey` from U7.5**
...
**Files**
- Create: `mods/iiex/src/BlockStructures/Forming/Blocks/BlockShear.cs`, `mods/iiex/src/BlockStructures/Forming/BlockEntities/BlockEntityShear.cs`, `mods/iiex/tests/Blocks/Forming/ShearTests.cs`, `mods/iiex/tests/goldens/iiex/blocktypes/forming/shear.json`
- Modify: `mods/iiex/src/BlockStructures/Forming/Blocks/BlockRollingMillAxle.cs:75-121 (generalise the three `is BlockRollingMill mill` casts so the axle bus can serve any forming principal)`, `mods/iiex/src/IiexConfig.cs:576-633 (add a Forming benches region after the Rolling mill region)`, `mods/iiex/assets/iiex/lang/en.json:105-124`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`, `mods/iiex/src/Generated/IiexBlocks.g.cs (regenerated, never hand-edited)`
- Test: `mods/iiex/tests/Blocks/Forming/ShearTests.cs`, `mods/iiex/tests/Definitions/IiexDefinitionGoldenTests.cs`, `mods/iiex/tests/Definitions/IiexLangCoverageTests.cs`, `mods/iiex/tests/Definitions/IiexNetworkNodeContractTests.cs`

**Consumes:** BlockRollingMill as the end-to-end template — `BlockNetworkNode, IExBlockDefProvider, IFillerHost, IFillerInteractionTarget` (BlockRollingMill.cs:31-36), `Definitions(string domain)` (:44-64), `StructureFootprint.Layout(f => ...)` (:74-94), `AxleOffsets`/`AxleCells(BlockPos)` (:99, :113-120), the CanPlaceBlock/OnBlockPlaced/OnBlockBroken triad (:122-215), `StructureAngle => Variant?["orientation"] == "ns" ? 90 : 0` (:106), `.SolidNonOpaque()` for an overhanging shape (:62-63); StructureFillers.FootprintCells/CanPlace/PlaceFillers/RemoveFillers (mods/exlib/src/Blocks/Structures/StructureFillers.cs:150,203,221,258); BlockEntityMpBench from U8.2; ShearFeed.Decide and ShearCrops.At from U8.3
...
- [ ] **Step 9.** Regenerate the block code table with `EXLIB_WRITE_BLOCKCODES=1` on the iwex test project and bless the new golden with `EXLIB_WRITE_GOLDENS=mods/iiex/tests/goldens/iiex/blocktypes/forming/shear.json`
...
**Files**
- Create: `mods/iiex/src/BlockStructures/Forming/Blocks/BlockNailMachine.cs`, `mods/iiex/src/BlockStructures/Forming/BlockEntities/BlockEntityNailMachine.cs`, `mods/iiex/tests/Blocks/Forming/NailMachineTests.cs`, `mods/iiex/tests/goldens/iiex/blocktypes/forming/nailmachine.json`
- Modify: `mods/iiex/src/IiexConfig.cs (nail bench keys)`, `mods/iiex/assets/iiex/lang/en.json`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`, `mods/iiex/src/Generated/IiexBlocks.g.cs`
- Test: `mods/iiex/tests/Blocks/Forming/NailMachineTests.cs`

**Consumes:** BlockEntityMpBench (U8.2); BlockShear's footprint/def/axle pattern (U8.4); the `nailplate` item from U7.4; ExIngredients.Nails resolves `game:metalnailsandstrips-*` (mods/exlib/src/Definitions/ExIngredients.cs:36-37) — the output code is the vanilla item
...
**Files**
- Create: `mods/iiex/src/Items/FastenerItemDefinitions.cs`, `mods/iiex/src/BlockStructures/Forming/Blocks/BlockRivetMachine.cs`, `mods/iiex/src/BlockStructures/Forming/BlockEntities/BlockEntityRivetMachine.cs`, `mods/iiex/tests/Items/FastenerMassTests.cs`, `mods/iiex/tests/Blocks/Forming/RivetMachineTests.cs`, `mods/iiex/tests/goldens/iiex/itemtypes/rivet.json`, `mods/iiex/tests/goldens/iiex/blocktypes/forming/rivetmachine.json`
- Modify: `mods/iiex/src/IiexConfig.cs (rivet bench keys)`, `mods/iiex/assets/iiex/lang/en.json`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`, `mods/iiex/src/Generated/IiexBlocks.g.cs`
- Test: `mods/iiex/tests/Items/FastenerMassTests.cs`, `mods/iiex/tests/Blocks/Forming/RivetMachineTests.cs`

**Consumes:** BlockEntityMpBench (U8.2); the 25 u rod product of ShearCrops (U8.3, drawn as item-rolled-rod.json's CutRivetRod1..4 at 1x1x10 vx each); the bundle convention set by `game:metalnailsandstrips` (one stack entry worth 25 u of iron and an unstated number of pieces); ExItemDef + IExItemDefProvider (mods/exlib/src/Definitions/), StockItemDefinitions.cs:36-56 as the nearest item-def template with MaterialDensity/materialUnits/combustibleProps
...
**Files**
- Create: `mods/exlib/tests/Definitions/FastenerSubstitutionTests.cs`, `mods/iiex/tests/Definitions/BoilerFastenerGateTests.cs`
- Modify: `mods/exlib/src/Definitions/ExIngredients.cs:35-41 (add a Fastener(qty) factory beside Nails/NailsSteel)`, `mods/exlib/src/Definitions/ConstructionStages.cs:107-115 and the shared body at :120-132 (add RequireFastener beside RequireMetalNails)`, `mods/iiex/src/Recipes/Grid/PipeRecipeDefinitions.cs:25,34,43,52`, `mods/iiex/src/Recipes/Grid/FurnaceRecipeDefinitions.cs:60`, `mods/iiex/src/Recipes/Grid/MoltenRecipeDefinitions.cs:34`
- Test: `mods/exlib/tests/Definitions/FastenerSubstitutionTests.cs`, `mods/iiex/tests/Definitions/BoilerFastenerGateTests.cs`, `mods/iiex/tests/Definitions/IiexDefinitionGoldenTests.cs`
...
**Files**
- Create: `mods/iiex/tests/goldens/iiex/recipes/grid/shear.json`, `mods/iiex/tests/goldens/iiex/recipes/grid/nailmachine.json`, `mods/iiex/tests/goldens/iiex/recipes/grid/rivetmachine.json`
- Modify: `mods/iiex/src/Recipes/Grid/FormingRecipeDefinitions.cs:26 (extend Definitions to four recipes)`, `mods/iiex/src/IiexRecipeConfig.cs:47-78 (add three catalogue rows)`
- Test: `mods/iiex/tests/Definitions/IiexDefinitionGoldenTests.cs`, `mods/iiex/tests/Definitions/IiexRecipeOutputTests.cs`, `mods/iiex/tests/Definitions/IiexCostSelectorTests.cs`
...
**Files**
- Create: `mods/exlib/src/Blocks/Structures/StockPile.cs`, `mods/exlib/tests/Blocks/StockPileTests.cs`
- Test: `mods/exlib/tests/Blocks/StockPileTests.cs`

**Consumes:** The pure-function house style (StockMesh.SideOf at mods/iiex/src/BlockStructures/Forming/StockMesh.cs:36, StockMesh.CacheKey at :67-73); GameMath.MurmurHash3 for reproducible jitter; ExMesh.RotateByShape(mesh, block) as the single final rotation — BlockEntityHeatingHearth.cs:121
...
**Files**
- Create: `workbench/shapes/storage-megablock-stockrack.json`, `mods/iiex/assets/iiex/shapes/forming/stockrack.json`, `mods/iiex/src/BlockStructures/Forming/Blocks/BlockStockRack.cs`, `mods/iiex/src/BlockStructures/Forming/BlockEntities/BlockEntityStockRack.cs`, `mods/iiex/tests/Blocks/Forming/StockRackTests.cs`, `mods/iiex/tests/goldens/iiex/blocktypes/forming/stockrack.json`, `mods/iiex/tests/goldens/iiex/recipes/grid/stockrack.json`
- Modify: `mods/iiex/src/Recipes/Grid/FormingRecipeDefinitions.cs`, `mods/iiex/src/IiexRecipeConfig.cs:45-78`, `mods/iiex/assets/iiex/lang/en.json`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`, `mods/iiex/src/Generated/IiexBlocks.g.cs`
- Test: `mods/iiex/tests/Blocks/Forming/StockRackTests.cs`

**Consumes:** StockPile.Place / PerLayer / Capacity (U8.9); BlockFilledMegastructure — `abstract class BlockFilledMegastructure : Block, IFillerHost` with the place/break triad folded in and `public abstract int StructureAngle` (mods/exlib/src/Blocks/Structures/BlockFilledMegastructure.cs:31-96) — usable here because the rack is not a network node; StructureFootprint.Rectangle(halfWidth, depth) (StructureFootprint.cs:51-69); BlockEntityHeatingHearth.cs:26-174 end to end as the BE template (slot array :30, TryLoad :57, TryTake :73, OnTesselation :99-126, per-slot SetItemstack + ResolveBlockOrItem :132-154, block-info readout :160-171)
...
> `mods/iiex/docs/handbook/10-formingshop.html`, in three locales.
...
- Test: `mods/exlib/tests/Localization/HandbookParityTests.cs`

**Consumes:** docs/internal/plans/STATE.md:620-649 § 'Fasteners — settled 2026-07-30: two machines, two routes, no dies' as the authoritative text; the handbook sync pipeline joining docs/<mod>/handbook/*.html to lang/en.json on the NN- prefix (mods/exlib/testing/HandbookSync.cs)
...
- A `cycle` clip authored with onAnimationEnd EaseOut makes the running mesh vanish in game. All three editables carry EaseOut; infra/tools/convert-shape.py:117-121 rewrites it, but only for exports that go through the script. A hand-copied shape passes every C# test in the repo and is broken only in game. Nothing today asserts anything about an emitted clip's onAnimationEnd.
...
1. **U6.4 and U6.5 landed — not U6.3**, which produces neither of the two things this condition actually requires. `NaturalDraughtFor(int courses, bool damperOpen, bool venting)` — **three** parameters, not the two this condition used to state — must have replaced the flat read at `mods/iiex/src/BlockStructures/Furnaces/BlockEntityFurnaceCore.cs:1229` (`float natural = IiexValues.BfNaturalDraughtFactor;`, default `0.5f` at `mods/iiex/src/IiexConfig.cs:207`), the counted stack walk must exist and must consume `CellRole.Flue` (`mods/exlib/src/Blocks/Structures/CellRole.cs:124`) and `CellRole.Damper` (`:131`) - **both roles exist and neither has a single consumer in `src/` today** - and per-machine charge loss must be a virtual over `IiexValues.BfChargeLossFull` (read at `BlockEntityFurnaceCore.cs:1241-1244`). Without all three the crucible furnace ships lit-and-never-melting.
...
3. **U1 complete** for U9.11 only (the cast-iron ingot mould is a sand-casting pattern; `PatternItemDefinitions.Molds` at `mods/iiex/src/BlockStructures/Casting/PatternItemDefinitions.cs:86`).

U9.1-U9.4 (the coke oven) only strictly need #1 and #2. U9.5-U9.11 need all three.

**Shared files** (collision risk): `mods/iiex/src/BlockStructures/Furnaces/BlockEntityFurnaceCore.cs - U2, U3, U4 and U6 all rewrite this file; U9 only reads it (ComputeHeatBalance :1211, natural :1229, cell-role accessors :373-429, ComponentScan* :1712-1713). Do not overlap.`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityFireboxFurnace.cs - U6 owns the branch; both U9 machines derive from it and depend on its sealed members (:62, :71, :241-242) and its tunables passthrough (:197-199).`, `mods/iiex/src/IiexConfig.cs - every unit adds keys; U6.3 rewrites BfNaturalDraughtFactor (:207) and BfChargeLossFull (:225) out from under U9.`, `mods/iiex/src/Generated/IiexBlocks.g.cs - generated. Every unit that adds a blocktype regenerates it; two units regenerating in parallel will produce a conflicting file. Regenerate last, never hand-edit.`, `mods/iiex/src/Recipes/Grid/FurnaceRecipeDefinitions.cs:16-17 - U6 adds the puddling/hearth recipes here too; U9 adds two more groups to the same array.`, `mods/iiex/src/IiexRecipeConfig.cs:46-77 - one Defaults() dictionary, every unit appends rows.`, `mods/iiex/src/BlockStructures/Casting/PatternItemDefinitions.cs:72-171 - U1 owns the casting route and has already added castbillet/castbloom/castslab; U9.11 adds one more Molds row for the ingot mould.`, `mods/iiex/assets/iiex/lang/{en,ru,uk}.json - every unit appends keys; three files, all three required by IiexLangCoverageTests.`, `infra/tools/convert-shape.py - U7 (rolled stock) and U8 (the three MP machines) also need TEXTURES entries; U9.5 adds blistersteel and slag1. One dict, three units.`, `scripts/test-floors.txt - one row per suite; every unit that adds tests should ratchet it, and two units editing it in parallel conflict.`, `mods/iiex/tests/Fixtures/FurnaceLayoutRig.cs - the shared oracle for every furnace in the line, including smex's; U6 and U9 both add glyph constants to it.`, `mods/iiex/tests/Blocks/Furnaces/FurnaceRoleCellsTests.cs:733-758 and FurnacePartsTests.cs:33-51 - the two role/code oracles both U6 and U9 must extend and whose floors both must raise.`, `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockChargeDoor.cs - the coke oven consumes its lid and door defs; U6 consumes the puddling door. Read-only for U9.`, `workbench/layouts.md - the two U9 drafts sit at :518-655; U6 edits the puddling/heating drafts in the same file.`, `docs/design/machines/{coke-oven.md,crucible-furnace.md} and docs/design/layered-charge.md - U2/U3 rewrite layered-charge's state-machine section under U9's feet.`
...
**Files**
- Create: `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockCokeOvenCore.cs`, `mods/iiex/assets/iiex/textures/block/furnace/co.png`, `mods/iiex/tests/goldens/iiex/blocktypes/furnace/cokeovencore.json`, `mods/iiex/tests/Blocks/Furnaces/CokeOvenLayoutTests.cs`
- Modify: `mods/iiex/src/Generated/IiexBlocks.g.cs`, `mods/iiex/assets/iiex/lang/en.json`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`, `workbench/layouts.md:590-660`
- Test: `mods/iiex/tests/Blocks/Furnaces/CokeOvenLayoutTests.cs`, `mods/iiex/tests/Blocks/Furnaces/FurnacePartsTests.cs:33-51`, `mods/iiex/tests/Definitions/IiexDefinitionGoldenTests.cs`

**Consumes:** BlockFurnaceCoreBase.Core(string domain, string code, string? type, string path, params string[] brickTiers) - mods/iiex/src/BlockStructures/Furnaces/BlockFurnaceCoreBase.cs:58-64; BlockFurnaceCoreBase.FurnaceCode = "furnace" :43; IiexBlocks.FurnaceChargedoor.WithSide(BlockFacing) / FurnaceChargelid.WithSide(BlockFacing) / HopperTall.WithSide(BlockFacing) - mods/iiex/src/Generated/IiexBlocks.g.cs:252,279,704; VanillaCodes.FireBricks :91, FireSlab(BlockFacing) :170, Air :50; ExCodes.Filler :27
...
- [x] **Step 3.** ⛔ **Built with no `tier` group**, against this step's `"tier1","tier2","tier3"`: the oven is fire brick throughout (vanilla's own coke-oven masonry, and it must stay cheap because it gates everything after it), so there is no tiered brick in the drawing for a tier variant to match and three tiers would be three indistinguishable blocks. Original text: Write BlockCokeOvenCore.cs modelled line-for-line on mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockHeatingFurnaceCore.cs:35-141: Core(domain, FurnaceCode, "cokeovencore", "furnace/cokeovencore", "tier1","tier2","tier3") then .Class/.EntityClass/.Texture(all|north|south)/.MultiblockLayout.
- [x] **Step 4.** Built rather than drawn: the `c` is lifted verbatim from `cf.png` and the `o` is that same `c` unioned with its own mirror, so both letters carry the shipped set's exact stroke weight, palette and anti-aliasing instead of a newly-drawn approximation. Original text: Draw mods/iiex/assets/iiex/textures/block/furnace/co.png as a 2-letter label to match the shipped set (bc, bfc, bfh, cf, cfd, cs, hf, n, pf, ss in mods/iiex/assets/iiex/textures/block/furnace/) and wire it as the south face.
...
- [x] **Step 8.** Bless the golden: EXLIB_WRITE_GOLDENS=mods/iiex/tests/goldens/iiex/blocktypes/furnace/cokeovencore.json ./scripts/exmod.sh test 1.21
- [x] **Step 9.** In CokeOvenLayoutTests, stand the structure up with FurnaceLayoutRig.Stand (mods/iiex/tests/Fixtures/FurnaceLayoutRig.cs:199) at all four sides and assert it completes without forcing StructureComplete; assert the two chambers are 6 cells each and carry CellRole.Firebox via FurnaceLayoutRig.RoleCellsOf (:364).
...
**Files**
- Create: `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityCokeOven.cs`, `mods/iiex/tests/Blocks/Furnaces/CokeOvenCycleTests.cs`
- Modify: `mods/iiex/src/IiexConfig.cs`, `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockCokeOvenCore.cs`
- Test: `mods/iiex/tests/Blocks/Furnaces/CokeOvenCycleTests.cs`
...
**Files**
- Create: `mods/iiex/tests/Blocks/Furnaces/CokeOvenLidGateTests.cs`
- Modify: `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityCokeOven.cs`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityChargeDoor.cs`, `mods/iiex/assets/iiex/lang/en.json`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`
- Test: `mods/iiex/tests/Blocks/Furnaces/CokeOvenLidGateTests.cs`

**Consumes:** BlockEntityChargeDoor.MainOpen :23, SmallOpen :26, HasSmallDoor :29, IsVenting => MainOpen || SmallOpen :44, ToggleMain() :49; BlockEntityFurnacePart.Core / ResolveOwningAnchor() (mods/iiex/src/BlockStructures/Furnaces/BlockEntityFurnacePart.cs:37,50); BlockEntityFurnaceCore.GetBlockInfo :1781 and AppendReadyInfo :1948
...
**Files**
- Create: `mods/iiex/docs/handbook/NN-cokeoven.html`
- Modify: `mods/iiex/src/Recipes/Grid/FurnaceRecipeDefinitions.cs:16-17`, `mods/iiex/src/IiexRecipeConfig.cs:46-77`, `mods/iiex/assets/iiex/lang/en.json`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`, `mods/iiex/tests/goldens/iiex/recipes/grid/cokeoven.json`
- Test: `mods/iiex/tests/Definitions/IiexRecipeOutputTests.cs`, `mods/iiex/tests/Definitions/IiexCostSelectorTests.cs`, `mods/iiex/tests/Definitions/IiexDefinitionGoldenTests.cs`
...
- [x] **Step 5.** Page **11**, in all three locales - the second handbook page this session, after the forming shop at 10. Original text: Write the handbook page under mods/iiex/docs/handbook/ with the NN- prefix convention and mirror its keys into mods/iiex/assets/iiex/lang/en.json (the handbook sync pipeline joins on the prefix).
...
> ★★ **DONE 2026-08-21.** Three shapes exported, the `blistersteel` mapping, and **two defects in the
> tooling itself**, both of the same shape: a guard reporting green because it was not looking.
> ⛔⛔ `--check` walked only the **top level** of `workbench/shapes`, which has held no files since
> the tree was filed into subdirectories - **0 of 155 shapes scanned**, reporting "everything is mapped".
> ⛔⛔ `convert-shape.py` **wrote its output before checking textures**, so a refused conversion replaced a
> correct shipped shape with the editable's absolute `F:/...` paths. It did exactly that here, and no test
> in the repo could see it - now `ShippedAssetJsonTests.No_shipped_asset_carries_an_authoring_path` can.
> ⛔ Only **three** of the five listed shapes needed exporting: `item/ingotmold.json` already shipped
> byte-identical, and `casting/cell-filling-ingotmold.json` already shipped **correctly** and must not be
> re-exported - see Step 3.


**Files**
- Create: `mods/iiex/assets/iiex/shapes/furnace/cruciblehearth.json`, `mods/iiex/assets/iiex/shapes/item/steelcrucible.json`, `mods/iiex/assets/iiex/shapes/item/metalchunk.json`, `mods/iiex/assets/iiex/shapes/item/ingotmold.json`, `mods/iiex/assets/iiex/shapes/casting/cell-filling-ingotmold.json`
- Modify: `infra/tools/convert-shape.py`
- Test: `mods/iiex/tests/Definitions/IiexDefinitionGoldenTests.cs`

**Consumes:** infra/tools/convert-shape.py TEXTURES map; the drawn editables workbench/shapes/{furnace-block-draftcruciblehearth,item-steelcrucible,item-shingled-metalchunk,item-sandcast-ingotmold,molten-sandcellfilling-castingotmold}.json

**Produces:** runtime shapes under mods/iiex/assets/iiex/shapes/{furnace,item,casting}/ that IiexDefinitionGoldenTests.Every_shape_reference_resolves_to_a_shipped_file can find

- [x] **Step 1.** ⛔⛔ **It reported nothing, because it was scanning nothing.** `check_all` used a flat `os.listdir`, and the editables have long since been filed into subdirectories: **0 of 155** shapes examined, printing "Every texture key is mapped". Made recursive; the true set is `basalt` (17 shapes), `iron42` (7), and one each of `blistersteel`, `hematite`, `lore-marked`, `scrolls-rotten`, `steel32`, `steel42`. Only `blistersteel` was added - the rest are either documented as deliberate or now recorded as undecided in the file, since a wrong mapping is silent. ⛔ `slag1` does not exist; the hearth's slag elements carry no key of their own. Original text: Run `python infra/tools/convert-shape.py --check`. It reports blistersteel and slag1 as unmapped, each used by exactly one editable - the crucible hearth. Add both to TEXTURES: blistersteel -> game:block/metal/ingot/blistersteel (the file exists at .game/1.20/assets/survival/textures/block/metal/ingot/blistersteel.png), slag1 -> iwex:block/slag/slag (mods/iiex/assets/iiex/textures/block/slag/slag.png). Follow the file's own rule: cite a shipped sibling in the comment.
- [x] **Step 2.** Confirmed and left alone; the export carries `front1: game:block/clay/refractory/tier3/front1` as the fallback, and U9.9 declares the tiered override. Original text: Caution: The hearth shape hardcodes the refractory face at tier3 via the front1 key. That is correct and must not be 'fixed': convert-shape.py's own comment (the front1 entry) explains the blocktype's textures map overrides by key, so a tiered block wears {tier}. Declare .Texture("front1", "game:block/clay/refractory/{tier}/front1") on the blocktype in U9.9.
- [x] **Step 3.** ⛔ **Three of the five, not five.** `item/ingotmold.json` already shipped byte-identical. `casting/cell-filling-ingotmold.json` already shipped **correctly** and must not be re-exported: its editable was re-drawn against `basalt` sand while every shipped filling - all 16 of its siblings - renders `andesite`, so converting it changes the sand under one mould and nothing else. That is a decision about all 17 fillings, not a step in this unit. Original text: Convert each editable: python infra/tools/convert-shape.py furnace-block-draftcruciblehearth mods/iiex/assets/iiex/shapes/furnace/cruciblehearth.json item-steelcrucible mods/iiex/assets/iiex/shapes/item/steelcrucible.json item-shingled-metalchunk mods/iiex/assets/iiex/shapes/item/metalchunk.json item-sandcast-ingotmold mods/iiex/assets/iiex/shapes/item/ingotmold.json molten-sandcellfilling-castingotmold mods/iiex/assets/iiex/shapes/casting/cell-filling-ingotmold.json
- [x] **Step 4.** Exactly as named, and pinned by `CrucibleHearthShapeTests` rather than eyeballed - including that `Coke/CokeL1..6` matches `BlockFirebox.ElementsFor`, which is the coupling that lets the hearth reuse the firebox bed mesher. Original text: Verify the converted hearth still carries the element tree the code will name: Base/{Masonry,Firebar1-3,ClayStand1-4}, Crucibles/Crucible1-4, FillingBlisterSteel/BlisterSteel1-4, FillingSlag/Slag1-4, Covers/Cover1-4, Coke/CokeL1-6. The Coke/CokeL{n} names are exactly what BlockFirebox.ElementsFor(int layers) emits (mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockFirebox.cs:127-134), so the hearth's bed can reuse the firebox bed mesher unchanged.
- [x] **Step 5.** Exactly those bounds, pinned. Original text: Assert the hearth's extent stays one cell: bbox x -1..16, y -1..14, z 0..16. It is a single block holding all four pots, not a row of holes.
- [x] **Step 6.** ./scripts/exmod.sh test 1.21

#### U9.6 — The refractory pot - a two-variant block on vanilla's crucible chassis

> ★★ **BUILT 2026-08-22**, on research that invalidated the task title first. Five parallel
> reads over vanilla and the repo turned up four corrections that have to be made before a line is written:
>
> 1. ⛔⛔ **It cannot be a TWO-variant block.** A clayforming recipe can only output a `-raw` variant, which
>    a pit kiln then fires (`combustiblePropsByType["*-raw"]`). A blocktype whose variantgroup omits `raw`
>    leaves the recipe with nothing to output and the pot **unobtainable**. Three variants:
>    `raw | burned | smelted`. (The plan audit already filed this as a MISSING_TASK at
>    `2026-08-05-iwex-plan-audit.md:64`.)
> 2. ⛔⛔ **`maxHeatableTemp` has no reader.** Step 3 says to raise it past 1600; a recursive grep of
>    `.compat/vintagestory/` finds **zero C# readers** - the firepit caps on
>    `Math.Max(combustibleProps.MaxTemperature, ItemAttributes["maxTemperature"])`
>    (`BEFirepit.cs:350-353`), a **different key**. Raising `maxHeatableTemp` changes nothing at runtime.
> 3. ⛔⛔ **`DoSmelt` hard-codes the variant swap**: `world.GetBlock(CodeWithVariant("type", "smelted"))`
>    followed by an **unchecked cast** to `BlockSmeltedContainer`. So the group must be literally `type`,
>    the state literally `smelted`, and that variant's `classByType` must be `BlockSmeltedContainer`. Any
>    of the three wrong is a null block or an `InvalidCastException` **server-side, mid-tick**.
> 4. ⛔⛔ **`classByType` has zero precedent in this repo** and there is no typed builder. It must go
>    through `RawByType("classByType", wildcard, name)`; ⛔ `AttributeByType` is the **wrong method** - it
>    writes under `attributes`, where the loader does not look. And ⛔ `Class<T>()` with a Vintagestory
>    type **compiles** but emits `iiex.BlockSmeltingContainer`, a key nobody registered: vanilla registers
>    the bare name, so the string overload is mandatory for vanilla classes.
>
> ⛔ Also: `emptiedBlockCode` must be declared on the smelted variant or the pour **crashes**
> (`AssetLocation.Create(null, ...)` throws); the burned variant must have **no** `combustibleProps` or a
> firepit will bake the metal out of a full pot; and the variant is `burned` in 1.20 but **`fired`** in
> 1.21/1.22, which this multi-version repo has to choose between deliberately.
>
> ★ Step 2(c) is **confirmed**: `MoldKinds.FitsPedestal` is `block is BlockToolMold`
> (`MoldKinds.cs:26`) - a type test, not a material test - so `WouldShatter(pot, 1600f)` is false and
> `&&` short-circuits before the temperature is even compared. ⛔ But that also means **nothing protects
> the pot**, and the pedestal's automated drain bypasses vanilla `CanReceive` entirely, so the Harmony
> patch covers only the hand pour.
>
> ⛔⛔ **A sixth correction the guards found, not the research**: `smeltedStack.code` and
> `emptiedBlockCode` read the same string **two different ways**. A stack code defaults an unqualified
> path to `game:` and resolves to nothing; `emptiedBlockCode` is resolved with
> `AssetLocation.Create(code, Code.Domain)` and so must stay **bare** to land in our own domain. Caught by
> `CrossModReferenceTests`, and now pinned in both directions.
>
> ⛔ Not built here: the pour and firing-life behaviour is exercised as pure arithmetic and as a def
> contract, **not** through a live firepit smelt or a live pour - the harness has no firepit, and both
> vanilla seams are `virtual` overrides on classes it cannot instantiate. `DoSmelt` and
> `OnHeldInteractStop` are overridden and unit-covered only by the contract cases around them.


**Files**
- Create: `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockSteelCrucible.cs`, `mods/iiex/tests/Blocks/Furnaces/SteelCrucibleTests.cs`, `mods/iiex/tests/goldens/iiex/blocktypes/furnace/steelcrucible.json`
- Modify: `mods/iiex/src/BlockNetworkMolten/Blocks/ClayHeatGate.cs`, `mods/iiex/src/Generated/IiexBlocks.g.cs`, `mods/iiex/assets/iiex/lang/en.json`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`
- Test: `mods/iiex/tests/Blocks/Furnaces/SteelCrucibleTests.cs`, `mods/iiex/tests/Casting/ClayHeatGateTests.cs`
...
- [x] **Step 1.** Confirmed and built as a block. ★ The design page's Code table still sends a reader to `.../Items/` modelled on `StockItemDefinitions`; corrected there too. Original text: Caution: It is a block, not an item. docs/design/machines/crucible-furnace.md's Code table sends you to .../Items/ modelled on StockItemDefinitions - that is wrong. Vanilla's crucible is a block precisely so it can be a BlockSmeltedContainer, which is what the mod's existing pour path accepts (BlockMoltenCanalStart.cs:77). An item cannot pour.
- [x] **Step 2.** (c) is confirmed and pinned at two temperatures, so the case cannot pass by the ceiling happening to be high. ⛔ (a) and (b) are **not** covered as live behaviour: the harness has no firepit and cannot drive vanilla's held-item interaction, so the pour and the three-heat death are covered as the def contract plus pure arithmetic. Original text: Write the failing tests first: (a) a smelted pot pours into a canal start; (b) a pot at 3 recorded firings breaks after the pour and the metal survives; (c) ClayHeatGate.WouldShatter(pot, 1600f) is false - MoldKinds.FitsPedestal is `block is BlockToolMold` (:27), so the pot never satisfies it; pin that so a future 'tidy' of FitsPedestal cannot silently start shattering pots.
- [x] **Step 3.** ⛔ **Three** variants, not two - a clayforming recipe can output only a `-raw` variant. ⛔ `maxHeatableTemp` is declared at 1600 for parity and is **inert**: it has no reader anywhere in the open source, and the firepit caps on `maxTemperature` instead. MaxStackSize 1 as specified. Original text: Define the two variants with classByType exactly as vanilla does, shape iwex:item/steelcrucible (from U9.5), and maxHeatableTemp raised past 1600. MaxStackSize must be 1 - the firing counter lives on the stack and two pots of different ages must never merge.
- [x] **Step 4.** Written as a `<remarks>` paragraph naming the pot and the three-heat life, with no branch - and the reason stated: there is nothing to branch on, and a branch naming the pot would imply the gate had considered it. Original text: Write the fireclay exception into ClayHeatGate.cs as a doc-comment paragraph naming this block and the 3-firing life, per the design's own instruction ('a documented exception is a rule, an undocumented one is a bug'). Do not add a branch - there is nothing to branch on.
- [x] **Step 5.** ⛔ **Neither option taken, deliberately.** The research showed the help gate and the pour gate are **independent**: the pour is decided by vanilla's `as ILiquidMetalSink` cast on the target, so a code named `iiex:steelcrucible-*` **pours** already. Only the interaction help misses it, and both caches match `Code.Path.StartsWith("crucible-")` - domain-blind in both directions, so widening them would also admit any other mod's crucible. Left as a recorded gap rather than papered over. Original text: Caution: Decide the code path deliberately. BlockMoltenCanalStart.cs:66 and BlockMoltenBarrel.cs:84 both cache pour-source stacks by `block.Code.Path.StartsWith("crucible-")`. A pot coded iwex:steelcrucible-* will not appear in the interaction help even if the pour works. Either name it so the prefix matches, or extend both caches - and pin whichever you choose with a test, because help text and pour capability are two independent things behind one string.
- [x] **Step 6.** Through `ExRecipeDef.Body`, fire clay only where vanilla's takes blue/fire/red. ⛔ `code` on a **clayforming** body is inert - `ClayFormingRecipe` has no such property - so it is not copied here. (⛔⛔ **Corrected 2026-08-22 by U9.7**: this was over-generalised. `SmithingRecipe.Code` **does** exist in 1.22.5, documented as a stable identifier for 1.23, so `PigRecipeDefinitions`' `code = "iwexpigbreak"` is a real field. It is still not the marker it looks like - the marker is the stack attribute.) Original text: Add the clayforming recipe from clay-fire (vanilla clayforming accepts only clay-* in blue|fire|red). Author it via ExRecipeDef.Body - there is no clayforming builder (mods/exlib/src/Definitions/ExRecipeDef.cs:11-16 documents the escape hatch; PigRecipeDefinitions.cs:22-34 is the worked example).
- [x] **Step 7.** Plus a cost-catalogue row, which the clayforming output needs like any other craftable block. Original text: Regenerate block codes, bless the golden, add lang keys in all three locales, ./scripts/exmod.sh test 1.21

#### U9.7 — Blister steel cold-crushes to 100 u of charge

> ★★ **DONE 2026-08-22.** The cold branch ships: `BlisterBreaking`, `ItemBlisterWorkItem`,
> `BlisterItemDefinitions` (`iiex:blisterchunk`), `BlisterRecipeDefinitions`, two Harmony patch classes,
> 25 tests, three blessed goldens and lang keys in three locales. 9/9 green at **4,559** per version.
> ⛔⛔ **A smithing pattern does not land where it reads** - `GenVoxels` centres it and, for smithing,
> transposes it - and a chain whose metal misses the shape is silent, because the helve's `FullyWorkable`
> pass **conjures metal** into any empty recipe cell. **Both already-shipped chains were misaligned**; the
> pig and the shingling pile are fixed here and all three are now guarded against vanilla's own layout pass.
> ⛔⛔ **U9.6's note that a recipe `code` is inert was wrong for smithing**: `SmithingRecipe.Code`
> exists (1.22.5, "not used in 1.22 but will be used in 1.23"). It is real, and it is not the marker - the
> marker is the stack attribute.

**Files**
- Create: `mods/iiex/src/Items/BlisterBreaking.cs`, `mods/iiex/src/Recipes/Smithing/BlisterRecipeDefinitions.cs`, `mods/iiex/src/Patches/AnvilBlisterBreakingPatches.cs`, `mods/iiex/tests/Items/BlisterBreakingTests.cs`, `mods/iiex/tests/goldens/iiex/recipes/smithing/blister.json`
- Modify: `mods/iiex/src/Items/CastPartItemDefinitions.cs`
- Test: `mods/iiex/tests/Items/BlisterBreakingTests.cs`, `mods/iiex/tests/Definitions/IiexDefinitionGoldenTests.cs`

**Consumes:** PigBreaking.PigVoxels :16, WorkItemCode :21, MarkerKey :25, UnitsPerVoxel :29, Emit(int voxelsRemoved, ref float remainder) -> (int Chunks, int Bits) :38-48 - all in mods/iiex/src/Items/PigBreaking.cs; PigRecipeDefinitions.Definitions :20-35 as the smithing-recipe shape; vanilla game:ingot-blistersteel

**Produces:** the cold branch of the blister fork: 3 chunks (25 u) + 5 bits (5 u) = 100 u exactly, plus the chunk item wearing mods/iiex/assets/iiex/shapes/item/metalchunk.json

- [x] **Step 1.** ⛔ The total is exact, not "within a crumb", and **it is not one function's output**: the three chunks come off the helve and the five bits are the smithing recipe's own output. A payout that made change every hit would settle in bits - a helve hit sheds one voxel, 2.5 u - and the run would end as twenty bits, so `Emit` pays **chunks only**. Original text: Write the conservation tests first, mirroring mods/iiex/tests/Items/PigBreakingTests.cs: over a full break the payout equals the removed voxels' units to within one sub-bit crumb, and the total is exactly 3x25 + 5x5 = 100. This is the trap PigBreaking already guards - do not re-open it.
- [x] **Step 2.** ⛔ **"110 u in / 100 u out" is not this unit's arithmetic** - it is the pot's charge and its melt loss, a U9.10 number. A vanilla ingot is 100 u and the crush conserves it exactly: **40 voxels at 2.5 u/vx³**, 30 shed and 10 kept. Original text: Copy the PigBreaking arithmetic rather than generalising it in this unit: pick a voxel count that makes units-per-voxel exact for 110 u in / 100 u out, and state the chosen number in a doc-comment the way PigBreaking.cs:14-17 does.
- [x] **Step 3.** ⛔ **Three** identifiers, not one, and they are read by three different things: the stack attribute `iiexblistercrush` is what the helve patch gates on, the recipe `code` is `SmithingRecipe.Code` (real, for 1.23), and the recipe **`name`** is the string the fork is read off - and it **must not be `blistersteel` or `plate`**, because vanilla's `ItemWorkItem` grants helve workability by matching exactly those two. Original text: Add the smithing recipe via ExRecipeDef.Body keyed on a new marker code (not 'iwexpigbreak'), so the helve patch acts only on blister work items and never on ordinary steel smithing.
- [x] **Step 4.** ★★ The gate is **vanilla's own refusal**, not a temperature of ours: the patch is a postfix on `ItemIngot.TryPlaceOn` that acts only where vanilla returned null, which it does exactly below half the melting point (blister melts at 1602, so the line is 801 °C). ⛔ One test was not enough - the **recipe list** had to be forked on the same answer, or a cold ingot on our cold-workable work item could still have vanilla's shear-steel recipe selected and the helve would run it. Both directions pinned. Original text: Caution: Gate on cold. The whole point of the fork is that hot-worked blister keeps vanilla behaviour and becomes shear steel via survival/recipes/smithing/steel.json. Write a test that a hot blister ingot does not enter the crushing route - otherwise the mod silently deletes vanilla's shear-steel path.
- [x] **Step 5.** ⛔ Its own provider, `BlisterItemDefinitions`, **not** a row in `CastPartItemDefinitions` - that file's own doc scopes it to cast-iron parts out of a sand casting cell, and this is neither cast nor iron. The chain's smaller denomination is **vanilla's** `game:metalbit-blistersteel`, 5 u by vanilla's own twenty-to-an-ingot ratio, so only the chunk is minted. Original text: Define the crushed chunk item (shape iwex:item/metalchunk from U9.5) beside CastPartItemDefinitions, with materialUnits carrying 25.
- [x] **Step 6.** Three goldens, not one: the recipe, the chunk and the work item. ⛔ **`IiexLangCoverageTests` covers block codes only** - an item's name key has no guard at all, so `item-blisterchunk` was added by hand in three locales and nothing would have caught its absence. Original text: Bless the smithing-recipe golden with a single-path EXLIB_WRITE_GOLDENS, ./scripts/exmod.sh test 1.21

#### U9.8 — cruciblesteel as a metal def

> ★★ **DONE 2026-08-21**, taken out of order because it is a leaf and depends on nothing else in U9. The
> metal def, the scrap role, the fixture seed, 13 blessed goldens, 19 lang keys per locale and 5 tests.
> 9/9 green at **4,516** per version. ⛔ **There is no preset above `good`** - the emitter's table is
> brittle/standard/good - so "above bessemersteel's good" is an explicit `durability` override riding that
> preset, not a fourth preset name with one consumer. ⛔ **`solidDrop` could not be `game:metalbit-steel`**
> the way bessemersteel's is: iiex ships a guard against paying out in vanilla bits, so the metal generates
> its own and that bit had to be given the scrap role or a broken casting would shed metal the cupola
> refuses.


**Files**
- Create: `mods/iiex/assets/iiex/config/metals/cruciblesteel.json`, `mods/iiex/tests/Definitions/CrucibleSteelMetalTests.cs`
- Modify: `mods/iiex/assets/iiex/lang/en.json`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`
- Test: `mods/iiex/tests/Definitions/CrucibleSteelMetalTests.cs`, `mods/iiex/tests/Definitions/PigIronMetalTests.cs`

**Consumes:** the shipped metal-def schema as written in mods/iiex/assets/iiex/config/metals/castiron.json (code, moltenItem, solidDrop, displayLangKey, castDomain, generateItemFamily, itemForms, texturePath, density, meltingPoint, tools.preset) and mods/siex/assets/siex/config/metals/bessemersteel.json (isAlloy); MetalRegistry + MetalDef in mods/exlib/src/Metals/

**Produces:** iwex:ingot-cruciblesteel and the emitted tool family; meltingPoint 1600

- [x] **Step 1.** ⛔ The obvious form of "highest meltingPoint in the catalogue" **asserts nothing** against `slag`, which declares none: a null compares false against any bound in C#, so the loop looked like it covered three metals and covered two. Slag is now asserted null explicitly. Original text: Write the failing test first, modelled on mods/iiex/tests/Definitions/PigIronMetalTests.cs: the registry resolves cruciblesteel, its meltingPoint is the highest in the iwex catalogue, and its emitted item forms exist.
- [x] **Step 2.** ⛔ **No preset exists above `good`** (`MetalToolEmitter.Presets` is brittle/standard/good), so the raise is an explicit `durability: 3300` riding `good` - which is what the emitter documents per-stat overrides for, and better than minting a fourth preset name with a single consumer. ⛔ `solidDrop` is `iiex:metalbit-cruciblesteel`, **not** `game:metalbit-steel`: `ShippedMetalDropTests.No_shipped_metal_pays_out_in_vanilla_bits` forbids it, and the bit needed a `scrap` row in `materialroles.json` (and in the harness's `MaterialRoleSeeds` mirror) or the cupola would refuse what a broken casting sheds. Original text: Author the JSON with isAlloy:true (it is a made steel, like bessemersteel), density ~7820, meltingPoint 1600, tools.preset above bessemersteel's 'good'. MetalDef.Durability + MetalToolEmitter already emit the whole tool family from this one entry - no per-tool work.
- [x] **Step 3.** Recorded as its own section in `docs/design/materials.md`, including the half the step did not name: the raise is **durability only**, because moving mining tier would be a progression gate where durability is not. Original text: Caution: Record in docs/design/materials.md that this raises vanilla's tool ceiling deliberately and that it gates nothing - wrought-iron heads still work. That is the settled ruling and it needs a home outside the machine page.
- [x] **Step 4.** Not attempted, and the reason is written into materials.md rather than left in the plan. Original text: Caution: Do not attempt the 'crucible-steel machine heads last longer' half. Drill bits, shear blades and roll sets have no durability or wear mechanic at all; that is a new system and is explicitly out of U9.
- [x] **Step 5.** ⛔ One key is not enough: `generateItemFamily` emits **19** codes - the metal name, five item forms and eight tools - and `IiexLangCoverageTests` wants every one in every locale. Original text: Add lang keys iwex:metal-cruciblesteel in all three locales, ./scripts/exmod.sh test 1.21

#### U9.9 — Crucible-furnace core + hearth blocktypes and the layout

> ★★ **DONE 2026-08-22.** `BlockCrucibleFurnaceCore`, `BlockCrucibleHearth`, `CrucibleHearthLayout`, two
> block entities, two blessed goldens, 17 tests and lang in three locales. 9/9 green at **4,585** per version.
> ★★ **The 1600 °C ceiling is cleared** - and not by the draught curve. A firebox carrying the
> reverberatory losses peaks at **1471 °C** at nine courses, short at any height; a transfer loss of zero
> (the pots stand in the fire) and half a firebox's charge loss (the charge is walled off from it) land
> **1604 °C at six courses and 1621 at the nine-course peak**.
> ⛔⛔ The guard `EveryFireboxReachesItsOwnProcessTemperature` fired the moment the block entity existed and
> is what forced the calibration. Its premise - "the stack its drawing declares" - does not hold for a
> machine whose chimney is the player's, so `StackCourses` reports the rated height as a **marked
> placeholder** until U9.10 writes the walk.
> ⛔⛔ **`DoorCell` had to be overridden.** The branch's default is the offset both reverberatory drawings
> agree on; on this drawing that cell is brick, so an inherited door resolves to nothing and `Venting` reads
> false with the mouth standing open.

**Files**
- Create: `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockCrucibleFurnaceCore.cs`, `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockCrucibleHearth.cs`, `mods/iiex/src/BlockStructures/Furnaces/CrucibleHearthLayout.cs`, `mods/iiex/assets/iiex/textures/block/furnace/csf.png`, `mods/iiex/tests/goldens/iiex/blocktypes/furnace/cruciblecore.json`, `mods/iiex/tests/goldens/iiex/blocktypes/furnace/cruciblehearth.json`, `mods/iiex/tests/Blocks/Furnaces/CrucibleFurnaceLayoutTests.cs`
- Modify: `mods/iiex/src/Generated/IiexBlocks.g.cs`, `workbench/layouts.md:518-589`, `docs/design/machines/crucible-furnace.md`, `mods/iiex/assets/iiex/lang/en.json`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`
- Test: `mods/iiex/tests/Blocks/Furnaces/CrucibleFurnaceLayoutTests.cs`, `mods/iiex/tests/Blocks/Furnaces/FurnacePartsTests.cs`, `mods/iiex/tests/Blocks/Furnaces/FurnaceRoleCellsTests.cs`
...
- [x] **Step 1.** One hearth cell, as the art dictates. `CrucibleHearthLayout` maps the four pot groups (`Crucible{n}`, `BlisterSteel{n}`, `Slag{n}`, `Cover{n}`) and the bed comes from `BlockFirebox.ElementsFor` unchanged. The Open item is closed as "neither". Original text: Take the drawn geometry as authoritative: workbench/shapes/furnace-block-draftcruciblehearth.json is one 16x16x16 block (measured bbox x -1..16, y -1..14, z 0..16) containing four clay stands in a 2x2, four pots, and a six-course coke bed. So the layout gets one hearth cell, not four holes. This settles crucible-furnace.md Open 'whether the four pot cells are parts or fillers' - neither.
- [x] **Step 2.** ★ Further than the step asked: `BlockCrucibleHearth` **derives from** `BlockFirebox`, so the charge gesture, the course-by-course draw-down, the fuel retexture and the break drop are inherited rather than reimplemented. `DefinitionsOf` uses `DeclaredOnly`, so a def-providing base is safe and already precedented by the special pipes. Ignites at 12 units, as the branch requires. Original text: Give BlockCrucibleHearth a BEBehaviorFirebox (as BlockFirebox does at BlockFirebox.cs:52) so the coke bed is the same substrate every other fuel bed uses, and mark the hearth cell CellRole.Firebox in the layout. MinChargeToIgnite is sealed to FireboxCellCount * FireboxMixPerCell (BlockEntityFireboxFurnace.cs:241-242), so a one-cell hearth ignites at 12 units - do not try to introduce a CrucibleMixRequiredToFire constant, the branch forbids it.
- [x] **Step 3.** Rewritten as the shipped drawing, with each of the draft's own three flagged gaps answered below it. No chargelid: the crucible has no crown. Original text: Rewrite the draft at workbench/layouts.md:518-585 against reality: 'iwex:draftcruciblefurnacehearth*' / 'iwex:draftcruciblefurnacecore-*' / 'iwex:chargelid-north*' -> the generated IiexBlocks accessors; ExCodes.Refractory/AnyBricks/Air/CokeOvenDoor -> VanillaCodes.* (CokeOvenDoor is a method taking a BlockFacing, VanillaCodes.cs:280, and VanillaCodes.Sealing(wall) :289 is the intended call because vanilla's spelling is inverted).
- [x] **Step 4.** All three. ⛔ The damper is `BlockPuddlingChimneyCap` faced **south**, not north: its footprint is two cells along its own +Z, so a north-faced cap would put its housing filler inside the flue. Original text: Fix the three gaps layouts.md itself flags: add the firebox/fuel cell (now the hearth block), give the flue column its own glyph on the air code so CellRole.Flue does not also claim the open cells at rows 0-1 (idiom 1 in layouts.md:192), and draw the damper at the flue base with CellRole.Damper. This is the first production use of CellRole.Damper anywhere.
- [x] **Step 5.** One course, exactly that ring. ⛔⛔ The consequence the step does not name: **the walk has to exist for the machine to be honest**, and it is U9.10's. `StackCourses` reports the rated height in the meantime, marked as a placeholder in the file, the design doc and the Open list. Original text: Author only the minimum stack in the layout - the base course - and leave the rest to the player, per the settled 'the core owns the walk' rule. A course is `. b . / b a b / . b .`, the exact predicate smex:smokestack already draws (BlockSmokeStackIntake.cs:80-148) with VanillaCodes.AnyBricks for b and VanillaCodes.Air for a.
- [x] **Step 6.** Reused. ⛔ Nothing reads it yet: `BlockEntityFireboxFurnace.ScanForOutlets` resolves its cap at the cell **over the highest flue course**, and this furnace's damper sits at the foot - which is what `CellRole.Damper` is marked for and what U9.10's own reader will use. Original text: Reuse the puddling chimney cap as the damper block (BlockPuddlingChimneyCap.cs:30-68, BlockEntityPuddlingChimneyCap.IsOpen :22 / Toggle() :25-30) rather than defining a second one, per the design's 'one implementation should serve both'.
- [x] **Step 7.** ⛔ **Nothing drawn: `mods/iiex/assets/iiex/textures/block/furnace/cs.png` already exists**, reads "CS", is in the shipped set's style and was unused. A third letter jammed in beside it the way `bfc`/`bfh` do would have been worse and unnecessary. Original text: Draw mods/iiex/assets/iiex/textures/block/furnace/csf.png as the core's south-face label, matching the shipped 2-letter set.
- [x] **Step 8.** All of it, plus two the step did not ask for: the hearth resolves as the furnace's own part at every facing (which needed a new `SeatCrucibleHearth` rig helper, since `LoadFireboxes` places the firebox blocktype and this cell requires the hearth), and the work door is the cell the drawing puts one in. The role-set case became a four-way theory over every firebox machine, with the coke oven's single-role set pinned beside the others. Original text: Tests: stand the structure at all four sides with FurnaceLayoutRig.Stand and assert it completes without forcing StructureComplete; assert the hearth cell carries Firebox, the column carries Flue, the base carries Damper; assert RoleNamesOf returns exactly ["Firebox","Flue","Damper"]. Also add both new machines to the loop in FurnaceRoleCellsTests.cs:733-758, which today pins the exact role set for the two hearths only.
- [x] **Step 9.** Codes regenerated, both goldens blessed, the parts floor raised 15 -> 18 and its "five structures" comment corrected to six. Original text: Regenerate codes, bless both goldens by explicit path, raise FurnacePartsTests.cs:50's floor again, ./scripts/exmod.sh test 1.21

#### U9.10 — BlockEntityCrucibleFurnace - seat, preheat, melt, crack, pull

> ★★ **DONE 2026-08-22. The machine works.** `CrucibleHole`, `CruciblePot`, `BlockEntityCrucibleHearth`,
> the furnace's melt cycle, the counted player-built chimney, the damper read off `CellRole.Damper`, the
> R7 readout, eight config keys, fourteen lang keys per locale and 31 tests. 9/9 green at **4,616** per
> version.
> ★★ **Step 1b's inversion is what shipped**: preheat with the damper shut, melt with it open.
> ⛔⛔ **The preheat cannot live in `SmeltCycle`** - the core calls it only above process temperature, and a
> furnace with its damper shut never gets there, so the phase the damper is shut *for* would never run. It
> runs from `OnProductionTick`.
> ⛔⛔ **The reachability guard was wrong in two ways, not one.** It read `StackCourses` (nothing built) and
> relied on `DamperOpen` defaulting true. Both are now stated rather than assumed, through
> `RatedStackCourses`, `BestNaturalDraught` and a `ComputeHeatBalanceAt` overload.
> ⛔⛔ **`BlockSmeltedContainer.SetContents`/`GetContents` are not public before 1.22.** The pot's contract
> is the two stack attributes, written by hand and read back through vanilla's own method where the version
> exposes it.

**Files**
- Create: `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityCrucibleFurnace.cs`, `mods/iiex/src/BlockStructures/Furnaces/BlockEntities/BlockEntityCrucibleHearth.cs`, `mods/iiex/tests/Blocks/Furnaces/CrucibleFurnaceTests.cs`, `mods/iiex/tests/Blocks/Furnaces/CrucibleHearthSlotTests.cs`
- Modify: `mods/iiex/src/IiexConfig.cs`, `mods/iiex/assets/iiex/lang/en.json`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`
- Test: `mods/iiex/tests/Blocks/Furnaces/CrucibleFurnaceTests.cs`, `mods/iiex/tests/Blocks/Furnaces/CrucibleHearthSlotTests.cs`, `mods/iiex/tests/Blocks/Furnaces/HeatBalanceTests.cs`
...
- [x] **Step 1.** ⛔ Already red from U9.9, and the shortfall was in the losses rather than the stack (see U9.9). Pinned here at three points instead of one: the rated chimney clears 1600, a bare one does not, and a shut damper is a **cooler** fire. ⛔ The per-machine charge-loss override the step asks for exists as `ChargeLossFull`; `BfChargeLossFull`'s 310 was already replaced by the firebox branch's 100, and this machine halves it again. Original text: Write the temperature test first and make it fail: with the drawn stack height and the damper shut, ComputeHeatBalance's T_process must clear MeltingPoint = 1600. This is B15/B2's exact failure mode and the only thing that decides whether the machine works at all. Pin it in HeatBalanceTests beside the shaft numbers, and pin the per-machine charge-loss override too (BfChargeLossFull's full 310 is wrong for a firebox - BlockEntityFurnaceCore.cs:1241-1244).
- [x] **Step 1b.** Applied exactly as written: shut to preheat, open to melt, and the crack belongs to the preheat. ⛔⛔ The consequence the ruling does not name: **the preheat cannot run in `SmeltCycle`**, because the core only calls that above process temperature and a shut damper never reaches it. Original text: Caution: **Ruled 2026-08-05: a shut damper means less draught and a cooler fire** (U6.5's rule; the conventional stove reading). **This task is the one that must change, not U6.5 — and it was backwards for its own machine, not merely inconsistent with the puddling furnace.** A Huntsman crucible furnace is a deep hole: pots on a grate, coke packed around, ash pit below, flue above, air drawn up through the grate by chimney draught alone. Melting steel at ~1600 °C is at the limit of what coke and natural draught reach — which is *why* these furnaces had famously tall stacks. **Open flue damper = full draught = fiercest.** Steps 1 and 5 below say "damper open = damped fire" and require T_process ≥ 1600 with the damper *shut*; invert both.
> 
...
- [x] **Step 2.** Done in U9.9, and it was a virtual, so nothing had to be raised with U6.4. ⛔ Zero was not enough on its own - see U9.9's note. Original text: Caution: Override the transfer loss to ~0. The reverberatory transfer term **U6.4** introduces for the puddling and reheat furnaces is what correctly stops a reverberatory melting iron; this furnace's pots sit in the coke bed with no bridge, so it must not pay it. If it is a branch constant rather than an overridable virtual, this machine cannot work - raise that with **U6.4** rather than working round it.
- [x] **Step 3.** `MeltIntervalSec` is the one cadence key this machine takes; the fuel clock and melt-start delay are the firebox branch's, as the design's own table says ("inherit"). ★ `SmeltCycle` advances each pot by the **interval** rather than by `dt`, which is what makes a taller stack finish a heat sooner: the core shortens the gap between cycles above the melt line, so accruing `dt` would have thrown that reward away. Original text: Derive BlockEntityCrucibleFurnace from BlockEntityFireboxFurnace and override MeltingPoint => a new IiexValues.CrucibleMeltingPoint (1600), plus the melt cadence keys ruling 2 leaves standing. Implement SmeltCycle(object) to act on each seated pot, not on a burden column - the same override shape BlockEntityHeatingFurnace.cs:54 leaves empty.
- [x] **Step 4.** Cycle-to-next-free, not selection boxes. ⛔ `TakeCharge` had to be **all-or-nothing**: a pot ten units short that swallowed a 25 u chunk ate the difference, and a test caught it. That is why the charge is four chunks and two bits rather than a round number of either. Original text: Pot slots are sub-block: the hearth is one cell with four stands, so the 'which cell you click picks which hole' idiom (BlockHeatingHearth.RowAt :72-78) does not apply. Implement four selection boxes or a cycle-to-next-free rule on BlockEntityCrucibleHearth, and pin slot->element mapping (Crucibles/Crucible1..4, Covers/Cover1..4, FillingBlisterSteel/BlisterSteel1..4, FillingSlag/Slag1..4) with a test that every named element exists in the shipped shape - the pattern FurnacePartsTests.cs:57-91 already uses for the two hearths.
- [x] **Step 5.** Both directions, at the inverted polarity: full draught on a cold pot breaks it, a shut damper brings it up, and a preheated pot takes the full fire. Original text: Damper as preheat: damper open = damped fire = the anneal; closing it on cold pots destroys them. Tests must cover both directions - a cold pot cracks when the damper is shut on it, and a preheated one does not.
- [x] **Step 6.** Into the ash pit, and the units are asserted whole rather than only present. Original text: Cracked pot spills its charge one block down as items. Test that the metal survives; the pot is lost, the steel is not. A silent void here is the puddling-hearth mistake repeated.
- [x] **Step 7.** Plus the case the step does not name: a re-charged pot starts its next heat from nothing, so the melt clock resets with the charge. Original text: 110 u blister -> 100 u crucible steel per pot (~9% melt loss). Assert the loss is applied once per heat over two consecutive heats, and that four pots is four independent heats.
- [x] **Step 8.** ⛔ **There is no such path to confirm.** `BfMaxMoltenSlag` no longer exists and the molten-slag accumulator lives on `BlockEntityShaftFurnace`, a branch this furnace is not on. Asserted structurally instead, so re-parenting would fail it. Original text: Slag: confirm the inherited BfMaxMoltenSlag path is genuinely unused rather than quietly accumulating (crucible-furnace.md Open). One assertion.
- [x] **Step 9.** Three keys rather than one with a direction word appended, so a translator gets whole sentences. Plus a damper line, which is invisible from outside the furnace and is the difference between a heat and four broken pots. Original text: R7 readout: block info must show `stack: N courses -> draught X (peak at P)` with the direction of travel. A declining draught past the peak is indistinguishable from a bug without it. Add lang keys in all three locales.
- [x] **Step 10.** Green.

#### U9.11 — The pour target, construction, and the end-to-end gate

> ★★ **DONE 2026-08-22. U9 IS CLOSED.** The gate scenario runs coal to a crucible-steel ingot through
> every machine the unit built, two grid recipes and two cost rows make the furnace craftable, and the
> handbook page ships in three locales. 9/9 green at **4,623** per version.
> ⛔⛔ **Steps 1, 2 and 7 name work that does not exist or was already done.** The pour target - the
> `castingotmold` pattern row, its cavity box and the cast-iron ingot mould - has been shipped since the
> casting line; and `scripts/test-floors.txt` is not a file. The floors this repo has are **coverage
> percentages** in `infra/tools/coverage_gate.py`, which adding tests raises rather than needing raised.
> ⛔⛔ **Step 4's rhythm is the old polarity** - "preheat on the open damper; shut it; melt" - superseded by
> Step 1b's inversion. The scenario preheats **shut** and melts **open**.
> ⛔ Two harness defects surfaced: an item registered in `TestWorld` carried **no api handle**, so
> `CollectibleObject.Equals` threw inside a pour; and a `BlockEntityCastMold` that is placed but never
> `Initialize`d reads as **full at zero capacity** and silently refuses everything poured into it.

**Files**
- Create: `mods/iiex/tests/Scenarios/CrucibleSteelScenarioTests.cs`, `mods/iiex/docs/handbook/NN-cruciblefurnace.html`, `mods/iiex/tests/goldens/iiex/recipes/grid/cruciblefurnace.json`
- Modify: `mods/iiex/src/BlockStructures/Casting/PatternItemDefinitions.cs:72-171`, `mods/iiex/src/Items/CastPartItemDefinitions.cs`, `mods/iiex/src/Recipes/Grid/FurnaceRecipeDefinitions.cs`, `mods/iiex/src/IiexRecipeConfig.cs:46-77`, `mods/iiex/assets/iiex/lang/en.json`, `mods/iiex/assets/iiex/lang/ru.json`, `mods/iiex/assets/iiex/lang/uk.json`
- Test: `mods/iiex/tests/Scenarios/CrucibleSteelScenarioTests.cs`, `mods/iiex/tests/Casting/PatternValidationTests.cs`, `mods/iiex/tests/Casting/MoldSpecTests.cs`, `mods/iiex/tests/Definitions/IiexCostSelectorTests.cs`

**Consumes:** PatternItemDefinitions.PatternShapes :72-82 and Molds :86-171; Mold(shape, capacity, cavityBoxes, output, outputType:, size:) as used at :143-172; MoldSpec/MoldSize.Cell (mods/iiex/src/BlockStructures/Casting/MoldSpec.cs:9-16); BlockEntitySandCastingCell.Imprint's iwex-castingcell-wrongsize refusal
...
- [x] **Step 1.** ⛔ **Already shipped.** `PatternShapes["castingotmold"]` and the matching `Molds` row - shape, cavity box, `outputType: "block"` - have been in `PatternItemDefinitions` since the casting line, and `mods/iiex/assets/iiex/shapes/casting/cell-filling-ingotmold.json` with them. Nothing to add. Original text: Add one row to PatternItemDefinitions.PatternShapes and one to Molds for castingotmold: shape iwex:casting/cell-filling-ingotmold (from U9.5), the cavity box measured off the drawn art, output the mould block, size defaulting to MoldSize.Cell (it is a 1x1 cast, unlike the LongCell stock). The file's own comment at :83-84 says adding a castable part is one line here plus the output item and the filling shape - hold it to that.
- [x] **Step 2.** Moot with Step 1: the shipped row already carries `capacity` 152 as the mass of the mould, with a comment saying exactly that. Original text: Caution: Capacity is the mass, not the drawn volume. Ruling 1: the sand cavities are illustrative; `capacity` on the pattern is what a pour measures against (PatternItemDefinitions.cs comment block above :143). Do not derive it from voxels and do not propose a redraw.
- [x] **Step 3.** Both, on patterns that collide with none of the five existing cores. ⛔ The step's own count is stale too - the catalogue has 25 rows, not 12, and one of them is the crucible pot's. Keys follow the file's `*-grid` convention: `cruciblecore-grid`, `cruciblehearth-grid`. ⛔ The cupola still has no cost row; still not this unit's. Original text: Add crucible-furnace core and hearth grid recipes to FurnaceRecipeDefinitions with patterns that collide with none of blastfurnace/cupola/cokeoven, and cost rows cruciblefurnacecore-grid + cruciblehearth-grid to IiexRecipeConfig.Defaults(). crucible-furnace.md claims those two keys already exist at IiexRecipeConfig.cs:47-70 - they do not; the shipped catalogue has 12 rows and none of them is a crucible or a cupola.
- [x] **Step 4.** ⛔ **The step's rhythm is the pre-inversion one** and is not what shipped: the scenario preheats with the damper **shut** and melts with it **open**, per Step 1b. Nothing forces `StructureComplete`; both furnaces stand their real footprints and light on their own ticks. ⛔ Two links are arithmetic rather than live and are marked in the file: the helve crush (no anvil in the harness) and vanilla's held-item pour, which is driven through the sink's own `ReceiveLiquidMetal` instead. Original text: Write the gate scenario end to end in one test class, modelled on mods/iiex/tests/Scenarios/ColdBlastFurnaceScenarioTests.cs: bituminous coal -> coke oven -> coke; vanilla ingot-blistersteel -> cold helve -> 3 chunks + 5 bits; charge a fireclay pot; build the crucible furnace and its stack; preheat on the open damper; shut it; melt; pull with tongs; pour into a cast-iron ingot mould; get iwex:ingot-cruciblesteel. Never force StructureComplete anywhere in it - build the real footprint.
- [x] **Step 5.** Both halves, as separate cases: it completes and lights with no chimney at all and simply never reaches its melt line, and every course laid makes it hotter. Original text: Assert in that scenario that the furnace completes and runs cold with no stack built, and gets hotter as courses are added. That is the intended, legible behaviour (the chimney is deliberately outside StructureComplete) and it must be pinned, not 'fixed'.
- [x] **Step 6.** `12-cruciblefurnace`, blessed into en from the HTML and translated by hand into ru and uk. Original text: Write the handbook page with the NN- prefix and mirror the keys into en.json; add every new lang key in ru.json and uk.json too (single hyphen, no em-dash).
- [x] **Step 7.** Golden blessed. ⛔ **`scripts/test-floors.txt` does not exist**, and there is no test-count floor anywhere: `infra/tools/coverage_gate.py` holds *coverage percentage* floors, which more tests raise rather than needing raised. Original text: Bless the recipe golden by explicit path; raise scripts/test-floors.txt IronworkingExpanded.Tests above its current 1132 by ~95% of the new count; ./scripts/exmod.sh test 1.21
...
> ★★ **DONE 2026-08-22. U10 IS CLOSED, and with it the whole U2-U10 expansion.** A layout demands that a
> cell opens outward rather than that it wears a variant; the three shaft furnaces are migrated onto it;
> pinning a node is refused at build. 9/9 green at **4,655** per version.
> ⛔⛔ **The scheme contract's natural filter reads a subset.** Scanning `BlockNetworkNode` subtypes misses
> every def authored by a stand-alone provider - iiex's plated and cast pipes come from
> `PlatedPipeDefinitions`, not from `BlockPipe` - so it saw **26 of 38** iiex node defs. Matching defs by
> their `class` key sees all 43. The pre-existing `type`-group rule has the same blind spot.
> ⛔⛔ **`GetBehavior<T>()` reads `CollectibleBehaviors`, not `BlockBehaviors`**, so a double that fills only
> the latter answers null to every production lookup.
> ⛔⛔ **The rig must RAISE a connector cell, not only count it** - a plain stand-in can never satisfy the
> demand, and twenty furnace tests went red until `Raise()` placed a node stand-in.
> ⛔ **Step 4's scheme mapping and Step 3's passthrough are stale**: the canal start is `Face`, not
> `DirectedAxisFlat`; the mold pedestal, shear and fastener bench are missing from the list; and no layout
> has ever had a passthrough legend. Five pins across three layouts, all tuyeres.
> ★★ **The scheme is derived, not named.** `ExBlockDef.NetworkOriented()` reads it off the def's own
> `orientation` states, so the misspelling Step 5 hardens against is unrepresentable in a code-first def.
> ⛔ The mod names throughout this section predate the merges: there are two mods, `iiex` and `siex`, and
> two `{Mod}NetworkNodeContractTests`, not five.
...
**Shared files** (collision risk): `mods/exlib/src/Blocks/Networks/BlockNetworkNode.cs — U4's tap/plug work has this file open for the same reason U10 does; two units editing Rotate/Recalculate concurrently will collide`, `mods/exlib/src/Blocks/Structures/BlockEntityMultiblockStructure.cs — U2, U3 and U6 all reshape furnace completion/tick behaviour through this base; U10.4 adds a branch inside IncompleteBlockCount`, `mods/exlib/src/Definitions/MultiblockLayoutBuilder.cs — U2 (layered-charge layouts), U5 (burdenmaker), U8 (three new megablocks) all author through it; U10.4 adds a DSL verb and U10.6 adds a refusal`, `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockBlastFurnaceCoreCold.cs — U2/U3 redraw this layout wholesale (tuyeres move y=1 → y=2, layer 2 becomes a full 3×3 raceway); U10.5 edits the same legend block`, `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockCupolaFurnaceCore.cs — same, plus U5's direct pig+scrap charging`, `mods/siex/src/BlockStructures/HotBlastFurnace/Blocks/BlockBlastFurnaceCoreHot.cs — mirrors the cold furnace's drawing; every cold-furnace layout change lands here too`, `mods/iiex/tests/Fixtures/FurnaceLayoutRig.cs — the shared oracle for iwex, lpex and smex furnace suites; a constant change here breaks three suites at once, and U2–U5 all touch it`, `mods/iiex/tests/Fixtures/ColdBlastFurnaceScenes.cs and mods/siex/tests/Fixtures/BlastFurnaceScenes.cs and mods/iiex/tests/Fixtures/CupolaScenes.cs — the three scene fixtures that place real tuyeres; U2/U3/U4 rewrite their melt and tap paths`, `mods/exlib/testing/StructureRig.cs — every megablock suite in the repo goes through it; U10.4 changes what `Missing` counts`, `mods/exlib/testing/NetworkNodeContract.cs and MultiblockCodes.cs — the shared per-mod guards, consumed by all five `{Mod}NetworkNodeContractTests``, `test/{ExpandedLib,IronworkingExpanded,LowPressureExpanded,HighPressureExpanded,SteelmakingExpanded}.Tests/goldens — 41 node blocktypes (U10.1) plus the three furnace cores (U10.5); any other unit blessing goldens in the same window will produce a confusing merged diff`, `scripts/test-floors.txt — one row per suite, read by both runners; must move in the same change as any net test-count drop`
...
`mods/exlib/src/Blocks/Networks/` (namespace `ExpandedLib.Blocks.Networks`); registered class keys are
`exlib.*` and iwex's plated-tier defs come from `mods/iiex/src/BlockNetworkPipe/PlatedPipeDefinitions.cs`.
Symbols are unchanged — navigate by symbol; the file list below predates the move.

**Files**
- Modify: `mods/exlib/src/Blocks/Behaviors/BlockBehaviorExOrientable.cs:104-125`, `mods/exlib/testing/NetworkNodeContract.cs:37-67`, `mods/exlib/src/Blocks/Networks/BlockPipe.cs:82-160 (straight/bend/tjunction/xjunction)`, `mods/exlib/src/Blocks/Networks/BlockPipePassthrough.cs:120`, `mods/iiex/src/BlockNetworkMolten/Blocks/BlockMoltenCanal.cs:194-228 (CanalFamilyDef, per CanalTypeSpec)`, `mods/iiex/src/BlockNetworkMolten/Blocks/BlockMoltenCanalTap.cs:56`, `mods/iiex/src/BlockNetworkEnergy/Blocks/BlockCastIronShaft.cs:40`, `mods/iiex/src/BlockNetworkEnergy/Blocks/BlockCastIronBevel.cs:49`, `mods/iiex/src/BlockNetworkEnergy/Blocks/BlockFlywheel.cs:118`, `mods/iiex/src/BlockStructures/Forming/Blocks/BlockRollingMill.cs:56`, `mods/iiex/src/BlockStructures/Forming/Blocks/BlockRollingMillAxle.cs:47`, `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockTuyere.cs:32`, `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockTwinTubMPBlower.cs:59`, `mods/iiex/src/BlockNetworkPipe/Blocks/BlockFluidIntake.cs:27`, `mods/iiex/src/BlockNetworkPipe/Blocks/BlockPipeOutlet.cs:51`, `mods/iiex/src/BlockNetworkPipe/Blocks/BlockValve.cs:39`, `mods/iiex/src/BlockNetworkPipe/Blocks/BlockPressureValve.cs:39`, `mods/siex/src/BlockStructures/SmokeStack/Blocks/BlockSmokeStackIntake.cs:154`, `test/*/goldens/**/blocktypes (the 41 files carrying a `"code": "orientation"` variant group)`
- Test: `mods/exlib/tests/Blocks/Behaviors/ExOrientableTests.cs`, `mods/iiex/tests/Definitions/IiexNetworkNodeContractTests.cs`, `mods/iiex/tests/Definitions/IiexNetworkNodeContractTests.cs`, `mods/siex/tests/Definitions/SiexNetworkNodeContractTests.cs`, `mods/siex/tests/Definitions/SiexNetworkNodeContractTests.cs`
...
- [x] **Step 1.** Added, but NOT filtered on the base class. ⛔⛔ A `BlockNetworkNode` subtype scan misses every def a stand-alone provider authors - iiex's plated and cast pipes come from `PlatedPipeDefinitions` - so it examined **26 of 38**. Defs are matched by their `class` key against a reverse map instead. Original text: Add `NetworkNodeContract.SchemeViolations(domain, asm, out int defsChecked)` beside the existing `Violations` (NetworkNodeContract.cs:43): for each concrete `BlockNetworkNode` subtype from `ReflectionScan.GetCandidateTypes`, for each `ExDefinitions.DefinitionsOf(type, domain)` def, read `def.ToJson()["behaviors"]`, require exactly one entry named `ExOrientable` with `properties.mode == "network"`, and require `ExOrientations.Resolve(def.VariantStates("orientation"))?.Name` to equal `properties.scheme`. Increment `defsChecked` per def examined.
- [x] **Step 2.** Two, not five - the merges left `iiex` and `siex`. Both assert `Empty` and `defsChecked > 0`. Original text: Add to each of the five `{Mod}NetworkNodeContractTests.cs` a fact calling `SchemeViolations` and asserting both `Assert.Empty(violations)` and `Assert.True(defsChecked > 0)` — the non-zero assert is mandatory, the sibling `MultiblockCodes.Unresolvable` (MultiblockCodes.cs:42-47) exists precisely because a checker that examines nothing passes forever.
- [x] **Step 3.** 43 across two mods: 38 iiex, 5 siex. Original text: watch every node def listed as a violation (~41 across three mods).
- [x] **Step 4.** ★★ Done through `ExBlockDef.NetworkOriented()`, which reads the scheme off the def's own `orientation` states and throws when they set-equal none - so the name cannot be misspelt rather than being caught when it is. ⛔ The step's mapping is stale twice: the canal start declares `[n,w,s,e]` = **Face**, not `DirectedAxisFlat`; and the mold pedestal, shear and fastener bench are network nodes the list omits. Original text: Add `.Behavior("ExOrientable", new { mode = "network", scheme = "<Name>" })` to each def, taking `<Name>` from `ExOrientations.Resolve(states)!.Name`: pipe straight/passthrough/shaft/bevel = `Axis`; flywheel/mill/millaxle/canal straight+pass = `AxisFlat`; valve/pressurevalve = `DirectedAxis`; canal start = `DirectedAxisFlat`; tuyere/blower/fluidintake/canal tap/smokestack intake = `Face`; pipe outlet = `FaceAll`; pipe bend = `PipeBend`; pipe tjunction = `PipeTee`; pipe xjunction = `PipeCross`; canal bend/tee/cross = `CanalBend`/`CanalTee`/`CanalCross`.
- [x] **Step 5.** The fallback stays for a JSON-authored block and now records the unresolved name (`UnresolvedScheme`), so a test tells "named nothing" from "named something wrong"; both cases are pinned. Step 4 makes the typo unreachable from a code-first def. Original text: Harden BlockBehaviorExOrientable.cs:118-121: today an unrecognised `scheme` string silently becomes `ExOrientations.Axis`, which would make `ApplyOrientation` refuse every real token on a bend. Log a `world`-less `Logger`-free failure is not available in `Initialize`, so instead keep the fallback but record the unresolved name and add an ExOrientableTests case asserting the fallback is reached only for a null/absent scheme; the typo case is what U10.1's contract test catches at build.
- [x] **Step 6.** 43, scoped. Original text: Re-bless the 41 goldens with a scoped list, e.g. `EXLIB_WRITE_GOLDENS=iwex/blocktypes/pipe,iwex/blocktypes/molten,iwex/blocktypes/mpenergy,iwex/blocktypes/forming,iwex/blocktypes/furnace/tuyere,iwex/blocktypes/furnace/twintubblower,lpex/blocktypes/pipe,hpex/blocktypes/pipe,smex/blocktypes/smokestack` — never `=1`.
- [x] **Step 7.** Confirmed: 43 files, additive only - the sole removed lines were two closing braces that gained a comma. Original text: Diff the blessed goldens and confirm the only change in each is a new entry in the `behaviors` array. Any other movement means a def was edited by accident.
- [x] **Step 8.** Green at 4,627. Original text: Run `./scripts/exmod.sh test 1.21` green on all three suites.

#### U10.2 — Route BlockNetworkNode's two real exchange sites through ApplyOrientation

**Files**
- Create: `mods/exlib/tests/Blocks/Networks/NetworkNodeOrientationTests.cs`
- Modify: `mods/exlib/src/Blocks/Networks/BlockNetworkNode.cs:364-409 (Rotate)`, `mods/exlib/src/Blocks/Networks/BlockNetworkNode.cs:808-857 (RecalculateAndSyncOrientations)`, `mods/exlib/src/Blocks/Behaviors/BlockBehaviorExOrientable.cs:139-152 (ApplyOrientation)`
- Test: `mods/exlib/tests/Blocks/Networks/NetworkNodeOrientationTests.cs`, `mods/exlib/tests/Blocks/Behaviors/ExOrientableTests.cs`, `mods/iiex/tests/Invariants/PipeInvariantTests.cs`
...
- [x] **Step 1.** `NetworkNodeOrientationTests`, seven cases. ⛔ Six were green on the unchanged code - they are the characterisation net the refactor had to keep - and only the missing-behaviour case was red. Original text: Write the failing test file first. Case 1 (wrench): stand a straight-pipe family (`ns`/`we`/`ud`, scheme `Axis`, behaviour attached) in a TestWorld, call `Rotate(entity, sel, +1)`, assert the block at pos wears the next declared token, that `World.Networks` re-registered the node (query `NetworkAt(pos)` connector faces), and that `MarkBlockDirty` was observed for that pos.
- [x] **Step 2.** Both, plus a third asserting the new choices reach the block entity. Original text: Case 2 (recalculate): place a node wearing `we` with a compatible neighbour that forces `ns`, call `RecalculateAndSyncOrientations`, assert the exchange happened and `BlockEntityNetworkNode.PossibleOrientations` was updated. Case 3: call it again with nothing changed and assert no second exchange (ApplyOrientation returns false when the block already wears the token — pinned by ExOrientableTests.cs:204-214, and load-bearing because this runs on every neighbour notification).
- [x] **Step 3.** Refuse and log in `Rotate`; refuse **silently** in `RecalculateAndSyncOrientations`, which runs on every neighbour notification and would otherwise log for as long as the block stands. Original text: Case 4 (negative): a node whose block carries no `ExOrientable` behaviour must still not crash — decide and pin the contract (either fall back to the old inline rewrite, or refuse and log). U10.1 makes the behaviour universal, so "refuse and log" is defensible and is the stronger statement.
- [x] **Step 4.** ⛔⛔ It did not fail there. `GetBehavior<T>()` reads `CollectibleBehaviors`, not `BlockBehaviors`, so a double filling only the latter answered **null** and every swap was refused - `ExOrientableRig` had been papering over the same thing with a `?? _behaviour` fallback. Original text: Run the new tests and watch Case 1's MarkBlockDirty assertion fail: `ApplyOrientation` (BlockBehaviorExOrientable.cs:144-151) does `GetBlock` + `ExchangeBlock` and nothing else, while the sites it replaces also call `MarkBlockDirty` (BlockNetworkNode.cs:400, :854).
- [x] **Step 5.** Moved in, both call sites' copies deleted, and the reason recorded on the method. Original text: Add `world.BlockAccessor.MarkBlockDirty(pos)` to `ApplyOrientation` after the exchange, and delete the now-duplicate calls at the two sites. Note in the XML doc that headless tests cannot see a missing mark — the test above is the only thing that will.
- [x] **Step 6.** Inside the `RemoveNode`/`AddNode` sandwich, with the missing-behaviour refusal ahead of it so the graph is never opened for a swap that cannot happen. Original text: Replace BlockNetworkNode.cs:388-399 with `bool swapped = GetBehavior<BlockBehaviorExOrientable>()?.ApplyOrientation(world, pos, choices[nextIndex]) ?? false;` inside the existing `netManager.RemoveNode(...)` / `netManager.AddNode(...)` sandwich, keeping `be?.MarkDirty(true)` and the neighbour `RecalculateAndSyncOrientations` loop (:404-405) exactly where they are.
- [x] **Step 7.** On `netBlock`, as the step says. Original text: Replace BlockNetworkNode.cs:846-855 with `netBlock.GetBehavior<BlockBehaviorExOrientable>()?.ApplyOrientation(world, pos, finalChoices[0]);` — call it on `netBlock` (the block at pos), never on `this`, because `this` may be a different variant instance.
- [x] **Step 8.** Both. Original text: Add an XML remark at BlockNetworkNode.cs:70 (TryPlaceBlock) and :675 (GetDrops) naming why those two sites do not move — see U10.3.
- [x] **Step 9.** Green. Original text: Run `./scripts/exmod.sh test 1.21` green, paying attention to the iwex pipe/molten invariant suites.

#### U10.3 — Pin the two sites that must not move, so a later tidy-up cannot silently change drops

**Files**
- Modify: `mods/exlib/src/Blocks/Networks/BlockNetworkNode.cs:671-688 (GetDrops/OnPickBlock XML docs)`, `mods/exlib/src/Blocks/Behaviors/BlockBehaviorExOrientable.cs:242-251 (CanonicalStack XML doc)`
- Test: `mods/exlib/tests/Blocks/Networks/NetworkNodeOrientationTests.cs`, `mods/iiex/tests/Definitions/IiexDefinitionBehaviorTests.cs`
...
- [x] **Step 1.** `iiex:`, not `iwex:` - the merges moved the domain. Asserted for both the drop and the pick. Original text: assert the tuyere's drop is `iwex:furnace-tuyere-s`. IiexDefinitionBehaviorTests.cs:18-27 already pins that the tuyere's orientation table is `[s,n,w,e]`; this states the consequence.
- [x] **Step 2.** And the premise with it: the same test asserts the attached behaviour's `CanonicalStack` really does answer `-n`, so the divergence is live rather than notional. Original text: Add an assertion in the same test that `ExOrientations.Face.Tokens[0]` is `"n"` — so the divergence is written down, not discovered. Replacing site :683 with `CanonicalStack` would silently change the tuyere's drop from `-s` to `-n`, and the same for `lpex:pipe-outlet` (declares `s,n,w,e,u,d`; FaceAll.Tokens[0] == "n"), `lpex:pipe-fluidintake` (overrides GetFallbackOrientation to "s", BlockFluidIntake.cs:39) and `iwex:molten-canal-start` (overrides to "s", BlockMoltenCanalStart.cs:55).
- [x] **Step 3.** Folded into Step 1's test, on the shipped tuyere rather than a probe: the behaviour is attached and the drop is still `-s`. Original text: Add a test proving adding the behaviour is drop-neutral: place a node block whose `BlockBehaviors` includes a `BlockBehaviorExOrientable`, call the block's `GetDrops`/`OnPickBlock`, and assert the behaviour's `CanonicalStack` was not the answer — BlockNetworkNode.cs:675 and :687 override `Block.GetDrops`/`Block.OnPickBlock` without calling base, so the behaviour's overrides never run.
- [x] **Step 4.** Recorded, along with the matching note on `GetDrops`. Original text: Record in the XML doc at BlockNetworkNode.cs:70 that TryPlaceBlock's `:121` rewrite is not an ApplyOrientation call site: `ApplyOrientation` exchanges the block at a position, and at :121 no block has been placed yet — it resolves a code and hands it to `DoPlaceBlock`. The shared part is code resolution, not the swap.
- [x] **Step 5.** Green at 4,635. Original text: Run `./scripts/exmod.sh test 1.21` green.

#### U10.4 — The connector check: a Connector layout mark, a multiblockConnectors attribute, and a completion-time subset test

**Files**
- Create: `mods/exlib/src/Blocks/Structures/MultiblockConnectors.cs`, `mods/exlib/tests/Structures/MultiblockConnectorsTests.cs`
- Modify: `mods/exlib/src/Definitions/MultiblockLayoutBuilder.cs:85-117 (Role/AddLegend siblings), :196-256 (Build), :292-309 (ValidateRoles)`, `mods/exlib/src/Definitions/ExBlockDef.cs:918-932 (MultiblockLayout emits the third sibling)`, `mods/exlib/src/Blocks/Structures/BlockEntityMultiblockStructure.cs:126-152 (SetStructureAngle loads it), :459-482 (IncompleteBlockCount enforces it)`, `mods/exlib/testing/StructureRig.cs:100-181, :244-254 (Missing must agree with the machine)`
- Test: `mods/exlib/tests/Structures/MultiblockConnectorsTests.cs`, `mods/exlib/tests/Structures/StructureRigTests.cs`, `mods/exlib/tests/Definitions/MultiblockFacingsTests.cs`
...
- [x] **Step 1.** `MultiblockConnectorsTests`, on a two-cell probe. Original text: Write the failing test first, on a probe layout, not on a furnace: a 1×3×1 drawing whose middle cell glyph is legended `exlib:probe-node-*` and marked `.Connector('Y', BlockFacing.NORTH)`. Place a node exposing a connector only on `south`; assert the structure never completes and that `Interact`'s missing report names the cell.
- [x] **Step 2.** Both, and the superset case is what lets a re-pick leave a standing structure alone. Original text: Second case in the same file: place a node exposing `north`; assert it completes. Third: a node exposing `ns` (a superset) also completes — this is the subset test the orientation-schemes page argues for (orientation-schemes.md:149-166), and it is what makes the check immune to a legitimate re-pick.
- [x] **Step 3.** Both halves, and the rotation case asserts in both directions - a north node fails at 90 where a west one passes. Original text: Fourth: raise the same structure at angle 90 and assert the demand becomes `west`, derived from `_structureInitAngle`. Fifth: a cell with a connector mark whose occupant is not an `INetworkConnector` at all counts missing (so the mark cannot be satisfied by a plain brick).
- [x] **Step 4.** Added, validated exactly as `Role` is - legend entry and drawn cell, both with their own tests. Original text: Add `MultiblockLayoutBuilder.Connector(char symbol, params BlockFacing[] outward)`, accumulating into a `Dictionary<char, List<string>>` of letters. Validate it in `ValidateRoles`/`Build` exactly as `Role` is: the glyph must have a `Legend` entry (MultiblockLayoutBuilder.cs:294-299) and must be drawn in some `Layer` (:245-250) — an undrawn connector glyph resolves to nothing at runtime, the silent-empty-set failure roles already guard.
- [x] **Step 5.** Third sibling, omitted when nothing is marked, pinned by its own test. Original text: Emit `attributes.multiblockConnectors` from `ExBlockDef.MultiblockLayout` as a third sibling beside `multiblockFacings` and `multiblockRoles` (ExBlockDef.cs:925-930), omitted entirely when nothing is marked — the additive guarantee that let facings and roles ship without a migration.
- [x] **Step 6.** Same total `Coord` read; keyed face-to-cells on the wire and inverted to cell-to-faces on load. Original text: Write `MultiblockConnectors` as a near-copy of `MultiblockCellRoles`: total, type-checked with the same `Coord(JToken?)` pattern (MultiblockCellRoles.cs:119-127), never throwing, because it is re-read on the server monitor tick and on client `GetBlockInfo`.
- [x] **Step 7.** Beside `_roles`. Original text: Load it in `SetStructureAngle` on the line after `_roles` (BlockEntityMultiblockStructure.cs:145), so it is dropped and reloaded with every angle change alongside `_codeByNumber`/`_cellsAccepting`/`_cellsWithRole`.
- [x] **Step 8.** Indexed rather than walked, so the authored offset is read back at the index `InitForUse` rotated it from. `onMissing` now carries a `MissingCell` - a misfaced cell is reported apart from a missing one, because the block is already there and wants turning, not fetching. Original text: In `IncompleteBlockCount` (BlockEntityMultiblockStructure.cs:459-482), after `WildcardUtil.Match(wanted, actual.Code)` succeeds, look the authored offset up (same `_structure.Offsets`/`TransformedOffsets` index alignment `CellsWithRole` uses at :288-302), and for each declared outward letter rotate it by `_structureInitAngle` and require `actual is INetworkConnector c && c.HasConnectorAt(Api.World.BlockAccessor, worldPos, face)`. Otherwise `missing++` and invoke `onMissing`.
- [x] **Step 9.** ⛔⛔ Counting was not enough: the rig also **fills** the footprint, and a plain stand-in can never satisfy a connector cell. Twenty furnace tests went red until `Raise()` placed a node stand-in wearing the demanded faces. Original text: Mirror the same rule in `StructureRig.Missing` and `UnsatisfiedReport` (StructureRig.cs:244-308). If you do not, `Complete()` throws `"0 of N cells unsatisfied"` — the rig counts by code, the machine counts by code and connector, and the two disagree.
- [x] **Step 10.** Ten new cases, gate green at 4,645. Original text: Run `./scripts/exmod.sh test 1.21` green; the exlib suite must gain the five new cases and every existing megablock suite must stay green (no shipped layout marks a connector yet).

#### U10.5 — Migrate the three furnace layouts off the orientation pins onto Connector

**Files**
- Modify: `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockBlastFurnaceCoreCold.cs:81-96`, `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockCupolaFurnaceCore.cs:74-76`, `mods/siex/src/BlockStructures/HotBlastFurnace/Blocks/BlockBlastFurnaceCoreHot.cs:79-84`, `mods/iiex/tests/Fixtures/FurnaceLayoutRig.cs:45-55`, `mods/iiex/tests/Blocks/Furnaces/FurnaceRoleCellsTests.cs:60-82`, `mods/siex/tests/Blocks/HotBlastFurnace/FurnaceGeometryTests.cs:300-315`, `mods/iiex/tests/Fixtures/ColdBlastFurnaceScenes.cs:345-365`, `mods/iiex/tests/Fixtures/CupolaScenes.cs:150-160`, `mods/siex/tests/Fixtures/BlastFurnaceScenes.cs:165-175`, `mods/iiex/tests/goldens/iiex/blocktypes/furnace/blastcore.json`, `mods/iiex/tests/goldens/iiex/blocktypes/furnace/cupolacore.json`, `mods/siex/tests/goldens/siex/blocktypes/blastfurnace/core.json`
- Test: `mods/iiex/tests/Blocks/Furnaces/FurnaceGeometryTests.cs`, `mods/iiex/tests/Blocks/Furnaces/FurnaceOrientationMatrixTests.cs`, `mods/iiex/tests/Scenarios (cold furnace scenarios)`, `test/IronIndustryExpanded.Tests (cupola scenarios)`, `mods/siex/tests/Blocks/HotBlastFurnace`

**Consumes:** MultiblockLayoutBuilder.Connector(char, params BlockFacing[]) from U10.4; IiexBlocks.FurnaceTuyere.Any == "iwex:furnace-tuyere-*" (Generated/IiexBlocks.g.cs:701-702); IiexBlocks.PipePassthrough.Any; PipeTestWorld.MakeTuyere(int id, string orientation) (mods/iiex/tests/Fixtures/PipeTestWorld.cs:101-112)
...
- [x] **Step 1.** `BackwardsTuyere` on `ColdBlastFurnaceScenes` plus a scenario case; green on the pin first, then green on the check. Original text: Write the failing test first, before touching any legend: add a knob to `ColdBlastFurnaceScenes` that fits one tuyere backwards (the `Tuyere(pos, id, orientation)` helper at ColdBlastFurnaceScenes.cs:355 already takes the letter), and a test asserting the furnace never completes. Run it now — it passes today, because the pin catches it. That green run is the baseline the migration must not lose.
- [x] **Step 2.** Done. Original text: In BlockBlastFurnaceCoreCold.cs:94-95, replace `.Legend('Y', IiexBlocks.FurnaceTuyere.WithOrientation("n"))` / `('T', …"s")` with `.Legend('Y', IiexBlocks.FurnaceTuyere.Any)` / `.Legend('T', IiexBlocks.FurnaceTuyere.Any)` plus `.Connector('Y', BlockFacing.NORTH)` and `.Connector('T', BlockFacing.SOUTH)`. Two glyphs on one code is already supported and shares one block number (MultiblockLayoutBuilder.cs:205-214), which is exactly what makes two connector directions on one code representable.
- [x] **Step 3.** ⛔ **There is no passthrough legend, and never was** - the cold furnace's own comment says so at that line: the tuyere sits at the outermost cell, so the player's pipe butts straight against it. Nothing to migrate. Original text: Do the same for the passthrough at :96: `.Legend('P', IiexBlocks.PipePassthrough.Any)` + `.Connector('P', BlockFacing.NORTH, BlockFacing.SOUTH)` — the passthrough must connect both ways through the wall, and the multi-face form is why `Connector` takes `params`.
- [x] **Step 4.** Both. Five pins across three layouts, all tuyeres. Original text: Repeat for BlockCupolaFurnaceCore.cs:76 (one tuyere, `NORTH`) and BlockBlastFurnaceCoreHot.cs:82-83 (`Y`→NORTH, `y`→SOUTH).
- [x] **Step 5.** Collapsed to one `TuyereGlyph`, and the weakening is replaced rather than accepted: `AssertConnectorFaces` reads the authored demand off the drawing and `AssertRotatedConnectorFaces` reads it back through the placed machine. Original text: Update FurnaceLayoutRig.cs:54-55: `NorthTuyereGlyph`/`SouthTuyereGlyph` collapse to one `TuyereGlyph = "iwex:furnace-tuyere-*"`. This is a real weakening of the `AssertRoleGlyphs` several-codes oracle (FurnaceLayoutRig.cs:690-719), whose second half asserts each listed glyph is used by some cell. Replace it with a connector-face oracle read off `multiblockConnectors` — 'the cell marked Tuyere in the north wall demands an outward `n` connector' — or the migration trades a strong assertion for a weak one.
- [x] **Step 6.** Both, off demanded faces rather than per-code counts. ⛔ smex's oracle was worse than stale: asking `CellsAccepting` with a concrete letter against a wildcard cell now matches **both** inlets, so the old assertion would have compared four cells against two. Original text: Update FurnaceRoleCellsTests.cs:70-82's `Tuyere(wall, side)` helper and smex FurnaceGeometryTests.cs:305-315 the same way: they currently rotate the authored letter through `ExOrientation.RotateOrientationToken` because the code was pinned; with a wildcard code the rotation moves to the connector face instead.
- [x] **Step 7.** Confirmed: `multiblockFacings` now holds only the player-oriented codes on all three, and the rest of the diff is the block-number collapse from two glyphs onto one code. Original text: Re-bless the three core goldens: `EXLIB_WRITE_GOLDENS=iwex/blocktypes/furnace/blastcore,iwex/blocktypes/furnace/cupolacore,smex/blocktypes/blastfurnace/core`. Diff and confirm `multiblockFacings` keeps exactly the player-oriented codes (`furnace-blastcore-*-n`, `furnace-irontap-w`, `furnace-slagtap-e`, `hopper-tall-e`) and nothing else.
- [x] **Step 8.** Still red for the furnace, green for the test - now by the connector check. Original text: Re-run the backwards-tuyere test from step 1 and confirm it still fails to complete — now because of the connector check rather than the pin. If it passes (i.e. the furnace completes), the check is not wired into the path the scenes use.
- [x] **Step 9.** Green at 4,646. Original text: Run `./scripts/exmod.sh test 1.21` across all three suites — smex and lpex fixtures reach into iwex's rig through the test-project chain, so a rig constant change breaks three suites at once.

#### U10.6 — Make a pinned self-orienting node unrepresentable — a builder guard plus a cross-mod def check

**Files**
- Create: `mods/exlib/testing/PinnedNetworkNodes.cs`
- Modify: `mods/exlib/src/Definitions/MultiblockLayoutBuilder.cs:94-117 (AddLegend), :119-154 (FindOrientationSegments)`, `mods/exlib/tests/Definitions/MultiblockFacingsTests.cs`, `mods/iiex/tests/Definitions/IiexDefinitionBehaviorTests.cs`, `mods/iiex/tests/Definitions`, `mods/siex/tests/Definitions`, `mods/siex/tests/Definitions`
- Test: `mods/exlib/tests/Definitions/MultiblockFacingsTests.cs`, `mods/exlib/tests/Definitions/StructureLayoutTests.cs`
...
- [x] **Step 1.** Confirmed, and the split is as described. Original text: Understand the limit before writing anything: `MultiblockLayoutBuilder` holds only a string (Legend at :48, AddLegend at :94), and `Build()` runs inside a def's static `Definitions(domain)` factory, so the block does not exist and cross-mod defs may not be registered yet. It cannot ask a behaviour anything. The guard therefore splits in two.
- [x] **Step 2.** Refused in `AddLegend`, matched against the tokens the declared schemes spell rather than against a letter set - so `north` and an ordinary segment that happens to be direction letters are not caught. ★ This also closes the bend/tee/cross hole from the other end: `MultiblockFacings` never recognised those tokens, and now nothing can author one. Original text: Builder half, which is exact: a multi-letter orientation token (`ns`, `we`, `ud`, `nswe`, `uns`, …) is spelled only by a network node — no player-oriented block spells one — so `AddLegend` can refuse it outright. Add the check where `FindOrientationSegments` already runs (:110), throwing when any recorded segment is longer than one letter and `oriented` is true. `LegendAnyFacing` (:56) stays lax and is the documented opt-out.
- [x] **Step 3.** Five token shapes plus the `LegendAnyFacing` companion and a side-word control. Original text: Failing test: a probe layout calling `.Legend('p', "iwex:pipe-plated-straight-ns")` must throw with a message naming the glyph, the code and the reason ("a network node takes its orientation from its neighbours; mark the cell with Connector instead"). Add a companion test that `.LegendAnyFacing('p', "iwex:pipe-plated-straight-ns")` still succeeds.
- [x] **Step 4.** ⛔⛔ Not through `AnyProvides`: it answers "does some def provide this", and resolving **which** def by `def.Code` names every furnace part at once - the first attempt reported fifteen false positives including the cores themselves. Matched against each def's expanded code shape (base plus one segment per variant group's states) instead. Original text: Cross-mod half, for the single-letter cases the builder cannot tell apart (`iwex:furnace-tuyere-n` looks exactly like `iwex:hopper-tall-e`): write `PinnedNetworkNodes.Violations` modelled on `MultiblockCodes.Unresolvable`. For every def in the sources, read `attributes.multiblockFacings`; for each key, find the def that provides that code (reuse `MultiblockCodes.AnyProvides`'s whole-segment matching, MultiblockCodes.cs:122-138) and fail when that def's `behaviors` array declares `ExOrientable` with `mode == "network"`.
- [x] **Step 5.** iiex's own suite, plus the cross-mod suite that holds all three registries - siex's hot furnace pins an iiex code, so only the latter can judge it. Both assert a non-zero examined count. Original text: Wire `PinnedNetworkNodes` into each mod's definition test with the same `Assert.True(codesChecked > 0)` pattern — a checker that examines nothing is exactly the failure `MultiblockCodes` documents at :42-47.
- [x] **Step 6.** Nothing tripped, and a mutation confirmed it would have: re-pinning the cupola's tuyere fails the check by name. Original text: Run `./scripts/exmod.sh test 1.21`. Nothing must trip: U10.5 removed the last three pins. If anything trips, U10.5 is incomplete — do not relax the guard.
- [x] **Step 7.** Observed. Original text: Caution: Do not attempt this task before U10.5. Three shipped layouts pin network-node orientations today (visible in blastcore.json's `multiblockFacings`: `iwex:furnace-tuyere-n`, `-s`, `iwex:pipe-passthrough-*-ns`), so a guard added first fails the build of iwex, lpex, hpex and smex simultaneously.

#### U10.7 — Delete the pending markers, and correct the over-claims the pins left behind

**Files**
- Modify: `mods/exlib/src/Blocks/Behaviors/BlockBehaviorExOrientable.cs:33-38, :47-64, :127-138`, `mods/exlib/src/Helpers/ExOrientations.cs:110-126`, `mods/iiex/src/BlockStructures/Furnaces/Blocks/BlockBlastFurnaceCoreCold.cs:81-93`, `mods/iiex/tests/Fixtures/FurnaceLayoutRig.cs:45-55`, `docs/design/mechanics/orientation-schemes.md:85-168`, `docs/design/mechanics/multiblock.md:131-168`, `WORKLOG.md`
...
- [x] **Step 1.** ⛔ U0 had already cleaned most of these; what remained was one sentence - "nothing enforces that yet" - now replaced by the two guards that do. `omni` is still declared by no block and is noted as such. Original text: Replace BlockBehaviorExOrientable.cs:33-38 (the "Only horizontal is in production use" pending note): `network` is now declared by every BlockNetworkNode def. Keep an accurate note that `omni` is still declared by no block but is exercised by ExOrientableTests.cs:145-170, so deleting the branch breaks tests.
- [x] **Step 2.** Stated on `ExOrientations`' own class doc, where the pinning claim lived. Original text: Replace :47-64 (the "NOT BUILT, and this is the dangerous one" pending paragraph) with the built statement and where it is enforced: the builder's multi-letter refusal plus `PinnedNetworkNodes` per mod, and the `Connector` mark as the sanctioned way to state what the layout actually wants.
- [x] **Step 3.** Recorded on `ApplyOrientation`: two of four sites call it, and why the other two cannot and must not. Original text: Replace :127-138's "intended, not yet wired" pending note on `ApplyOrientation` with the truth from U10.2/U10.3: two of `BlockNetworkNode`'s four rewrite sites now call it; `TryPlaceBlock`'s cannot (no block at the position yet) and `GetDrops`'s must not (the per-type fallback is not the scheme's first token).
- [x] **Step 4.** Both rewritten in U10.5 as part of the migration - neither claims a backwards tuyere is unbuildable any more, because the connector check is what catches it. Original text: Correct the over-claim at BlockBlastFurnaceCoreCold.cs:88-93 and its copy at FurnaceLayoutRig.cs:49-53. Both assert that a backwards tuyere is "unbuildable" because a pipe neighbour makes that face required and the node is exchanged onto it. Traced: `ComputeValidOrientations` uses the `connectsAny` relaxation for single-axis shapes (BlockNetworkNode.cs:247-256) and the solid-brick branch re-adds the current letter as required (:228-240), so a tuyere already facing `s` stays inside `finalChoices` and `RecalculateAndSyncOrientations` never exchanges it (:841-844). The layout was the only thing catching it; now the connector check is.
- [x] **Step 5.** "What exists today" names the built registry and the builder's refusal; the "Require connector faces out" row is marked CHOSEN and BUILT, with how it is built; "Pin the cardinal" is marked rejected and refused. Original text: Update orientation-schemes.md:85-97 ("What exists today"): the declared-scheme registry the page proposes at :172-193 is built (`mods/exlib/src/Helpers/ExOrientations.cs`, twelve schemes + the two-step `Rotate`), and step 2 of its cost table — pointing ~18 blocks' `VariantGroup` at it — is what U10.1 effectively completed via the `scheme` property. Mark the "Require connector faces out" row of the :149-156 table as the chosen and built form.
- [x] **Step 6.** `FindSideSegment` corrected to `FindOrientationSegments` and "the last such segment wins" to "every rotating segment is recorded"; a Connector marks section added, plus the sibling in the emission list and two rows in the build-time guard table. Original text: Update multiblock.md:141-145: it still names `FindSideSegment` and says "the last such segment wins"; the code is `MultiblockLayoutBuilder.FindOrientationSegments` (:144-154) and it records all rotating segments. Add the `multiblockConnectors` sibling to the emission list at :561.
- [x] **Step 7.** `docs/internal/worklog/2026-08.md`, and NEXT.md with it. Nothing committed. Original text: Append a WORKLOG.md entry describing what landed and why the plan's task order was inverted. Do not commit — the user owns git history.
...
> `infra/tools/coverage_gate.py`). **Only one shape exists** — `networks/molten/molten-megablock-laddle.json`
...
*Appended 2026-08-06 from a user handoff. The art is **drawn** (`workbench/shapes/molten-megablock-laddle.json`,
`workbench/shapes/molten-block-laddlecore.json`) and the **layout is specified by the user**, reproduced
...
`mods/siex/src/BlockStructures/Ladle/`; both are stale against the ruling five lines below them, and
U11.7 fixes them.

**Entry condition.** Everything this unit stands on exists today: the molten canal family (`BlockMoltenCanal`,
`BlockMoltenCanalStart`, `BlockMoltenCanalTap` in `mods/iiex/src/BlockNetworkMolten/`), `MoltenCharge`,
...
**Shared files** (collision risk): `mods/exlib/src/Blocks/Structures/StructureFootprint.cs` +
`mods/exlib/src/Definitions/ExBlockDef.cs` — U11.1 widens `FillerCellSpec`; no other unit touches either ·
`mods/exlib/src/Definitions/VanillaCodes.cs` — U11.4 adds one rung (`AnyBricksOrAir`); U10.5 reads the file but
edits no constant · `mods/iiex/src/IiexRecipeConfig.cs` — U11.6 appends the `ladle-grid` row alongside
U5.6/U6/U8.8/U9.4 · `mods/iiex/assets/iiex/lang/{en,ru,uk}.json` — append-only, one contiguous block ·
`infra/tools/convert-shape.py` — U11.2 appends TEXTURES entries only (all five keys the two shapes use are already
mapped, verified 2026-08-06 via `--check`) · `mods/iiex/src/Generated/IiexBlocks.g.cs` + iwex goldens —
regenerate last.
...
- Modify: `mods/exlib/src/Blocks/Structures/StructureFootprint.cs` (`FillerCellSpec`),
  `mods/exlib/src/Definitions/ExBlockDef.cs` (`SerializeFillerCells`),
  `mods/exlib/src/Blocks/Structures/FillerLayoutBuilder.cs` (a glyph→boxes registration)
- Test: `mods/exlib/tests/Blocks/Structures/` (beside the existing footprint tests)
...
**Files**
- Create: `mods/iiex/assets/iiex/shapes/molten/laddle.json`, `mods/iiex/assets/iiex/shapes/molten/laddlecore.json`
- Modify: `infra/tools/convert-shape.py` only if a texture key turns out unmapped

**Produces:** two runtime shapes, animations intact

- [ ] **Step 1.** `python infra/tools/convert-shape.py molten-megablock-laddle mods/iiex/assets/iiex/shapes/molten/laddle.json
      molten-block-laddlecore mods/iiex/assets/iiex/shapes/molten/laddlecore.json`. **Mandate the script** — a
...
**Files**
- Create: `mods/iiex/src/BlockStructures/Ladle/Blocks/BlockLadleCore.cs`
- Test: `mods/iiex/tests/Blocks/Ladle/LadleLayoutTests.cs`
...
- Modify: `BlockLadleCore.cs` (the `.MultiblockLayout(...)` block),
  `mods/exlib/src/Definitions/VanillaCodes.cs` (one new rung)
...
**Files**
- Create: `mods/iiex/src/BlockStructures/Ladle/Blocks/BlockLadle.cs`,
  `.../Ladle/BlockEntities/BlockEntityLadle.cs`
- Test: `mods/iiex/tests/Blocks/Ladle/LadleFootprintTests.cs`
...
**Files**
- Modify: `mods/iiex/src/Recipes/Grid/MoltenRecipeDefinitions.cs`,
  `mods/iiex/src/IiexRecipeConfig.cs`, `mods/iiex/assets/iiex/lang/{en,ru,uk}.json`, `mods/iiex/docs/handbook/`
...
- [ ] **Step 1.** Fix `ladle.md`'s front-matter **Mod smex** → iwex and § *Code*'s
      `mods/siex/src/BlockStructures/Ladle/` paths, both stale against the ruling on the same page.
