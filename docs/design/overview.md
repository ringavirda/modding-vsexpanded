...
- **You buy operating efficiency with build complexity.** This is the engine of the whole suite, and
  everything else is downstream of it. **Complexity goes into the *build*; efficiency comes out of the
  *operation*.** You come all the way from casting iron into sand, through converters, to a full rolling
  hall with steam hammers and stamping — and what that investment returns is *speed at the lever*: a
  **3000 u cast slab is eight feeds and five stamps for fifteen plates**, where hand-forging fifteen plates
  is an afternoon even with a helve. **So yes, we optimise hard for fewer clicks — on the operating side
  only.** Never trade it the wrong way: do not simplify the factory to save the player a build, and do not
  add clicks to a process to make it feel weightier.
- **Operating friction is the currency the tech tree is denominated in.** Every upgrade buys down a
  *specific, named* friction: hand forging → the mill; the carry-back walk → the train; crop-and-head each →
  the cutter bench; trips to the reheat furnace → a bigger power plant. So early friction must be **real and
  felt** (or there is nothing to sell) and **removable by investment** (or there is no reward). Historicity
  and the hands-on feel of running real machinery are what make that purchase legible — and this is
  **confirmed by players**: the published `main` branch is far smaller than what is being built here and its
  machines are expensive, yet what people praise is precisely the feel of **running an actual factory**
  against vanilla ironmaking's *light it → sleep → collect*.
- **Cheap kickstart — and it answers a real complaint.** The mod's biggest reported problem was the
  **enormous item count needed to construct everything**: smex assumes plenty of iron and lpex is a bigger
  iron sink still, so there was a mountain to climb before seeing any result. The answer is **cast iron** —
  instead of hammering out dozens of plates you cast a few frames and `castplate`s, and those make up most of
  a machine — plus **iron throughput**: the blast furnace is ~2× a bloomery, and ore roasting on top.
  **Note what did *not* change: the machines still cost a ton of iron.** Nothing was made materially
  cheaper; the iron simply became faster to produce and the parts faster to make. Steel is never required to
  *start*; it is the reward for progressing.
...
- **Nothing is hidden** (R7). Every machine's state is legible to a player standing in front of it —
  ...
  verb-based: you work the machine, not a menu. Machines carry status UI, two stations carry full windows
  (design table, boring machine) and more may follow. What is forbidden is *state you cannot see*.
