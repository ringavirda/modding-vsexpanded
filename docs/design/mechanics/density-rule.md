...
**Mod** cross-cutting (an authoring convention). The only code that applies it today is iwex.
...
* an ingot is 100 u - the mold fill quantity, and the scale the mod already uses (`MoldDefaultUnits` = 100,
  `src/IronworkingExpanded/IwexConfig.cs:138`; vanilla's tool molds are on the same scale at
...
* Children are drawn in the parent's local frame. A child that is offset out of the parent is
  additional solid (`assets/iwex/shapes/item/castbillet.json`: `CastBillet1` at `z 0…12` with child
...
|---|---|---|---|
| units per voxel³ | 2.5 | `src/IronworkingExpanded/Items/PigBreaking.cs:36-37` - the only place in the codebase that expresses it, and even there it is derived (`ItemPig.PigUnits / PigVoxels`) rather than declared | no shared constant exists |
...
|---|---|---|---|---|---|---|
| `pig` | `Items/ItemPig.cs:39` | 375 | `assets/iwex/shapes/pig.json` (5×2×12 + 3×1×12) | 156 | 390 → settled 375 | matches the settled mass |
...
|---|---|---|
| `PigBreaking.UnitsPerVoxel` | `src/IronworkingExpanded/Items/PigBreaking.cs:36-37` | `ItemPig.PigUnits / (float)PigVoxels` = 2.5. The only expression of the rule in code |
...
| `AnvilPigBreakingPatches` | `src/IronworkingExpanded/Patches/` | Pays the shed voxels out as chunks and bits |
...
| Tests | `test/IronworkingExpanded.Tests/Items/PigBreakingTests.cs` | Asserts the 2.5 u/voxel payout and end-to-end conservation |
