# Boring Machine
**Status** designed - art drawn, nothing built (no block, no block entity, no def, no recipe, no lang key, no test)   **Mod** lpex (`LowPressureExpanded`)
...
2. The cast pipe tier has no other route. Cast pipe is assembled from cast pipe-parts finished on the boring machine ([conventions.md § pipe tiers](../conventions.md), [pipe network](../mechanics/pipe-network.md)), so with the machine unbuilt, lpex's own pipe tier has no survival source.
...
Placement: a machine lives with the content it feeds ([STATE.md](../../internal/plans/STATE.md)). The cylinder, the cast pipe-parts and the machine gears are all steam-tier parts, so the machine is lpex's; the cast blanks it eats are iwex's. iwex casts rough, lpex machines to spec.
...
| runtime shape | missing - `assets/lpex/shapes/` holds only `boiler/`, `engine/` and `pipes/` (16 files); nothing is copied out of `editable/` |
| textures | see the path table below - not shippable as authored |
| lang | `assets/lpex/lang/en.json` carries no `boringmachine-*` key |
| handbook | `docs/lpex/handbook/` has 5 pages (`00-steampower` … `04-startersetup`); none is this machine. The sync pipeline joins on the `NN-` prefix and drift fails a test |
| schematic textures | `diag-item-cylinder.png`, `diag-item-gear12.png`, `diag-item-gear24.png`, `diag-item-gearbevel.png`, `diag-item-rollers*.png` are drawn in `assets/editable/textures/` but not shipped - `assets/iwex/textures/item/diagram/` holds 22 files and none of them is a cutting schematic |

Texture paths - the same bug the design table had.

| Key | Authored value | Verdict |
|---|---|---|
| `cast-iron1` | `F:/repos/modding-vsexpanded/assets/editable/textures/cast-iron1` | absolute local path, identical to the `paper` bug fixed on the [design table](design-table.md). Ships as `iwex:block/metal/castiron` (`assets/iwex/textures/block/metal/castiron.png`) |
| `iron5` | `block/metal/sheet-plain/iron5` | undomained → resolves to `lpex:block/…`, which does not exist. Needs `game:` |
...
The fixed design table is the worked example of what the runtime copy must look like - every key domained (`assets/iwex/shapes/crafting/designtable.json`).
...
| heavy cast plate | yes `iwex:castplate-heavy` (`CastPartItemDefinitions.cs:31-42`) |
| `gear-iron` | yes but in lpex, not iwex - see [gears](gears.md) |
...
A cost key `boringmachine-grid` (and `-rcc` if it is built as a RightClickConstructable) belongs in `LpexRecipeConfig.Defaults()` (`:59-86`) so it rescales with `LpexConfig.RecipeLevel` (`:224`). The catalogue only understands `Type = "rcc"` and `Type = "grid"` (`LpexRecipeConfig.cs:40-54`).
...
| cost key | `boringmachine-grid` | `LpexRecipeConfig.cs:59-86` | rescales with `RecipeLevel` (`LpexConfig.cs:224`) |
...
The exlib station-window base does not exist. [diagram crafting](../mechanics/diagram-crafting.md) plans one (slots + list + info panel + guide viewer, parameterised); today `GuiDialogDesignTable` lives in iwex and enumerates `capi.World.Items` directly (`:53-56`). So "reuses the design table's station-window infra" means copy an iwex class into lpex, or promote it to exlib first. Promoting it is the right call and a prerequisite: lpex depends on iwex, so a cross-mod `using IronworkingExpanded.…` would work but would put an iwex GUI class in lpex's dependency surface.
...
- This machine gates the cast pipe tier. Until it exists, `lpex:pipe-*` has no cast-pipe-part route and the tier is reachable only through the recipes that currently accept the plain plated iwex segment (`MachineRecipeDefinitions.cs`'s `StraightPipe`, which is deliberately `iwex:pipe-straight-*`). The dead cost key `pipe-straight-grid` (`LpexRecipeConfig.cs:76`) costs a grid recipe that does not exist ([STATE.md](../../internal/plans/STATE.md)).
...
- The schematic item family is unspecified. Four cutting-schematic textures are drawn; no item, no variant list, no owner mod, no craft. Are they `diagram-{type}` variants (lpex-contributed, per [diagram crafting](../mechanics/diagram-crafting.md)) or a separate `schematic-{type}` item? The diagram catalogue's picker matches on `FirstCodePart() == "diagram"` (`GuiDialogDesignTable.cs:69-73`), so a separate family would be invisible to the design table.
