...
- This machine gates the cast pipe tier. Until it exists, `lpex:pipe-*` has no cast-pipe-part route and the tier is reachable only through the recipes that currently accept the plain plated iwex segment (`MachineRecipeDefinitions.cs`'s `StraightPipe`, which is deliberately `iwex:pipe-plated-straight-*`). The dead cost key `pipe-straight-grid` (`LpexRecipeConfig.cs:72`) costs a grid recipe that does not exist ([STATE.md](../../internal/plans/STATE.md)).
