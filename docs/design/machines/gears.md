# Gears
**Status** live - both items and all three smithing recipes ship   **Mod** lpex (`LowPressureExpanded`)

**Owns**
* the gear item family - `lpex:gear-{metal}` and `lpex:largegear-{metal}`: codes, the variant axis, shapes, transforms, stack sizes, density, creative tabs, lang keys;
* their only production route - the three anvil smithing plans and their voxel patterns;
* the domain-ownership fact: `gear-iron` is defined in lpex and no mod upstream of lpex references it - iwex ships its own `iwex:spurgear` for its RCC stages ([flywheel & shafting](flywheel-and-shafting.md));
...
[flywheel & shafting](flywheel-and-shafting.md) owns the transmission block, its RCC stage table and the `iwex:spurgear` / `iwex:bevelgear` items ·
...
[overview.md](../overview.md) · [STATE.md](../../plans/STATE.md)
...
|---|---|---|
| `gear` (small) | the ordinary machine gear - a line item in a bill of materials | 10 grid recipes across 4 mods (iwex's RCC stages take `iwex:spurgear`, not this family) |
| `largegear` | a heavy drive gear | one consumer: the Bessemer vessel's spawn gate |

> Three other "gear" concepts exist and are not this page's. `iwex:bevelgear` is a cast-iron block component consumed to turn a shaft into a bevel junction, and `iwex:spurgear` (60 u, `SpurGearItemDefinitions.cs`) is the iron tier's own machine gear, consumed by iwex's RCC stages - [flywheel & shafting](flywheel-and-shafting.md) owns both. `gearpinion`, the block-scale heavy cast drive gear the design names, exists nowhere in code. The distinction: a gear-iron is a part in a bill of materials, a pinion is a block-scale drive component.
...
| | `gear` | `largegear` |
|---|---|---|
| code | `lpex:gear-{metal}` | `lpex:largegear-{metal}` |
...
| creative tabs | `general`, `items`, `lpex` - `:23-25` | same |
| `materialUnits` | absent | absent |

Both variants share one surface through the private `Common` helper (`:19-25`); only the shape, the stack size, the extra texture key and the three model transforms differ. Golden parity files: `test/LowPressureExpanded.Tests/goldens/lpex/itemtypes/gear.json` and `…/largegear.json`.

---

## Assets

No new art: both gears reuse vanilla shapes retextured off the ingot texture, the same route `iwex:spurgear` takes.

| Asset | State |
|---|---|
| shapes | vanilla, referenced not copied (`GearDefinitions.cs:29`, `:61`) - nothing under `assets/lpex/shapes/` |
...
| lang | `assets/lpex/lang/en.json:4-7` - all four codes named; `ru.json:4-7` and `uk.json:4-7` complete |
| handbook | none - `docs/lpex/handbook/` has 5 pages and no gear entry. The sync pipeline joins on the `NN-` prefix, so adding one is a page and a lang block |
...
|---|---|---|---|---|
| `2gears-{metal}` | one 5-row voxel plan | `game:ingot-*`, variants `iron`/`steel` | 2 × `lpex:gear-{metal}` | `GearRecipeDefinitions.cs:38-49` |
| `4gears-{metal}` | the same plan stacked twice (10 rows) | same | 4 × `lpex:gear-{metal}` | `:50-59` |
| `largegear-{metal}` | a single 10 × 10 ring plan | same | 1 × `lpex:largegear-{metal}` | `:60-86` |
...
Smithing is outside the recipe-cost framework. `LpexRecipeConfig` only constructs `Type = "rcc"` and `Type = "grid"` entries (`:40-54`), and its catalogue (`:59-86`) has no gear key, so `/exmod recipes lpex cheap` rescales every machine bill but not the gears themselves.
...
Contract: the caller passes the literal code. There is no metal capture - unlike `Plate` (`:29`), `Nails` (`:37`) and `Rod` (`:45`), which all call `.Metal()`. So `lpex:gear-*` is an uncaptured wildcard: iron and steel gears are interchangeable in every recipe and neither keys the output's metal. The house pattern is a `foreach` over both codes (`MachineRecipeDefinitions.cs:29-34`, `HighPressureExpanded/.../MachineRecipeDefinitions.cs:23-24`) or two explicit `.Grid(…)` calls (`ConverterRecipeDefinitions.cs:31`, `:43`).

### Consumer census

Every gear requirement in `src/`. This census is this page's; the per-machine stage tables are not - follow the links.

Grid recipes - each row exists twice, once for `game:gear-rusty` and once for `lpex:gear-*`, unless noted:

| Mod | Recipe | Gears | file:line |
|---|---|---|---|
| lpex | Watt Engine | 2 - declared but never placed | `MachineRecipeDefinitions.cs:78` |
| lpex | Fluid Pump | 1 | `:89` |
| lpex | Manual Fluid Pump | 2 | `:101` |
| lpex | Mechanical Power Generator | 2 | `:115` |
| lpex | Piping (Valve) | 2 | `PipeRecipeDefinitions.cs:59` (rusty), `:79` (lpex) |
| lpex | Piping (Pressure Valve) | 2 | `:69` (rusty), `:89` (lpex) |
...
|---|---|---|---|---|
| iwex | transmission, stage 2 | `iwex:spurgear` × 2 - iwex's own gear, not this family | `BlockTransmission.cs:72` | [flywheel & shafting](flywheel-and-shafting.md) |
| smex | Bessemer vessel spawn | `lpex:largegear-iron` or `-steel` × `BessemerRequiredGears` | `BlockEntityConverterControl.Peripherals.cs:218-219` | [bessemer](bessemer.md) |
...
| iwex transmission broken | the RCC scatters its construction materials - the block itself is `NoDrops()`; [flywheel & shafting](flywheel-and-shafting.md) owns the salvage rule |
...
|---|---|---|
| `GearDefinitions : IExItemDefProvider` | `src/LowPressureExpanded/Items/GearDefinitions.cs:12` | a never-instantiated stand-alone provider - both gears use the vanilla `Item` class, so there is no mod class to hang the defs on (`:6-11`) |
...
| `GearRecipeDefinitions : IExRecipeDefProvider` | `src/LowPressureExpanded/Recipes/Smithing/GearRecipeDefinitions.cs:14` | the three plans |
...
- `largegear` carries a dead `rusty-iron` texture key. `Common` sets `rusty-iron` for both items (`GearDefinitions.cs:22`), and `LargeGear` adds `gold` (`:62`) because the `gear24` shape uses that key. The shipped golden has both (`goldens/lpex/itemtypes/largegear.json`). One of the two must be unused on the large gear; the explicit `.Texture("gold", …)` override is strong evidence it is `rusty-iron`. (Not verified against the vanilla shape - `D:/Gaming/Others/Vintagestory` is not readable from this checkout.)
- The creative tab is still named "Pipes & Power". `assets/lpex/lang/en.json:2` maps `game:tabname-lpex` to "Pipes & Power" (ru: "Трубы и приводы", uk likewise), and `GearDefinitions.cs:25` puts both gears in that tab. The mod's display name is Low Pressure Expanded (`modinfo.json`).
...
- Steel gears have no distinct function. `metal` ∈ {iron, steel} on both items, but every consumer takes `lpex:gear-*` uncaptured, so a steel gear costs a steel ingot and buys nothing. The only place metal is mentioned is the Bessemer gate, which accepts either (`BlockEntityConverterControl.cs:1056-1057`).
...
- Two near-identical small gears coexist. `iwex:spurgear` serves iwex's constructions; `lpex:gear-*` serves the steam-tier grid recipes. Whether they merge, and in which mod, is open; a merge breaks every `lpex:gear-*` recipe and every save holding one, so it needs a block/item migrator ([recipes & config](../mechanics/recipes-config.md)).
...
- `gearpinion` still has no item. The design's split is bill-of-materials gear (covered by `gear-iron` and `iwex:spurgear`) vs block-scale drive gear (`gearpinion`); the second half exists nowhere.
- No handbook page. Both a page under `docs/lpex/handbook/` and its `NN-`-prefixed lang block are missing; the handbook-sync test fails on drift, so this is a two-file change.
