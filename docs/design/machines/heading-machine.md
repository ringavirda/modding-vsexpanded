...
* the die catalogue and who ships each entry (nail → iiex, bolt → iiex, rivet → iiex, ball → hpex), and the reason it is a heading machine rather than a rivet machine;
...
**Depends on**
[mp-energy](https://github.com/ringavirda/exlib/blob/main/docs/design/mechanics/mp-energy.md) (the run it loads; a heading blow is a hammer blow, i.e. a pulsed load) ·
...
|---|---|---|---|
| nail | `nailplate` | `game:metalnailsandstrips` | iiex - on the [nail machine](nail-machine.md), a different mechanism |
| **bolt** | rod @ 25 u | bolts | iiex - this bench |
| rivet | rod @ 25 u | rivets | iiex |
...
| spur gear | `iiex:spurgear` | ships, with two grid routes (`EnergyRecipeDefinitions.cs:39-59`); the transmission's RCC already consumes it (`BlockTransmission.cs:72`) |
...
Cost key `headingmachine-grid` in `IiexRecipeConfig.DefaultCatalogue` (`IiexRecipeConfig.cs:47-70`).

Dies are crafted separately and are the extension point: many dies, one machine. A die is an ordinary item with an attribute, so iiex adds the rivet die and hpex the ball die without touching this block.
...
Hard-coded elsewhere, and relevant: network tick 1000 ms (`BlockNetworkModSystem.cs:42-45`) · `MpMaxSpeed` 2.0 (`ExlibConfig.cs:98`) · `ShaftInertia` 0.5 (`IiexConfig.cs:477`) · transmission ratios `x2/x4/clutch` (`BlockEntityTransmission.cs:59-64`).
...
Where a caller hooks in. To add a fastener: ship one die item whose `die` attribute names `Bench`, `Accepts`, `Output` and `Count`. No block change, no dependency on iiex beyond the attribute shape - the same contract `MoldSpec` gives casting patterns (`MoldSpec.cs:18-25`). To add a bench: derive from the shared BE base and give it a different `Bench` string.

---

## Gotchas

- **Do not name the block "rivet machine".** The rivet die is iiex's; a block whose only shipped product came from another mod is the exact failure the placement rule forbids.
...
- **`rolledrod` and `rivetrod` do not exist either** (build item 8), and the `grooved` set cannot reach them: its first gap is 1.0 against 3.0 stock (`RollSetItemDefinitions.cs:89`) - a 2.0 draft against `δ_max = μ²R = 0.5² × 4 = 1.0` (`RollingPass.cs:43-44`, `IiexConfig.cs:506`). That is blocker B4, and it sits directly upstream of this bench.
...
- **Where the `ItemDie` type should live.** In iiex next to `RollSetSpec` is the obvious home, but iiex and hpex both ship dies - so either they take an iiex reference (the chain allows it: `exlib ← iiex ← iiex ← smex`) or the spec moves to exlib beside the other cross-mod contracts. `MoldSpec` set the precedent by staying in iiex (`MoldSpec.cs:6`); no one has re-examined it since iiex gained a die.
...
- **The rivet die has no home to be written into** - iiex's forming pages ([steam-hammer](steam-hammer.md), [wide-hall](wide-hall.md)) do not yet claim it.
