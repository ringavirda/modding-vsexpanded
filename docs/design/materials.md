...
| what is live vs designed vs blocked | [STATE.md](STATE.md) |
...
windows are also that page's; that carbon is set by *process* while the metallic elements are added by
held proportion is [machines/ladle.md](machines/ladle.md)'s mechanic, along with off-spec → waste alloy
and the chill model.
...
- **Steel slab/bloom/billet = cast.** Only molten steel (Bessemer / open-hearth) can be poured into a form, via
  ...
  rolling stock (see [stock](items/stock.md)).
