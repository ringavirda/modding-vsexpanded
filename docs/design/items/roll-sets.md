...
**Status** live as an item, inert as a mechanism. Four `iwex:rollset-*` items ship - variant-emitted,
...
supposed to gate on is never read. The other eight settled sets (six lpex, two smex) exist only on paper.
**Mod** iwex ships the itemtype and all four shipped sets. lpex and smex are settled to ship their own -
...
* the roll-set family census - every set that exists in code or is settled on paper, across iwex / lpex /
  smex, in one table, with the page that owns each one's numbers;
* the `iwex:rollset` item definition as an item - one itemtype, the `type` variant group, the placeholder
...
* the cross-mod emit question - how lpex and smex ship sets without touching iwex's itemtype, and what
...
| 4 | The machine's gate is domain-blind - it tests only that the attribute exists, never the item's domain, which is what makes an `lpex:` set legal at an `iwex:` mill with no new code | `BlockRollingMill.cs:294` · `BlockEntitySandCastingCell.cs:188` (`FirstCodePart() == "pattern"`) |
...
2. a `*Validation.Validate(IEnumerable<CollectibleObject>)` static, called from the mod system's
   `AssetsFinalize` (iwex's is `IronworkingExpandedModSystem.cs:113-116`);
...
`test/IronworkingExpanded.Tests/goldens/iwex/itemtypes/rollset.json`), so the
...
|---|---|---|---|---|
| `flatwide` 2.5 | lpex | 2.5 | first stand a `shingledslab` enters | [wide hall](../machines/wide-hall.md) |
| `flatwide` 2.0 / 1.5 / 1.0 | lpex | one each | the rest of the four-stand train | " |
...
`iwex:rolledplate-iron`, `iwex:rolledsheet-iron`, `iwex:wirerod-iron`, `iwex:nailrod-iron` do not exist
...
|---|---|---|---|
| `assets/editable/shapes/item-rollers-flat.json` | four 4-voxel barrel segments named `20Gap`/`15Gap`/`10Gap`/`05Gap`, up-and-down roll pairs | iwex `flat` | drawn to the shipped schedule, not the settled one |
| `item-rollers-grooved.json` | four 4-voxel segments named `25Gap`/`20Gap`/`15Gap`/`10Gap` | iwex/smex `grooved` | drawn to the settled schedule |
| `item-rollers-flatwide20.json` · `…15` · `…10` | one 16-long roll pair each | lpex 2.0 / 1.5 / 1.0 | gap measurements are [wide hall](../machines/wide-hall.md)'s |
...
| — | — | lpex 2.5, smex 3.0 / 3.5 | must be drawn |
...
| runtime shapes | none. `assets/iwex/shapes/forming/` holds `rollingmill.json` and the ten generated `stock-*.json` only |
| handbook | none for the roll set in `docs/iwex/handbook/` |

### Lang — the one part of the family that is complete

| Key | en (`assets/iwex/lang/en.json`) | ru | uk |
|---|---|---|---|
...
`IwexRecipeConfig.DefaultCatalogue` nor `SmexRecipeConfig.Defaults()` carries a roll-set cost row
...
question - whether four (lpex) or six (smex) sets plus their mills is a sane ask - is
...
|---|---|---|
| `RollSetSpec` | `src/IronworkingExpanded/BlockStructures/Forming/RollSetSpec.cs:31` | the record; `TryParse` at `:108-201` |
...
| golden | `test/IronworkingExpanded.Tests/goldens/iwex/itemtypes/rollset.json` | pins all four specs byte-for-byte |
| tests | `test/IronworkingExpanded.Tests/Blocks/Forming/RollSetSpecTests.cs` - 15 `[Fact]`/`[Theory]`, 230 lines | the schema; the mill's other eight files are [rolling mill](../machines/rolling-mill.md)'s |

### The cross-mod emit question

A second mod ships sets by emitting its own itemtype, not by extending iwex's. The fit gate never looks at
the domain (`BlockRollingMill.cs:294`), and `attributesByType` is per-itemtype, so lpex would ship
`lpex:rollset-*` from its own `IExItemDefProvider` and smex `smex:rollset-*` from its own
([steel roll sets](../machines/steel-roll-sets.md) § Code). That works today with no iwex change.
...
| a set is visually indistinguishable from any other set | all of them render `game:item/ingot` (`:122`), and the fitted set is legible only from block info. Mis-fitting an lpex wide set where a smex one is needed is silent |
| the double-not-float rule must be re-derived | `RollSetItemDefinitions.cs:22-24` is a comment in iwex; a new provider that authors `float[]` gaps ships a destabilised golden and nothing warns |
| `RollSetSpec` itself stays in iwex | so lpex and smex take an iwex reference. The chain allows it (`exlib ← iwex ← lpex ← smex`), but the same question is open for `ItemDie` - see [dies § Open](dies.md) |
...
* A malformed set reports "busy". `TryFitRollSet` returns `false` both for a running pass and for a parse
  ...
  `iwex-rollingmill-busy` (`BlockRollingMill.cs:312`). An author error in a new mod's set is
...
| 3 | **Wire the seven drawn shapes** and re-texture them with domain-relative references | plus draw 2.5 (lpex) and 3.0 / 3.5 (smex) |
