...
**Mod** iwex ships the two live items; smex owns the three cast forms (who owns the forming line)
...
|---|---|---|---|---|---|
| `shingledbar` | 3 × 3 × 18 | 162 | 400 | helve hammer, 2 wrought balls @ 200 | the iwex mill - narrow `flat` and `grooved` |
| `shingledslab` | 8 × 3 × 20 | 480 | 1200 | [steam hammer](../machines/steam-hammer.md) (lpex), 6 wrought balls @ 200; the hammer reads the pile at the first lever pull | the lpex [wide hall](../machines/wide-hall.md) - four stands, 2.5 → 1.0 |
| `castbillet` | 3 × 3 × 27 | 243 | 600 | [long cell](../machines/long-cell.md), 3-lane billet pattern | the iwex mill's own barrel, on steel roll sets - never the train |
...
| Asset | Path | State |
|---|---|---|
| stage shapes | `assets/iwex/shapes/forming/stock-{bloom,slab}-{5,10,15,20,30}.json` | ship, untracked in git, resolved by `StockItemDefinitions.cs:41`, measured off disk by `RolledStockStagesTests.cs:24-29` |
...
| `assets/iwex/shapes/item/{castbillet,castbloom,castslab}.json` | — | the same three, exported to the runtime domain, untracked, no item resolves them (a grep for these names in `src/` returns only `HeatingHearthLayout.cs:72-77`) |
...
| `RolledStockStagesTests` | `test/IronworkingExpanded.Tests/Blocks/Forming/RolledStockStagesTests.cs:22` | 9 methods pinning the ten generated shapes to the spread model |
