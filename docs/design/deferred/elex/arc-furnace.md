...
and no code)   **Deferred by** D8 ([STATE.md](../../../../../docs/superpowers/plans/STATE.md)) - the release target is the ferrous line,
...
D8 ([STATE.md](../../../../../docs/superpowers/plans/STATE.md)): the release target is the complete ferrous line, and elex is not on it. The reasoning belongs to [scope](../../scope.md) § The release target.
...
Nothing in the shipped ferrous line is walled by this machine. It is the ceiling of the tool-material ladder; crucible steel (D9, [STATE.md](../../../../../docs/superpowers/plans/STATE.md)) was adopted so the suite had a gear reward that did not wait on elex.
...
* A latent bootstrap loop sits in the bearing chain ([STATE.md](../../../../../docs/superpowers/plans/STATE.md)) - "only HP jaws crack chromite" would make an hpex machine the sole source of a material hpex needs. It is safe today only because chromite is vanilla. HSS puts a second consumer on that same crusher, so if the gate is ever enforced literally, this furnace tightens the loop rather than relieving it.
