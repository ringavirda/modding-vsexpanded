...
|---|---|---|---|---|
| megablock (multiblock projection), open-top stack | cold air, **2 tuyeres @ 24 L/s** (MP blower) | dense burden + cold blast → **molten pig iron** (+ slag) | ~30 u/s pig, ~30 s melt interval (tunable) | *(live)* |

- Runs the shared **dynamic heat balance** (conventions.md): internal temp = coke% (burden) + blast buff; cold blast holds the *natural* ceiling, never the boosted one (that needs the smex hot blast + cowpers). Per **R5** there is always a guaranteed melt path (high-coke + cold blast); the model gates speed/efficiency, not possibility. Block-info always shows current T, the threshold and contributors.
...
- FSM (`FurnaceState`: Idle → Firing → Melting): fills → ignites on full high-coke charge → soaks above the melt point → melts on the interval, draining pig into the tap. Disruptions (low mix, exhaust in tuyeres, choke, open door, liquid full) count toward extinguish.
...
- Parts *(live)*: `blastfurnacedoor`, reinforced hopper, bell hopper, tuyere, blast-furnace tap, slag block. Pig currently solidifies at the tap (`BlockSolidifiedIron`, chiselled out); casting to standardised 200 u pigs via the mold pedestal is *(planned)*.
