...
/// the one shaft. The shaft <b>math</b> is pinned in <see cref="MpEnergyNetworkStateTests"/>; here we drive the
/// whole loop with node doubles to prove it discovers storage/drives/loads and that torque - not accumulated
/// energy - governs whether the run turns.
/// </summary>
public class MpEnergyNetworkTickTests
{
  ...
  /// <summary>Builds a single network holding the given nodes (placed in the world so the tick resolves them
  /// by position). Bypasses the graph so the loop can be driven without connector-correct doubles.</summary>
  ...
  )
  {
    ...
    {
      world.Place(pos, TestBlocks.Configure(new Block(), $"test:mpnode{id}", id), be);
      net.Nodes.Add(pos);
      id++;
    }
    return net;
  ...
  public void A_drive_spins_the_storage_run_up()
  {
    ...
    Assert.True(s.Speed > 0f, "a drive above the idle floor should spin the shaft up");
    Assert.Equal(MpEnergyNetworkState.EnergyAtSpeed(10f, s.Speed), s.StoredEnergy, 3);
  ...
  public void A_load_beyond_the_drive_stalls_the_run()
  {
    ...
    // The drive can't out-torque the load, so the shaft never leaves rest - no energy accumulates (not a battery).
  ...
  public void A_charged_flywheel_coasts_when_the_drive_is_cut()
  {
    ...
    // A big inertia coasts: still turning, just a little slower - it doesn't drop dead the instant the drive stops.
  ...
  public void A_run_with_no_storage_has_no_reservoir()
  {
    ...
    var net = Wire(world, (new BlockPos(0, 0, 0), new DriveNode { Torque = 5f }));
