...
/// <summary>
/// Unit coverage for the item-side fluent builder <see cref="ExItemDef"/>: each method emits the itemtype
/// JSON key the vanilla object loader expects, the asset location targets <c>itemtypes/</c> (not
/// <c>blocktypes/</c>), and the object-valued transform / recipe / attribute helpers round-trip a POCO. Pure
/// (no registry), so no serialization collection is needed.
/// </summary>
public class ExItemDefTests
{
  ...
  public void Scalars_and_held_animations_emit_their_keys()
  {
    ...
    Assert.Equal("game:item/resource/crushed/normal", (string?)json["shape"]!["base"]);
  }
  ...
  public void Texture_and_texture_all_accumulate_under_textures()
  {
    ...
    Assert.Equal("game:block/metal/ingot/{metal}", (string?)json["textures"]!["rusty-iron"]!["base"]);
    Assert.Equal("game:block/metal/ingot/{metal}", (string?)json["textures"]!["gold"]!["base"]);

    JObject all = ExItemDef.Create("iwex", "slag").TextureAll("game:x").ToJson();
  ...
  public void CreativeTab_accumulates_and_CreativeCommon_derives_the_mod_tab()
  {
    JObject tabs = ExItemDef
      ...
      .CreativeTab("items", "*")
      .ToJson();
    Assert.Equal("*", (string?)((JArray)tabs["creativeinventory"]!["general"]!)[0]);
    Assert.Equal("*", (string?)((JArray)tabs["creativeinventory"]!["items"]!)[0]);

    JObject common = ExItemDef.Create("iwex", "slag").CreativeCommon("*").ToJson();
  ...
  public void Transforms_take_a_poco_and_land_at_the_root_transform_keys()
  {
    JObject json = ExItemDef
      .Create("iwex", "slag")
      .GuiTransform(new { rotation = new { x = 150, y = -38, z = 0 }, scale = 3.8 })
  ...
  public void CombustibleProps_and_GrindingProps_land_at_the_root()
  {
    JObject json = ExItemDef
      ...
      .GrindingProps(new { groundStack = new { type = "item", code = "iwex:powderedslag" } })
      .ToJson();

    Assert.Equal(720, (int)json["combustibleProps"]!["meltingPoint"]!);
    Assert.Equal("iwex:powderedslag", (string?)json["grindingProps"]!["groundStack"]!["code"]);
  }
  ...
  public void Attribute_and_Attributes_populate_the_attributes_object()
  {
    JObject json = ExItemDef
      ...
      .Attributes(new { dissolveInWater = true, fertilizerTextureCode = "potash" })
      .ToJson();

    Assert.Equal("iwex:slag", (string?)json["attributes"]!["shatteredStack"]!["code"]);
    Assert.True((bool)json["attributes"]!["dissolveInWater"]!);
    Assert.Equal("potash", (string?)json["attributes"]!["fertilizerTextureCode"]);
  }
