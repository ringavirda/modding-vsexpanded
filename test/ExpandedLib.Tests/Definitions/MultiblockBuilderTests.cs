...
/// sub-volumes + bespoke <c>At</c> cells, with build-time validation that retires the block-number class of
/// bugs (an offset referencing a number with no <c>blockNumbers</c> entry, or two offsets on one cell). Order
/// is pinned because the emitted offsets array is compared position-by-position against the hand-written JSON.
/// </summary>
public class MultiblockBuilderTests
{
  ...
  public void Fill_emits_offsets_in_z_outer_y_mid_x_inner_order()
  {
    ...
    Assert.True(JToken.DeepEquals(expected, offsets), "Fill order diverged:\n" + offsets);
  }
  ...
  {
    // A transposed/inverted range would silently omit the sub-volume; the builder must fail loudly.
    var ex = Assert.Throws<ArgumentException>(
