...
/// <summary>
/// Golden-file parity for every iwex code-first def: each def must reproduce its committed
/// <c>goldens/iwex/{Location.Path}</c> file, and the golden set must exactly cover the defs. Non-parity
/// behavioural checks (orientation derivation, multiblock cell counts, filler ports) live in
/// <see cref="IwexDefinitionBehaviorTests"/>; the harness is <see cref="DefinitionGoldens"/>.
/// </summary>
public class IwexDefinitionGoldenTests {
  ...
  private static readonly string GoldenRoot =
    DefinitionGoldens.SolutionRelative(
      "test/IronworkingExpanded.Tests/goldens"
    );
  ...
  /// <para>
  /// Narrower than "every blocktype has a resolving name key": some iwex blocks ship no name key at all,
