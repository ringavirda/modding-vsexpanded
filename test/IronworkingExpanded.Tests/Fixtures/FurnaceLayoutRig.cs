...
/// <summary>
/// Shared geometry oracle for the furnace multiblocks - iwex's cold blast furnace and cupola, smex's hot
...
public static class FurnaceLayoutRig {
  ...
  // cell hold `iwex:furnace-chargepile` as well as `game:air`/`game:coalpile` - vanilla's matcher compares
  ...
  /// <c>IwexCodes.HearthCell</c>. A different string from <see cref="ShaftGlyph"/> carrying the same
  ...
  /// both iwex furnaces - see <see cref="AssertNoExhaustOutlets"/>.</returns>
