...
/// <summary>
/// The work piece: stock part-way through a rolling schedule, carrying a thickness per <b>strip</b> across its
/// width. The state lives on the item stack because a two-high stand cannot be fed backwards - the piece is
/// walked back around the mill after every pass, so it spends most of its life in the player's hands or on the
/// ground, not in the machine.
/// <para>
/// The invariant worth guarding hardest is that <b>strips sum back to the whole piece</b>: each spreads on its
/// own share of the base width and its own share of the ceiling, so an evenly-rolled piece is exactly as wide
/// as <see cref="StockForm.WidthAt"/> says. If that drifts, half-rolled geometry and the shipped stage shapes
/// stop agreeing.
/// </para>
/// </summary>
public class WorkPieceTests
{
  ...
  )
  {
    ...
    // Summing the strips must reproduce StockForm.WidthAt, or the strip view and the stage shapes disagree.
  ...
  public void A_half_rolled_piece_is_lopsided_and_sits_between_the_two_even_widths()
  {
    ...
    Assert.True(half.Width > bloom.WidthAt(3f), "the rolled side should have spread");
    Assert.True(half.Width < bloom.WidthAt(1f), "but the unrolled side has not");
  }
  ...
  public void Rolling_one_strip_leaves_the_others_alone()
  {
    ...
    Assert.Equal(3f, before.Strips[0]); // the original is untouched - the record copies
  }

  [Fact]
  public void A_strip_never_spreads_past_its_share_of_the_forms_ceiling()
  {
    // Otherwise an unevenly-rolled piece could end up wider than the form is ever allowed to be.
    StockForm bloom = StockForm.Bloom;
  ...
  {
    // It is dropped on the ground and picked up again between every pass, so this is the load-bearing path.
    ...
    Assert.Equal(original.Turned, read.Turned); // the turn-over survives the carry-back too
  }
  ...
  {
    // A piece from a mod that has since been removed must not silently become a bloom.
    ItemStack stack = Stack();
