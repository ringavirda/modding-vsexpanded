...
/// <summary>
/// What happens when a player offers stock to the rolls. The player picks two things at the deck - where along
/// the barrel to feed (the gap) and which strip of the piece to put through - and everything else follows from
/// geometry.
/// <para>
/// The rule worth pinning hardest is the one that <b>is not written down anywhere</b>: there is no explicit
/// "you may only use the next gap" check. Skipping is stopped by <c>δ_max = μ²R</c> - a gap far narrower than
/// the stock simply skids - so the schedule is enforced by the same relation that makes this a hot mill, and a
/// legal deeper bite is allowed if the operator spots one.
/// </para>
/// </summary>
public class MillFeedTests
{
  ...
  ) => MillFeed.Decide(set ?? FlatSet, piece, gapIndex, strip, tempC, Radius, RollingTemp);
  ...
  public void The_deck_divides_into_one_band_per_gap(float alongBarrel, int expected)
  {
  ...
  {
    // A wide set has one gap across the whole barrel, so wherever you feed it is the same pass.
  ...
  public void The_hit_point_maps_onto_the_length_of_the_deck(double localX, float expected)
  {
  ...
  public void The_far_end_of_the_deck_is_the_widest_gap_and_the_near_end_the_narrowest()
  {
    ...
    Assert.Equal(gapCount - 1, MillFeed.GapZone(MillFeed.AlongBarrel(0.9), gapCount));
  }
  ...
  {
    // With two sides the choice is real; with one there is only ever the whole piece to feed.
    Assert.NotEqual(MillFeed.StripIndex(true, 2), MillFeed.StripIndex(false, 2));
  ...
  {
    // 3.0 straight to 0.5 is a 2.5 draft against delta_max of 1.0 - the rolls cannot pull it in. Note there is
    // no "wrong gap" rule doing this; it is the friction limit.
  ...
  {
    // From 2.0, the 1.0 gap is a 1.0 draft - exactly delta_max, so it is legal even though it skips 1.5. An
    // operator who spots that may take it, and pays for it in torque.
  ...
  {
    // The piece passes through untouched. Forgiving, and it teaches itself.
  ...
  {
    // Reported as cold, not as a bad gap - the fix is the furnace, not a wider gap.
    FeedDecision d = Feed(WorkPiece.Fresh(StockForm.Bloom), gapIndex: 0, tempC: 500f);
  ...
  {
    // Half-rolled: strip 0 already down to 2.0, strip 1 untouched at 3.0. The same gap is a reduction for one
    // and a skid for the other - which is exactly why strips are tracked separately.
    var half = new WorkPiece(StockForm.Bloom, [2f, 3f], new bool[2]);

    Assert.True(Feed(half, gapIndex: 1, strip: 0).Accepted); // 2.0 -> 1.5
    Assert.Equal(FeedVerdict.WontBite, Feed(half, gapIndex: 1, strip: 1).Verdict); // 3.0 -> 1.5 is too deep
  }
  ...
  public void A_stand_with_no_roll_set_rolls_nothing()
  {
    Assert.Equal(
      ...
      MillFeed
        .Decide(null, WorkPiece.Fresh(StockForm.Bloom), 0, 0, Hot, Radius, RollingTemp)
        .Verdict
    );
  }
  ...
  public void A_set_refuses_stock_it_does_not_accept()
  {
    ...
    Assert.Equal(FeedVerdict.WrongForm, Feed(WorkPiece.Fresh(StockForm.Slab), 0).Verdict);
    Assert.Equal(
  ...
  public void An_out_of_range_gap_or_strip_is_a_no_op_not_a_crash()
  {
    ...
    Assert.Equal(FeedVerdict.NoReduction, Feed(piece, gapIndex: 0, strip: 99).Verdict);
  }
