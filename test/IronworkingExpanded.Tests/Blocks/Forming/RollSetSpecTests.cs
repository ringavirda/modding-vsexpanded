...
/// <summary>
/// The roll-set tooling contract. A set carries its own schedule, so adding a rolled product is an item def
/// and nothing else - the same idiom the casting patterns use.
/// <para>
/// The rules worth pinning are the ones that encode the design's central conceit: the gap sequence is cut into
/// the barrel <b>widest first</b>, so it must strictly descend; the player walks it one segment at a time and
/// cannot skip; and because each gap is a real stopping point, <b>the product is the thickness you stop at</b>.
/// </para>
/// </summary>
public class RollSetSpecTests
{
  ...
  {
    Assert.True(RollSetSpec.TryParse(Json(json), out RollSetSpec? spec, out string? error), error);
    return spec!;
  }
  ...
  {
    // An out-of-order gap would silently let the stock skip a reduction, or take a negative one - so it is a
    // load-time complaint, not a mystery at the mill.
    Assert.Contains("descend", Rejects(FlatSet.Replace("[ 1.5, 1.0, 0.5 ]", "[ 1.0, 1.5, 0.5 ]")));
    Assert.Contains("descend", Rejects(FlatSet.Replace("[ 1.5, 1.0, 0.5 ]", "[ 1.0, 1.0 ]")));
  }

  [Fact]
  public void A_set_that_bites_nothing_or_makes_nothing_is_rejected()
  {
    Assert.Contains("accepts", Rejects(FlatSet.Replace("[ \"bloom\", \"billet\" ]", "[ ]")));
    Assert.Contains("outputs", Rejects(FlatSet.Replace("\"outputs\"", "\"unused\"")));
  }

  [Fact]
  public void An_output_must_sit_on_a_real_gap()
  {
    // Otherwise it is a product you could never stop at.
    Assert.Contains("not one of", Rejects(FlatSet.Replace("\"gap\": 1.0", "\"gap\": 1.25")));
  }
  ...
  {
    // The schedule is geometry, not a setting: from 1.5 the only legal next step is 1.0, never straight to
    // 0.5. That is what keeps every pass inside delta_max without the mill having to check it.
    RollSetSpec spec = Parse(FlatSet);
  ...
  {
    // Slab and bloom fill the whole barrel, so there is no room for a sequence of segments: reducing them is
    // a train of stands, not a walk. The flag is what tells the mill (and the handbook) which it is looking at.
  ...
  {
    // One bite never comes out flat and even: the piece is passed, turned over, and passed again. So a
    // reduction is never a single click, however small the stock.
    RollSetSpec spec = Parse(FlatSet);
  ...
  {
    // The whole argument for wide rolls, in one assertion. A shingled bloom starts 3 wide and spreads as it is
    // reduced; the narrow barrel stops swallowing it partway down the schedule, while the wide barrel never
    // notices.
    RollSetSpec narrow = Parse(FlatSet); // barrel 4
    RollSetSpec wide = Parse(FlatSet.Replace("\"barrelWidth\": 4.0", "\"barrelWidth\": 20.0"));
  ...
  {
    Assert.Contains("barrelWidth", Rejects(FlatSet.Replace("\"barrelWidth\"", "\"unused\"")));
  }
