...
/// <summary>
/// The smex to iwex domain migration for the relocated molten-network and blast-furnace blocks. The
...
public class SmexToIwexMigrationTests {
  // smex code as it shipped in 0.9.4 -> the iwex code that block lives under today. The last three are
  ...
  [InlineData(
    ...
    "iwex:molten-canal-straight-fire-ns"
  )]
  ...
  public void Blocks_that_never_shipped_under_smex_are_not_remapped() {
    // The ore line arrived with iwex and the slag paving line was added after the split, so neither ever
    // shipped under smex and neither may be claimed.
    var world = WorldWith(
