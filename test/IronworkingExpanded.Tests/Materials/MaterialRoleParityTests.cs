...
/// <summary>
/// Parity guard for iwex's material classification: for every item the machines see, the
/// <see cref="MaterialRoleRegistry"/> predicate must return the same result as the
/// <c>Collectible.Code.Path</c> equality check it replaced. Reads the module-init-seeded iwex roles
...
public class MaterialRoleParityTests {
  ...
  [Theory]
  [InlineData("iwex:burden")] // prepared ore charge for the blast furnace, never the cupola
