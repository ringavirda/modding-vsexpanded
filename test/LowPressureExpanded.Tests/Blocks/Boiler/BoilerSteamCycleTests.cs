...
/// <summary>
/// The boiler's internal steam bookkeeping outside the engine draw: bleeding excess steam out of the
/// open lid (<c>VentExcessSteam</c>), condensing leftover steam back into water after a shutdown
/// (<c>CondenseInternal</c>), and the shutdown reset. Pure field math on a primed Cornish boiler, so no
/// Api or structure is required. Venting must never collapse the working pressure and condensation must
...
public class BoilerSteamCycleTests {
  ...
  public void BoilStep_expands_water_into_steam_at_the_taxonomy_resolved_ratio() {
    // The water-to-steam expansion comes from the medium taxonomy (Water's vaporisation factor), and
    // falls back to the lpex steam-expansion constant when the medium leaves it unset. The built-in
    // Water leaves it unset, so the ratio resolves to 16:1.
    var be = Boiler(
      water: 100f,
      steam: 0f,
      state: BlockEntityBoiler.BoilerState.Boiling
  ...
  // InternalPressure can never read past MaxOutputPressure. The cap sits at the ceiling rather than
  // below it so a burning, closed boiler still trips the burst grace, which fires at >= ceiling.
