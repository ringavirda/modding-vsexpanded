...
/// <summary>
/// The boiler's pressure and temperature formulas, exercised on a Cornish boiler entity with its
/// water/steam state primed directly. They drive the danger zone and the steam the engines see, and are
/// pinned apart from the game-coupled tick loop.
/// </summary>
public class BoilerMathTests {
  ...
  public void SteamTemperature_reads_the_boiling_point_at_zero_gauge_pressure() {
    ...
    Assert.Equal(LpexValues.BoilingPoint, temp, 3);
  }
  ...
  public void SteamTemperature_follows_the_saturation_curve() {
    ...
    float expected =
      ...
      * (float)Math.Pow(2f + 1f, LpexValues.SteamSaturationExponent);
    float temp = (float)ReflectionHelpers.Invoke(be, "SteamTemperature")!;
