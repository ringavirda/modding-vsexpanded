...
/// <summary>
/// The fluid intake is the water source: it draws from the pond below into its own pipe network only
/// when the cube beneath is full water and no other intake crowds it. Covers the water scan, the
...
public class FluidIntakeBeTests {
  ...
  private static (
    ...
    var block = TestBlocks.Configure(new Block(), "lpex:fluidintake", 60);
  ...
  public void A_nearby_intake_marks_this_one_crowded() {
    ...
    world.Place(
      ...
      TestBlocks.Configure(new BlockFluidIntake(), "lpex:fluidintake", 61)
    );
