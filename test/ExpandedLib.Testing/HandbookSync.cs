...
/// <c>assets/{domain}/lang/en.json</c>. A parity test fails when a page's shipped text no longer matches its
...
public static class HandbookSync {
  ...
  public static (bool ok, string message) Check(Page page) {
    ...
    string relative =
      $"docs/{page.Domain}/handbook/{Path.GetFileName(page.HtmlPath)}";
    if (have == null)
      return (
        false,
  ...
  private static SortedDictionary<string, string> SourcesByNumber(
    ...
    ByNumber(
      DefinitionGoldens.SolutionRelative($"docs/{domain}/handbook"),
      "*.html"
    );
