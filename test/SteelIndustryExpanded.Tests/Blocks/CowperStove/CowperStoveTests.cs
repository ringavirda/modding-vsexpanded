...
/// <summary>
/// Cowper-stove regenerator behaviour that needs no gas network: the internal core temperature is
/// pushed out to the stacked heat sinks each tick and persisted. The network-fed heat-exchange path
/// needs a connected exhaust run and is covered by <see cref="CowperStoveHeatingTests"/>.
/// </summary>
public class CowperStoveTests {
  ...
  private static BlockEntityHeatSink PlaceSink(
    ...
    var hs = new BlockEntityHeatSink {
      Block = TestBlocks.Configure(new Block(), "siex:heatsink", 80),
    };
