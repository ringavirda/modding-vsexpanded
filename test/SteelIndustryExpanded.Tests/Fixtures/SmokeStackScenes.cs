...
/// <see cref="SiexValues.SmokestackGasIntakeVolume"/> litres of exhaust off the connected network and
...
/// (<c>siex:smokestack-intake-*</c>). The chimney is a solid 3x3 column through z=0..2, so the main
...
internal sealed class SmokeStackRig {
  ...
  public SmokeStackRig(int mainLength = 4) {
    ...
    Stack = new BlockEntitySmokeStack {
      Pos = Anchor.Copy(),
      // The layout's origin cell requires "siex:smokestack-intake*": a generic "siex:smokestack-n"
    ...
    Structure = StructureRig
      .Around(
        World,
        Stack,
        BlockSmokeStackIntake.Definitions("siex").Single(),
        angle: 0
      )
      .Complete();
