...
public class IiexCodesTests {
  ...
  public void A_side_group_takes_a_facing_and_keeps_the_letter(
    ...
    string code = IiexBlocks.HopperTall.WithSide(BlockFacing.FromCode(side));

    Assert.Equal($"iiex:hopper-tall-{letter}", code);
  ...
  public void An_orientation_group_takes_the_same_facing_and_keeps_the_letter(
    ...
    string code = IiexBlocks.FurnaceTuyere.WithOrientation(
      BlockFacing.FromCode(side)
    );

    Assert.Equal($"iiex:furnace-tuyere-{letter}", code);
    Assert.NotNull(FacingsFor(code));
  }
  ...
  public void A_vanilla_sourced_group_takes_the_same_facing_and_keeps_the_full_word(
    ...
    string code = IiexBlocks.SlagBrickstairs.WithHorizontalorientation(
      BlockFacing.FromCode(side)
    );

    Assert.Equal($"iiex:slag-brickstairs-up-{side}-*", code);
  }
  ...
  public void A_wildcarded_code_is_not_orientation_checked() {
    ...
    Assert.Null(FacingsFor(IiexBlocks.HopperTall.Any));
    Assert.Null(FacingsFor(IiexBlocks.FurnaceIrontap.Any));
  }
