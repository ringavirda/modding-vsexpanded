...
/// <summary>
/// The in-line gas valve block entity. An open valve is a normal pipe node and the run flows through
/// as one network; a closed valve severs the run at its own cell, and toggling re-walks the graph. A
/// closed valve must also drop any pool it cached while open, or the pressurised state bursts the
...
public class ValveBeTests {
  ...
  private static (TestWorld world, BlockEntityValve valve) Run() {
    ...
    world.Initialize(valve); // registers the valve node and captures the manager
  ...
  public void Closing_discards_the_pool_cached_while_open() {
    ...
    // Pressurise the joined run, then broadcast so the valve caches the pool.
  ...
  public void Loading_a_closed_valve_drops_any_persisted_pool() {
    // A hand-built save tree: valve closed but still carrying a pressurised pool. FromTree must
    // discard it before Initialize can restore it.
    var world = new TestWorld();
    var pipe = PipeTestWorld.MakePipe();
    var valve = new BlockEntityValve {
