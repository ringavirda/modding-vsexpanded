...
public class BurdenmakerTests {
  ...
  public void A_full_hopper_takes_no_more() {
    ...
    var (_, be, ore, _) = NewMachine();
    int cap = IiexValues.BurdenmakerOreCapacity;
  ...
  public void Every_tank_can_actually_reach_its_configured_capacity() {
    ...
    for (
      int i = 0;
      i < 64 && be.FluxUnits < IiexValues.BurdenmakerFluxCapacity;
      i++
    )
      be.TryLoadFlux(
        Slot(lime, IiexValues.BurdenmakerFluxCapacity),
        wholeStack: true
      );
    Assert.Equal(IiexValues.BurdenmakerFluxCapacity, be.FluxUnits);

    for (
      int i = 0;
      i < 64 && be.OreUnits < IiexValues.BurdenmakerOreCapacity;
      i++
    )
      be.TryLoadOre(
        Slot(ore, IiexValues.BurdenmakerOreCapacity),
        wholeStack: true
      );
    ...
    Assert.Equal(
      IiexValues.BurdenmakerOreCapacity + IiexValues.BurdenmakerFluxCapacity,
      be.BurdenUnits
    );
    Assert.True(be.BurdenUnits <= IiexValues.BurdenmakerBunkerCapacity);
  }
  ...
  public void The_gate_refuses_when_nothing_is_loaded() {
    ...
    Assert.Equal("iiex-burdenmaker-nothingloaded", error);
  ...
  public void The_gate_refuses_while_the_basin_still_holds_a_batch() {
    ...
    Assert.False(be.ToggleGate(out string? error));
    Assert.Equal("iiex-burdenmaker-emptybunker", error);
