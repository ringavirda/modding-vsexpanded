...
/// <summary>
/// The molten-canal start is the network's <see cref="ILiquidMetalSink"/>: a tap or crucible pours
/// liquid metal here and it flows down the run. Covers the acceptance predicates, the server-side
/// fill (including the soak-heat-when-full path that keeps a fed start from solidifying) and the
/// pour tally that drives the block info readout.
/// </summary>
public class MoltenCanalStartTests {
  ...
  public void CanReceiveOrSoak_stays_true_for_the_same_metal_even_when_full() {
    ...
    // Full: CanReceive is false (no room), but the looser soak predicate stays true so a furnace tap
    // keeps heating the cell instead of letting it solidify.
