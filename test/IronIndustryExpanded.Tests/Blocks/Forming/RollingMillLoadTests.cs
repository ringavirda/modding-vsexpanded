...
public class RollingMillLoadTests {
  ...
  private const float NormalWheel = 10f; // IiexValues.FlywheelInertiaNormal
  private const float LargeWheel = 150f; // IiexValues.FlywheelInertiaLarge
  ...
  private static (
    ...
    foreach (var (pos, be) in cells) {
      world.Place(
        pos,
        TestBlocks.Configure(new Block(), $"iiex:millrun{id}", id),
        be
      );
      net.Nodes.Add(pos);
      id++;
