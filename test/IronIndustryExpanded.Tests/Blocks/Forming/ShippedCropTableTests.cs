...
public class ShippedCropTableTests {
  ...
  private static string RepoRoot() {
    for (
      DirectoryInfo? dir = new(System.AppContext.BaseDirectory);
      dir != null;
      dir = dir.Parent
    )
      if (Directory.Exists(Path.Combine(dir.FullName, "assets", Domain)))
        return dir.FullName;
    throw new DirectoryNotFoundException("no repo root above the test output");
  }
