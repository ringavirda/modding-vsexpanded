...
public class FurnaceHudDistributionTests {
  ...
  private const string MetalKey = "iiex:bf-info-molteniron";
  private const string SlagKey = "iiex:bf-info-moltenslag";
  private const string CastIronKey = "iiex:cupola-info-moltencastiron";

  // The shaft-charge line: the loaded count against the furnace's capacity.
  private const string MixKey = "iiex:bf-info-shaftfull";
  private const string WrongBurdenKey = "iiex:bf-info-wrongburden";
  private const string TapStateKey = "iiex:tap-state";
  private const string NeedsMixKey = "iiex:bf-info-needsmix";
  ...
  private static BlockEntityCupolaFurnace CupolaFurnace(TestWorld world) =>
    Furnace<BlockEntityCupolaFurnace>(
      ...
      (JObject)
        BlockCupolaFurnaceCore.Definitions("iiex").Single().ToJson()[
          "attributes"
        ]!
    );
  ...
  private static BlockEntityHopperTall Hopper(TestWorld world, BlockPos pos) {
    ...
    world.Place(
      pos,
      TestBlocks.Configure(new Block(), "iiex:hopper-tall", 3),
      be
    );
    world.Attach(be);
    return be;
