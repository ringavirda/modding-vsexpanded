...
public class FurnaceRoleCellsTests {
  ...
  /// <summary>
  /// The two reverberatory hearths, raised but not completed: their footprints need blocks the iiex test
  ...
  public void Each_tap_is_exactly_one_cell_and_the_build_refuses_a_second() {
    ...
    InvalidOperationException ex = Assert.Throws<InvalidOperationException>(
      () =>
        ExBlockDef
          .Create("iiex", "twodrainfurnacecore")
          .MultiblockLayout(s =>
            s.Origin(0, 0)
  ...
  public void Two_glyphs_cannot_split_one_drain_between_them() {
    ...
    InvalidOperationException ex = Assert.Throws<InvalidOperationException>(
      () =>
        ExBlockDef
          .Create("iiex", "splitdrainfurnacecore")
          .MultiblockLayout(s =>
            s.Origin(0, 0)
              .Legend('C', "iiex:furnace-blastcore-*")
              .Legend('T', IronTapGlyph)
  ...
  public void No_iwex_furnace_declares_a_gas_outlet_because_none_of_them_vents() {
    ...
    // iiex layout that grows a pipe outlet has to add the role deliberately.
    foreach (
  ...
  public void A_box_derived_before_the_layout_arrives_is_never_memoised() {
    ...
    var core = new BlockEntityBlastFurnaceCold();
    Orient(core, "iiex:furnace-blastcore-tier1-n", "north"); // a block with no attributes at all
    Assert.Null(ShaftBoxOf(core));
    Assert.Empty(core.ShaftColumns);

    // The layout arrives late, and the furnace recovers completely.
    Stand(core, ColdDef(), Anchor, "iiex:furnace-blastcore-tier1", "north");
