...
public class ChargeColumnTests {
  ...
  private const string Burden = "iiex:burden";
  ...
  // What the blocks over a column draw. Band size arrives as a parameter rather than off IiexValues, so
  ...
  public void An_ore_pile_is_thirty_two_ITEMS_and_a_remelt_pile_is_three_thousand_UNITS() {
    ...
    Assert.Equal(2, IiexValues.ChargeItemsPerBand);
    Assert.Equal(
      32,
      IiexValues.ChargeItemsPerBand * ChargeColumn.BandsPerBlock
    );
    Assert.Equal(3000, IiexValues.CupolaChargeMetalUnitsPerBlock);
    ...
    Assert.NotEqual(
      0,
      IiexValues.CupolaChargeMetalUnitsPerBlock % ChargeColumn.BandsPerBlock
    );
