...
public class BlastFurnaceTapTests {
  ...
  /// <summary>
  /// <c>DrainIronTap</c>'s per-tick pool draw follows <see cref="IiexValues.TapDrainPerTick"/> rather than
  ...
  public void Retuning_the_tap_rate_moves_the_drain_with_it() {
    var world = NewWorld();
    // The drain resolves its carrier through MetalRegistry, a process-global the suite seeds from
    // several places, so the item to register is asked for rather than assumed: with nothing
    ...
    try {
      ...
      IiexValues.Edit(c => c.TapDrainPerTick = original);
    }
  ...
  /// <c>_moltenSlag</c> and <see cref="IiexValues.TapSlagStackFactor"/>. No other test exercises the slag
  ...
  public void The_slag_tap_drains_by_its_own_stack_factor_not_the_irons() {
    var world = NewWorld();
    // MetalRegistry.MoltenItemOf("slag") answers iiex:slag when a MetalDef is registered (process-wide
  ...
  public void The_creative_entry_the_grid_recipe_and_the_drop_all_name_the_south_facing() {
    ...
    string creative = (string)
      BlockFurnaceTap.Definitions("iiex").First().ToJson()[
        "creativeinventory"
      ]!["general"]!.First()!;
    ...
    [
      .. RecipeCodes
        .OutputBlockCodes("iiex", typeof(BlockFurnaceTap).Assembly)
        .Where(c =>
          c.StartsWith("iiex:furnace-irontap", StringComparison.Ordinal)
          || c.StartsWith("iiex:furnace-slagtap", StringComparison.Ordinal)
        ),
    ];
