...
public class CastIronBevelTests {
  ...
  public void IsBevelGearItem_matches_the_gear_code() {
    var gear = new ItemStack(
      TestBlocks.Configure(new Block(), "iiex:bevelgear", 211)
    );
    var other = new ItemStack(
      TestBlocks.Configure(new Block(), "iiex:mpenergy-shaft-ns", 212)
    );
  ...
  public void A_broken_bevel_returns_the_shaft_and_one_gear() {
    ...
    world.RegisterItem("iiex:bevelgear");
    ...
    Assert.Contains(
      drops,
      d => d.Collectible?.Code?.ToString() == "iiex:mpenergy-shaft-ns"
    );
    Assert.Contains(
      drops,
      d => d.Collectible?.Code?.ToString() == "iiex:bevelgear"
    );
  }
