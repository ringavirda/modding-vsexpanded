...
internal sealed class RegulatedEnginePlant {
  ...
  public RegulatedEnginePlant(Scene scene, BlockPos enginePos, float gateAtm) {
    ...
    var valveBlock = TestBlocks.Configure(
      ...
      $"iiex:pipe-cast-pressurevalve-{orient}",
      58,
      ("tier", "cast"),
      ("type", "pressurevalve"),
      ("orientation", orient)
    ...
    scene.Block(d0.AddCopy(inletFace), IiexScenes.Cap(60));
    _drain = d0;
...
internal sealed class ManualPumpPlant {
  ...
  public ManualPumpPlant(Scene scene, BlockPos pos) {
    ...
    scene.Block(_output.AddCopy(outFace), IiexScenes.Cap(65));
  }
...
internal sealed class CondenserPlant {
  ...
  public CondenserPlant(Scene scene, BlockPos pos) {
    ...
    var block = TestBlocks.Configure(
      new IronIndustryExpanded.BlockNetworkPipe.Blocks.BlockSteamCondenser(),
      "iiex:steamcondenser-n",
      66,
      ("side", "north")
    );
    ...
    // North: the spent-steam line, cast (iiex) tier. Spent steam runs above the plated tier's burst
    ...
    scene.Block(_steam.AddCopy(BlockFacing.NORTH), IiexScenes.Cap(68));
    ...
    scene.Block(_feed.AddCopy(BlockFacing.WEST), IiexScenes.Cap(71));
    ...
    scene.Block(_recovered.AddCopy(BlockFacing.EAST), IiexScenes.Cap(73));
  }
