...
/// <summary>
/// Property tests over the pipe simulation: many randomized sequences of produce/consume/tick, each
/// asserting the laws that hold for all of them - finite, non-negative state; pressure never past the
/// burst ceiling without bursting; no gas created from nothing.
/// </summary>
public class PipeInvariantTests {
  ...
  // Randomly interleaves gas/water production, consumption and ticks, including medium switches on a
  // drained run. An empty run (State null, or Volume ~0) must never permanently reject a fresh medium:
  // a latched stale-label guard would wedge a physically empty run so neither medium could re-claim it.
  [Theory]
  [InlineData(3)]
  [InlineData(64)]
  [InlineData(512)]
  ...
  public void An_empty_run_can_always_be_re_claimed_by_either_medium(int seed) {
    ...
    for (int op = 0; op < 60; op++) {
      ...
      // Drain the run completely: one of the two mediums must then be accepted into the empty pipes.
