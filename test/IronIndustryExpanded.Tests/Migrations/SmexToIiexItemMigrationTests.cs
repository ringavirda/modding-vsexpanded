...
/// <summary>
/// The smex to iiex domain migration for the relocated items (blast mix, slag, powdered slag): it pairs
/// every registered <c>iiex</c> item whose base code is relocated with its old <c>smex</c> code, leaves
/// the iiex-only <c>burden</c> item and non-iiex items untouched, and stays independent of the same-named
/// <c>slag</c> block migration.
/// </summary>
public class SmexToIiexItemMigrationTests {
  ...
  public void Relocated_items_remap_from_smex_to_iwex_preserving_path() {
    ...
    foreach (string path in new[] { "slag", "powderedslag" })
      Assert.Equal(
        new AssetLocation("iiex", path),
        remaps[new AssetLocation("smex", path)]
      );
  }

  /// <summary>
  /// <c>blastmix</c> is not remapped: the <c>iiex:blastmix</c> item was deleted with the coal-pile
  /// charge path, so there is nothing to remap onto. The walk enumerates registered <c>iiex</c> items,
  /// so a stale entry in the relocated set yields no pair; both spellings become unresolvable and the
  /// stack is dropped on load.
  /// <para>
  /// It is not pointed at <c>iiex:burden</c> either: burden carries a stamped iron/flux/coke mix and
  ...
  public void Blast_mix_is_not_remapped_because_the_item_no_longer_exists() {
    ...
    var world = WorldWith(Item("iiex:blastmix", 100), Item("iiex:slag", 101));
    var remaps = Remaps(world);
  ...
  public void The_new_burden_item_is_not_remapped() {
    var world = WorldWith(
      Item("iiex:burden", 100),
      Item("iiex:slag", 101) // a relocated one, for contrast
    );
    var remaps = Remaps(world);
