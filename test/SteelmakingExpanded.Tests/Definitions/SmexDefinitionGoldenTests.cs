...
/// <summary>
/// The golden-file parity oracle for every smex code-first def: the trivial blocks, the Bessemer
...
/// <c>goldens/smex/{Location.Path}</c> file, and the golden set must exactly cover the defs. Behavioural
/// checks live in <see cref="SmexDefinitionBehaviorTests"/>; the harness is
...
public class SmexDefinitionGoldenTests {
  ...
  private static readonly string GoldenRoot =
    DefinitionGoldens.SolutionRelative(
      "test/SteelmakingExpanded.Tests/goldens"
    );
