...
/// all three zips.
/// <para>
/// The expected version is read from <c>mods/Directory.Build.props</c>, the manifest the build itself
...
public class ModinfoTests {
  ...
  public void Every_mod_declares_the_current_game_version() {
    ...
    Assert.True(
      wrong.Count == 0,
      $"every mods/*/src/modinfo.json must declare \"game\": \"{expected}\" (CurrentGameVer in "
        + $"mods/Directory.Build.props): {string.Join("; ", wrong)}"
    );
  }
  ...
  public void No_mod_s_source_version_is_at_or_below_a_version_it_has_already_released() {
    ...
    foreach (Mod m in Mods()) {
      ...
      if (Compare(m.Version, shipped) <= 0)
        behind.Add(
          $"{m.Folder} is {m.Version} but {m.ModId} {shipped} is already published"
        );
    }
  ...
  /// <summary>
  /// The highest version published per modid, off <see cref="ExpandedLib.Testing.ReleasedVersions"/>
  ...
  /// still carries the version that wrote it. siex has no such rename, so it is not remapped.
  /// </summary>
  ...
  private static string CurrentGameVer() {
    string props = File.ReadAllText(
      Path.Combine(RepoRoot(), "mods", "Directory.Build.props")
    );
    Match m = Regex.Match(
      props,
    ...
    Assert.True(
      m.Success,
      "mods/Directory.Build.props declares no <CurrentGameVer>"
    );
