...
public abstract class BEBehaviorProductionMachine(BlockEntity blockentity)
  ...
  /// the <see cref="CanRunProduction"/> gate and the catch-up <c>dt</c> clamp. Does not touch away-catch-up
  /// replay or the last-tick stamp, so it is safe to call from a fixture that never registers the
  /// listener at all.</summary>
  internal void DriveProductionTick(float dt) => RunOneTick(dt);

  /// <summary>Test seam: runs <see cref="OnIdleProductionTick"/> directly, bypassing
  /// <see cref="CanRunProduction"/> - for asserting the idle path itself rather than the gate that
  /// routes to it.</summary>
