...
public static class SandBedLayout {
  ...
  /// <summary>
  /// The flank letter of the element that draws <paramref name="side"/>. The shape's rows are drawn
  /// extending the opposite way from the footprint's, and the half turn <c>StructureAngle</c> carries to
  /// reconcile them crosses the flanks over as well: a west slot occupies the cell the shape's east mold
  /// is drawn in.
  /// </summary>
  private static string DrawnFlank(BedSlotSide side) =>
    side == BedSlotSide.West ? "E" : "W";
  ...
  public static string ElementFor(BedSlot slot, BedSlotState state) {
    ...
    string carved = slot.IsMold
      ? $"Mold{slot.Row}{DrawnFlank(slot.Side)}"
