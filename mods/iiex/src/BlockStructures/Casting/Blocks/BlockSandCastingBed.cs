...
public partial class BlockSandCastingBed
  ...
  private static ExBlockDef Bed(string domain) =>
    ExBlockDef
      ...
      // this same shape file. It spins by the side angle alone; the half turn that reconciles the drawn
      // rows with the declared footprint is on StructureAngle.
      .ShapeSpunPerOrientation("iiex:casting/sandcastingbed")
  ...
  /// <summary>
  /// Structure and filler rotation. The shape spins by the side angle alone; its rows are drawn extending
  /// the opposite way from the footprint's declared cells, and the +180 here carries that half turn. The
  /// same half turn crosses the flanks over, which <see cref="SandBedLayout.ElementFor"/> accounts for.
  /// The block entity's molten surfaces follow the mesh, so they turn by the shape's spin, not by this.
  /// </summary>
