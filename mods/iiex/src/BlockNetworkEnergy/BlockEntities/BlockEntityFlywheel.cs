...
public class BlockEntityFlywheel
  ...
  /// iiex config so it can be retuned without a rebuild.</summary>
  ...
  public float DriveTorque(float shaftSpeed) =>
    BridgeDriveTorque(
      HubAxleSpeed(),
      IiexValues.FlywheelBridgeChargePower,
      IiexValues.FlywheelBridgeRatedAxleSpeed
    );
  ...
  public override void GetBlockInfo(IPlayer forPlayer, StringBuilder dsc) {
    ...
    if (_savedNetworkState is not MpEnergyNetworkState s || s.Inertia <= 0f) {
      dsc.AppendLine(Lang.Get("iiex:flywheel-info-idle"));
      return;
    }
    ...
    dsc.AppendLine(
      Lang.Get(
        "iiex:flywheel-info-charge",
        ExMeasure.Charge(s.StoredEnergy, capacity)
      )
    );
    dsc.AppendLine(
      Lang.Get("iiex:flywheel-info-speed", ExMeasure.Speed(s.Speed))
    );
    if (s.SupplyPower > 1f || s.DemandPower > 1f)
      dsc.AppendLine(
        Lang.Get(
          "iiex:flywheel-info-power",
