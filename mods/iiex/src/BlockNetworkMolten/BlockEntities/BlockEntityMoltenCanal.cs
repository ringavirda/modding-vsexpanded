...
public class BlockEntityMoltenCanal
  ...
  public virtual int MaxUnitCapacity => IiexValues.CanalDefaultUnitCapacity;
  ...
  private void SetStackTemperature(IWorldAccessor world, float temp) {
    ...
    MoltenMetal.SetCooldownSpeed(
      _cellMetalStack,
      IiexValues.MoltenCooldownSpeed
    );
  }
  ...
  string? IChiselableMolten.ChiselBlockedError => "iiex-canaltoohot";
      ...
      : ExMeshCache.GetOrCreate(
        ...
        () => {
          Shape? baseShape = ExMeshCache.LoadShape(
            Api,
            new AssetLocation(
              $"iiex:shapes/molten/canal/{Block?.Variant["type"]}.json"
            )
          );
          if (baseShape == null)
            return null;
  ...
  public override void GetBlockInfo(IPlayer forPlayer, StringBuilder dsc) {
    ...
    if (Sealed)
      dsc.AppendLine(Lang.Get("iiex:canal-sealed"));

    if (Solidified) {
      string solidMetalName = MoltenMetal.DisplayName(CellMetalType);
      dsc.AppendLine(
        Lang.Get(
          "iiex:canal-solidified",
          CellAmount,
          MaxUnitCapacity,
          solidMetalName,
      ...
      // threshold.
      dsc.AppendLine(
        Lang.Get(IsHardened ? "iiex:canal-chiselready" : "iiex:canal-cooling")
      );
      return;
    }

    if (CellAmount <= 0f) {
      dsc.AppendLine(Lang.Get("iiex:canal-empty"));
    } else {
