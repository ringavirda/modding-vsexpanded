...
public class HopperTallTests {
  ...
  private static (
    ...
    rig.World.RegisterItem("iiex:remeltburden");
    ...
    var (hopperPos, wanted) = rig.Cells.Single(c =>
      c.Wanted.StartsWith("iiex:hopper-tall", StringComparison.Ordinal)
    );
  ...
  public void The_tank_fills_to_capacity_and_refuses_the_overflow() {
    ...
    int cap = IiexValues.HopperTallCapacity;
  ...
  public void Drips_burden_onto_the_shafts_lowest_column() {
    ...
    int per = IiexValues.HopperTallDropPerSecond;
  ...
  public void A_load_spread_over_many_ticks_stays_ONE_band_per_column() {
    ...
    var (core, rig, be) = Standing();
    int columnCount = core.ShaftColumns.Count;
    Deposit(be, BurdenItem(rig), IiexValues.HopperTallCapacity);
  ...
  public void Drips_what_the_furnace_below_declares_chargeable() {
    ...
    Assert.Equal("iiex:burden", Columns(core)[0].Segments[0].Material);
  }
  ...
  public void Fuel_goes_onto_burden_but_never_onto_more_fuel() {
    ...
    Deposit(be, coke, IiexValues.HopperTallDropPerSecond * columnCount);
    for (int i = 0; i < columnCount; i++)
      Tick(be);
  ...
  public void The_course_line_reports_the_top_course_of_the_column_being_filled() {
    ...
    Course(core, "iiex:burden", 3, new BurdenMix(75f, 5f, 20f));
    ...
    Assert.Contains("iiex:bf-info-course", Hud(be));
  ...
  public void The_course_line_names_the_fuel_laid_and_refuses_to_name_a_mixed_course() {
    ...
    Course(burnt, "iiex:burden", 3, new BurdenMix(75f, 5f, 20f));

    Assert.Equal(4, burnt.TopCourse.FuelBands);
    Assert.Equal("charcoal", burnt.TopCourse.FuelMaterial);
    Assert.Contains("iiex:bf-info-course", Hud(burntBe));
    ...
    Course(bare, "iiex:burden", 3, new BurdenMix(75f, 5f, 20f));

    Assert.Equal(0, bare.TopCourse.FuelBands);
    Assert.Null(bare.TopCourse.FuelMaterial);
  }

  /// <summary>What the course line prints for its fuel - the string <c>iiex:bf-info-course</c>'s second
  ...
  public void TopCourse_tells_a_charcoal_course_from_a_coke_course() {
    ...
    Course(coked, "iiex:burden", 3, grade);

    var (charred, _, charredBe) = Standing();
    Course(charred, "charcoal", 4, default);
    Course(charred, "iiex:burden", 3, grade);
    ...
    // (see FuelName).
    Assert.Contains("iiex:bf-info-course", Hud(cokedBe));
    Assert.Contains("iiex:bf-info-course", Hud(charredBe));
  ...
  public void The_charge_verdict_tells_a_coke_round_from_a_charcoal_round_at_the_same_band_count() {
    ...
    Assert.Contains("iiex:bf-info-chargemelts", Hud(cokedBe));
    Assert.DoesNotContain("iiex:bf-info-chargechills", Hud(cokedBe));

    Assert.Contains("iiex:bf-info-chargechills", Hud(charredBe));
    Assert.DoesNotContain("iiex:bf-info-chargemelts", Hud(charredBe));
  }
  ...
  public void The_readout_is_derived_so_it_survives_a_save_round_trip_unchanged() {
    ...
    Assert.Contains("iiex:bf-info-shaftfull", before);
  }
  ...
  public void An_incomplete_structure_says_nothing_about_the_shaft() {
    ...
    Assert.Contains("iiex:hoppertall-empty", hud);
    foreach (
      string key in new[]
      {
        "iiex:bf-info-course",
        "iiex:bf-info-chargemelts",
        "iiex:bf-info-chargechills",
