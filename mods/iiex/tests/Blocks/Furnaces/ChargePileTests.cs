...
/// <summary>
/// <c>iiex:furnace-chargepile</c> - the 16-band window onto a shaft column. The block owns no charge, it
...
public class ChargePileTests {
  ...
  private sealed class PileRig {
    ...
    /// <c>iiex:furnace-chargepile</c>, so a later monitor tick still reads the furnace complete.
    /// </summary>
  ...
  private static PileRig Build(
    ...
    core.Block = TestBlocks.Configure(
      new Block(),
      $"iiex:furnace-blastcore-tier1-{token}",
      1,
      ("side", token)
    );
    ...
    StructureRig structure = StructureRig.Around(
      world,
      core,
      def ?? BlockBlastFurnaceCoreCold.Definitions("iiex").Single(),
      angle
    );
  ...
  public void PileCode_is_the_code_the_definition_actually_renders() {
    ...
    ExBlockDef def = BlockChargePile.Definitions("iiex").Single();
    ...
    Assert.Equal(
      IiexBlocks.FurnaceChargepile.Any,
  ...
  public void A_charged_shaft_does_not_take_the_furnace_out_of_completion() {
    ...
    Assert.True(
      core.StructureComplete,
      "a charged furnace went incomplete: the shaft legend does not accept iiex:furnace-chargepile."
  ...
  public void The_shaft_legend_still_accepts_an_open_shaft_and_a_vanilla_coal_pile() {
    ...
    Assert.True(
      WildcardUtil.Match(wanted, new AssetLocation("iiex:furnace-chargepile"))
    );
  ...
  // unit figures rather than computed from IiexValues.ChargeUnitsPerBand: an expectation computed from
  ...
  [Theory]
  [InlineData("iiex:burden", "Burden")]
  [InlineData("iiex:remeltburden", "Burden")]
  ...
  public void A_pile_may_not_be_hand_placed_outside_a_shaft() {
    ...
    Assert.Equal("iiex-chargepile-notinshaft", failureCode);
  }
  ...
  public void The_shaft_gate_is_not_what_refuses_a_cell_inside_a_real_shaft() {
    ...
    // gate is under test.
    Assert.NotEqual("iiex-chargepile-notinshaft", failureCode);
  ...
  public void The_new_block_has_a_name_in_every_shipped_locale() {
    string langDir = Path.Combine(RepoPaths.Assets("iiex"), "lang");
    string[] locales =
    [
      .. Directory
    ...
    foreach (string locale in locales) {
      ...
      foreach (
        string key in new[]
        {
          ...
          "game:ingameerror-iiex-chargepile-notinshaft",
        }
      )
        if (
    ...
    foreach (string locale in locales) {
      var lang = JObject.Parse(File.ReadAllText(Path.Combine(langDir, locale)));
      Assert.Null(lang["game:ingameerror-iiex-chargepile-furnaceowns"]);
    }
  }
  ...
  public void The_name_key_is_exact_because_the_piles_only_variant_group_has_one_state() {
    ...
    var groups = (JArray)
      ((JObject)BlockChargePile.Definitions("iiex").Single().ToJson())[
        "variantgroups"
      ]!;
