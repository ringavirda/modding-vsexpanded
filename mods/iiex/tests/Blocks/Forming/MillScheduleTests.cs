...
public class MillScheduleTests {
  ...
  private const string BarRoute = """
    {
      ...
      "stages": [
        ...
        { "thickness": 2.0, "element": "Flattened200", "acceptedBy": [ "flat" ], "code": "iiex:beam" }
      ]
    }
    """;
  ...
  public void A_single_rung_branch_is_a_wide_set_that_needs_a_train() {
    ...
    MillSchedule wide = MillSchedule.For(
      ...
      Registry(
        ...
        {
          "family": "shingledbar",
          "stages": [ { "thickness": 2.0, "acceptedBy": [ "wide" ], "code": "iiex:heavyplate" } ]
        }
        """
      )
    )!;
  ...
  public void The_product_is_the_stage_you_stop_at() {
    Assert.Equal("iiex:rolledrod", Schedule().OutputAt(2.0f));
  }
  ...
  public void A_gauge_a_hair_off_the_rung_still_finds_its_stopping_point() {
    ...
    Assert.Equal("iiex:rolledrod", Schedule().OutputAt(2.0f + 1e-5f));
    Assert.Equal("iiex:rolledrod", Schedule().OutputAt(2.0f - 1e-5f));
  }
