...
public class SandBedLayoutTests {
    ...
    foreach (BedSlot slot in SandBedLayout.Slots.Where(s => s.IsMold))
      Assert.Equal(
        SandBedLayout.ImpressionsPerMold(slot.Row)
          * IronIndustryExpanded.Items.ItemPig.PigUnits,
  ...
  public void A_brick_and_a_pig_share_a_cavity_so_neither_pour_strands_a_remainder() {
    ...
    Assert.Equal(
      IronIndustryExpanded.Items.ItemPig.PigUnits,
      IronIndustryExpanded.Items.SlagItemDefinitions.SlagBrickUnits
    );
    foreach (BedSlot slot in SandBedLayout.Slots.Where(s => s.IsMold))
      Assert.Equal(
        0,
  ...
  // A mold element's letter is the flank the shape draws it on, and the shape is drawn a half turn from
  // the footprint, so a west slot is served by an east element. CastingBedFootprintGuards measures that
  // the pairing puts each element in the cell its slot carves.
  [Theory]
  ...
  [InlineData(3, BedSlotSide.West, BedSlotState.Mold, "Mold3E")]
  [InlineData(3, BedSlotSide.West, BedSlotState.Sand, "Mold3EFull")]
  [InlineData(4, BedSlotSide.East, BedSlotState.Mold, "Mold4W")]
  ...
  public void Full_means_full_of_sand_not_full_of_metal() {
    ...
    Assert.Equal("Mold1E", SandBedLayout.ElementFor(flank, BedSlotState.Mold));
  }
  ...
  public void Carving_one_slot_changes_only_that_slot() {
    ...
    Assert.Contains("SandRunners/RunnerRow1/Mold1E/*", after);
  }
