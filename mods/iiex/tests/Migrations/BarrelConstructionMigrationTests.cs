...
/// barrel). Covers both the pre-merge <c>iwex</c> code and the pre-split <c>smex</c> code, and that it
...
public class BarrelConstructionMigrationTests {
  ...
  public void Both_old_barrel_codes_map_to_the_plated_variant() {
    ...
    world.Register(
      TestBlocks.Configure(new Block(), "iiex:molten-barrel-plated", 100)
    );

    var remaps = Remaps(world);
    var plated = new AssetLocation("iiex", "molten-barrel-plated");
