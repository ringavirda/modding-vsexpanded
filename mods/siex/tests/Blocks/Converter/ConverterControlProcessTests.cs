...
public class ConverterControlProcessTests {
  ...
  // populated one (iiex:/siex:) line up between what the test pushes and what the control reads.
  ...
  private static TestWorld NewWorld() {
    ...
    // Register both the convention (game:) and the shipped (iiex:/siex:) codes so the resolved token
    ...
    foreach (
      var (code, melt) in new[]
      {
        ...
        ("siex:ingot-bessemersteel", 1500f),
  ...
  private static BlockEntityConverterControl Control(TestWorld world) {
    var be = new BlockEntityConverterControl {
      ...
      Block = TestBlocks.Configure(
        new Block(),
        "siex:converterbessemercontrol-n",
        1,
        ("side", "north")
      ),
    };
  ...
  private static void GivePig(
    ...
    ReflectionHelpers.SetField(
      be,
      "_carbon",
      SiexValues.BessemerPigCarbonStart
    );
  ...
  public void TickFilling_draws_molten_pig_and_seeds_the_carbon() {
    ...
    Assert.Equal(SiexValues.BessemerPigCarbonStart, CarbonOf(be), 4);
  ...
  public void Blowing_to_the_carbon_target_retypes_pig_to_bessemer_steel() {
    ...
    BlowToCarbon(
      be,
      (SiexValues.BessemerSteelCarbonTarget + SiexValues.BessemerOverblowCarbon)
        / 2f
    );
  ...
  public void Stopping_the_blow_above_the_target_leaves_it_as_pig() {
    ...
    BlowToCarbon(be, SiexValues.BessemerSteelCarbonTarget * 2f);
  ...
  public void Blowing_past_the_target_over_blows_the_steel_to_ingot_iron() {
    ...
    BlowToCarbon(be, SiexValues.BessemerOverblowCarbon / 2f);
  ...
  public void A_full_blow_conserves_mass_steel_plus_slag_never_exceeding_the_pig() {
    ...
    Assert.Equal(1000 * SiexValues.BessemerSteelYield, steel, 0f);
    Assert.Equal(1000 * SiexValues.BessemerSlagYield, slag, 1f);
  }
  ...
  public void A_modest_scrap_charge_still_refines_and_yields_extra_steel() {
    ...
    Assert.True(
      HeatBalance(be, 1f).TProcess >= SiexValues.BessemerRefineTemperature
    );
    ...
    int expected =
      (int)(1000 * SiexValues.BessemerSteelYield)
      + (int)Math.Round(200 * SiexValues.BessemerScrapSteelYield);
    Assert.Contains("bessemersteel", ChargeCode(be));
    Assert.Equal(expected, ChargeUnits(be));
    Assert.True(
      ChargeUnits(be) > 1000 * SiexValues.BessemerSteelYield,
