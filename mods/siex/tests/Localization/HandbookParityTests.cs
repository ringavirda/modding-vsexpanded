...
/// <summary>
/// The handbook authoring pipeline (<see cref="HandbookSync"/>) over siex's own domain: prose written
...
public class HandbookParityTests {
  ...
  public void Every_shipped_handbook_page_matches_its_authoring_source() {
    ...
    foreach (HandbookSync.Page page in HandbookSync.Pages("siex")) {
  ...
  public void Every_handbook_page_is_wired_end_to_end() {
    var problems = HandbookSync.Problems("siex");
    Assert.True(problems.Count == 0, string.Join("\n", problems));
  }

  [Fact]
  public void Siex_ships_at_least_one_handbook_page() {
    ...
    Assert.NotEmpty(HandbookSync.Pages("siex"));
  }
}
