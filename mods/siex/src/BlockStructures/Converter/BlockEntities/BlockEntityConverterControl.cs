...
public partial class BlockEntityConverterControl : BlockEntityMultiblockMachine {
  ...
  #region Tunables (see SiexValues)
  private static int CapacityUnits => SiexValues.BessemerConverterCapacity;
  private static float BlastPerSecond => SiexValues.BessemerBlastPerSecond;
  private static float PowerSpeedThreshold =>
    SiexValues.BessemerPowerSpeedThreshold;
  private static float PourRate => SiexValues.BessemerPourRate;

  // Carbon model + autothermal heat balance. All live config, re-read on every access.
  private static float PigCarbonStart => SiexValues.BessemerPigCarbonStart;
  private static float SteelCarbonTarget =>
    SiexValues.BessemerSteelCarbonTarget;
  private static float OverblowCarbon => SiexValues.BessemerOverblowCarbon;
  private static float CarbonPerBlastLitre =>
    SiexValues.BessemerCarbonPerBlastLitre;
  private static float AutothermalBase => SiexValues.BessemerAutothermalBase;
  private static float HeatPerCarbonUnit =>
    SiexValues.BessemerHeatPerCarbonUnit;
  private static float AutothermalCeiling =>
    SiexValues.BessemerAutothermalCeiling;
  private static float RadiationLoss => SiexValues.BessemerRadiationLoss;
  private static float RefineTemperature =>
    SiexValues.BessemerRefineTemperature;
  private static float ColdScrapLossCoefficient =>
    SiexValues.BessemerColdScrapLossCoefficient;
  private static int ScrapUnitValue => SiexValues.BessemerScrapUnitValue;
  private static float ScrapSteelYield => SiexValues.BessemerScrapSteelYield;
  private static float SteelYield => SiexValues.BessemerSteelYield;
  private static float SlagYield => SiexValues.BessemerSlagYield;
  ...
  private static float ContentCooldownSpeed =>
    IiexValues.MoltenCooldownSpeed * SiexValues.BessemerCooldownCoefficient;
  ...
  private static float ChiselMaxFraction =>
    SiexValues.BessemerChiselMaxFraction;
  ...
  protected override void OnProductionTick(float dt) {
    ...
    if (!IsGasIntakeAligned()) {
      SetStatus(Lang.Get("siex:bessemer-status-misaligned"));
      return;
    }

    if (!IsTransmissionAligned()) {
      SetStatus(Lang.Get("siex:bessemer-status-transmission-misaligned"));
      return;
    }
  ...
  private void TickNormal(float dt) {
    ...
    if (_charge == null || _charge.Units <= 0) {
      SetStatus(Lang.Get("siex:bessemer-status-empty"));
      return;
    }
    ...
    if (!IsBlowable()) {
      SetStatus(
        Lang.Get(
          IsOverblownIron()
            ? "siex:bessemer-status-ironready"
            : "siex:bessemer-status-foreign"
        )
      );
      return;
    }
    ...
    if (blastConsumed <= 0f) {
      ...
      SetStatus(
        Lang.Get(
          IsBessemerSteel()
            ? "siex:bessemer-status-steelready"
            : "siex:bessemer-status-blow-paused",
          CarbonPercentText()
        )
      );
      return;
    ...
    if (_lastHeatBalance.TProcess < RefineTemperature) {
      SetStatus(
        Lang.Get("siex:bessemer-status-blow-stalled", CarbonPercentText())
      );
      return;
    }
  ...
  private void TickFilling(float dt) {
    ...
    if (inputCell is BlockEntityMoltenCanalTap { IsPouring: false }) {
      SetStatus(Lang.Get("siex:bessemer-status-filling-tapclosed"));
      return;
    }

    if (inputCell == null || !inputCell.HasMoltenMetal) {
      SetStatus(Lang.Get("siex:bessemer-status-filling-nometal"));
      return;
    }
    ...
    if (held >= CapacityUnits) {
      SetStatus(Lang.Get("siex:bessemer-status-filling-full"));
      return;
    }
    ...
    if (
      ...
      SetStatus(Lang.Get("siex:bessemer-status-filling-mismatch"));
      return;
    }
  ...
  private void TickSlagPouring(float dt) {
    ...
    if (_moltenSlag <= 0f) {
      SetStatus(Lang.Get("siex:bessemer-status-slag-empty"));
      return;
    }

    var outputCell = GetMoltenCell(OutputStartLocal);
    if (outputCell == null) {
      SetStatus(Lang.Get("siex:bessemer-status-pouring-nocanal"));
      return;
    }
    ...
    if (accepted <= 0f) {
      ...
      SetStatus(Lang.Get("siex:bessemer-status-slag-blocked"));
      return;
    }
    ...
    SetStatus(Lang.Get("siex:bessemer-status-slag-pouring", (int)_moltenSlag));
    MarkDirty();
  }
  ...
  private void TickSteelPouring(float dt) {
    if (_charge == null || _charge.Units <= 0) {
      SetStatus(Lang.Get("siex:bessemer-status-pouring-empty"));
      return;
    }
    ...
    if (accepted <= 0f) {
      ...
      SetStatus(Lang.Get("siex:bessemer-status-pouring-full"));
      return;
    }
    ...
    if (_charge.Units <= 0) {
      ...
      _scrapUnits = 0;
      _shedCarry = 0f;
      SetStatus(Lang.Get("siex:bessemer-status-emptied"));
    } else {
      SetStatus(
        Lang.Get("siex:bessemer-status-pouring", _charge.Units, CapacityUnits)
      );
    }
    MarkDirty();
  }
  ...
  public bool TryChargeScrap(IPlayer byPlayer, out string error) {
    ...
    if (!CanOperate(out error))
      return true;
    if (_solidified) {
      error = Lang.Get("siex:bessemer-err-scrap-solidified");
      return true;
    }
    ...
    if (_charge != null && _charge.MetalCode.ToString() != PigCode) {
      error = Lang.Get("siex:bessemer-err-scrap-notpig");
      return true;
    }
    ...
    if (roomBits <= 0) {
      error = Lang.Get("siex:bessemer-status-filling-full");
      return true;
    }
    ...
    SetStatus(Lang.Get("siex:bessemer-status-scrap-charged", _scrapUnits));
    MarkDirty(true);
    return true;
  }
  ...
  private string BlowingStatus() =>
    Lang.Get(
      IsBessemerSteel()
        ? "siex:bessemer-status-overblowing"
        : "siex:bessemer-status-blowing",
      CarbonPercentText()
    );
  ...
  public bool CanOperate(out string error) {
    ...
    if (!StructureComplete) {
      error = Lang.Get("siex:bessemer-err-incomplete");
      return false;
    }
    if (!IsConverterConstructed()) {
      error = Lang.Get("siex:bessemer-err-notbuilt");
      return false;
    }
    if (!IsGasIntakeAligned()) {
      error = Lang.Get("siex:bessemer-err-intake-misaligned");
      return false;
    }
    if (!IsTransmissionAligned()) {
      error = Lang.Get("siex:bessemer-err-transmission-misaligned");
      return false;
    }
    if (!HasPower()) {
      error = Lang.Get("siex:bessemer-err-nopower");
      return false;
    }
    return true;
  }
  ...
  private string SolidifiedStatus() {
    if ((_charge?.Units ?? 0) >= ChiselMaxFraction * CapacityUnits)
      return Lang.Get("siex:bessemer-status-solidified");

    return Lang.Get(
      ChargeIsHardened
        ? "siex:bessemer-status-chiselout"
        : "siex:bessemer-status-coolingtochisel"
    );
  }
  ...
  protected override string GetIncompleteMessage(int missingCount) =>
    Lang.Get("siex:bessemer-err-incomplete-count", missingCount);

  protected override string GetCompleteMessage() =>
    Lang.Get("siex:bessemer-complete");
