...
public class BlockEntityCowperStove : BlockEntityMultiblockMachine {
  ...
  // Config tunables (see SiexValues), cached in fields by CacheTunables so the tick body does not
  ...
  // so an `/exmod config siex ...` change applies immediately rather than after a chunk reload.
  private void CacheTunables() {
    _factorAnthracite = SiexValues.CowperHeatingSpeedAnthracite;
    _factorOtherCoal = SiexValues.CowperHeatingSpeedOtherCoal;
    _factorDefault = SiexValues.CowperHeatingSpeedDefault;
    _coolingSpeedExhaust = SiexValues.CowperCoolingSpeedExhaust;
    _coolingSpeedAir = SiexValues.CowperCoolingSpeedAir;
    _exhaustAttenuation = SiexValues.CowperExhaustAttenuation;
    _maxTemperature = SiexValues.CowperMaxTemperature;
    _intakeVolume = SiexValues.CowperIntakeVolume;
  }
  ...
  protected override string GetIncompleteMessage(int missingCount) =>
    Lang.Get("siex:structure-incomplete-count", missingCount);

  protected override string GetCompleteMessage() =>
    Lang.Get("siex:cowperstove-complete");
  ...
  protected override void OnProductionTick(float dt) {
    ...
    string newStatus = Lang.Get("siex:cowperstove-status-idle");
    ...
    if (isReceivingExhaust && passthroughVol > ExlibValues.LitresPerPipe) {
      ...
      // stays flagged.
      newStatus = Lang.Get("siex:cowperstove-status-exhaustmix");
      passthrough?.TryConsume(passthroughVol);
    } else if (isReceivingExhaust) {
      newStatus = Lang.Get("siex:cowperstove-status-heatingup");
      ...
      newStatus = Lang.Get("siex:cowperstove-status-heating", inGasType);
  ...
  public override void GetBlockInfo(IPlayer forPlayer, StringBuilder dsc) {
    if (!StructureComplete) {
      dsc.AppendLine(Lang.Get("siex:structure-incomplete"));
      return;
    }
    dsc.AppendLine(Lang.Get("siex:cowperstove-info-status", _lastStatus));
  }
