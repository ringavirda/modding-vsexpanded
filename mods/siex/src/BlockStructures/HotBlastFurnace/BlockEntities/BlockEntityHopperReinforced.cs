...
public class BlockEntityHopperReinforced : ExBlockEntity {
  ...
  /// feeding.</summary>
  public int Capacity => SiexValues.HopperReinforcedCapacity;
  ...
  public override void GetBlockInfo(IPlayer forPlayer, StringBuilder dsc) {
    ...
    if (_tank == null || _tank.StackSize <= 0)
      dsc.AppendLine(Lang.Get("siex:hopper-empty"));
    else
      dsc.AppendLine(
        Lang.Get(
          "siex:hopper-holds",
    ...
    if (
      ...
      if (bell.IsFurnaceFull())
        dsc.AppendLine(Lang.Get("siex:hopper-info-furnacefull"));
    } else {
      dsc.AppendLine(Lang.Get("siex:hopper-info-nobell"));
    }
  }
